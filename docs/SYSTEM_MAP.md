...
const fetchClassrooms = async (userId: string, role: UserRole, tenantId: string) => {
    if (role === 'teacher') {
        return supabase.from('classes').select('*')
            .eq('teacher_id', userId).eq('tenant_id', tenantId);
    }
...
*This document serves as the single source of truth for all AI agents. Any changes to module architecture must be reflected here.*
