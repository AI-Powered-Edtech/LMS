...
Request Flow:
  Browser --> React Router
    --> AuthGuard (must be logged in)
      --> TenantGuard (must have tenant selected)
        --> RoleGuard / RoleRoute (role check)
          --> [optional] CourseEnrollmentGuard
            --> Page Component
              --> Supabase Client (JS SDK)
                --> PostgreSQL RLS (tenant_id + role policies)
                  --> SECURITY DEFINER RPCs (additional role checks)
```

**Frontend layer** (React): `RoleGuard` / `RoleRoute` prevents UI access.
**Database layer** (PostgreSQL): RLS policies + RPC role checks prevent data access.

Both layers must agree. Even if a user bypasses the frontend guard (e.g., direct API call),
the RLS policies and RPC role checks enforce the same permissions at the database level.
...
| ---------------------------- | :-----: | :-----: | :---: |
| View own dashboard           |    x    |    x    |   x   |
| Enroll in classes            |    x    |         |       |
| View lessons / take quizzes  |    x    |         |       |
| Track own progress & grades  |    x    |         |       |
| View leaderboard             |    x    |    x    |       |
| View certificates            |    x    |         |       |
| Create / edit courses        |         |    x    |   x   |
| Create / manage classes      |         |    x    |   x   |
| Create / edit quizzes        |         |    x    |   x   |
| Grade quiz attempts          |         |    x    |   x   |
| Grade assignment submissions |         |    x    |   x   |
| View course analytics        |         |    x    |   x   |
| Manage question bank         |         |    x    |   x   |
| Scan attendance              |         |    x    |   x   |
| Manage documents             |         |    x    |   x   |
| View forum / announcements   |    x    |    x    |   x   |
| Manage announcements         |         |    x    |   x   |
| Content moderation           |         |    x    |   x   |
| Manage users & roles         |         |         |   x   |
| Manage billing & finance     |         |         |   x   |
| PPDB (student admission)     |         |         |   x   |
| View audit logs              |         |         |   x   |
| Admin analytics dashboard    |         |         |   x   |
| Manage tenant settings       |         |         |   x   |
| Invite / suspend users       |         |         |   x   |
