# Kemajuan Belajar (progress)

## Overview

Tracking progress belajar siswa secara granular per kursus, modul, dan pelajaran. Visualisasi kemajuan.

Modul ini merupakan bagian dari arsitektur feature-module EduSync LMS, terletak di `src/features/progress/`. Setiap feature module memiliki struktur standar: api/, queries/, hooks/, types/, components/, dan **tests**/.
...
Tabel yang digunakan oleh progress dilindungi oleh RLS policy:

```sql
CREATE POLICY "tenant_isolation" ON student_progress
  USING (tenant_id = (SELECT get_my_tenant_id()));
```

## Database Tables

- `student_progress` — Tabel student progress untuk Kemajuan Belajar
- `module_progress` — Tabel module progress untuk Kemajuan Belajar

## RPC / Edge Functions

Fungsi-fungsi database yang terkait dengan progress:

- `get_progress_stats()` — Statistik aggregat
- `search_progress()` — Full-text search

## UI Pages

| Route                     | Deskripsi                            | Role    |
| ------------------------- | ------------------------------------ | ------- |
| `/#/app/student/progress` | Halaman Kemajuan Belajar untuk siswa | Student |
| `/#/app/teacher/progress` | Halaman Kemajuan Belajar untuk guru  | Teacher |
| `/#/app/admin/progress`   | Halaman Kemajuan Belajar untuk admin | Admin   |

## Komponen

- **ProgressSkeleton** — Loading skeleton
- **ProgressCard** — Kartu item
- **ProgressTable** — Tabel data
- **ProgressStats** — Kartu statistik
- **ProgressFilterBar** — Bar pencarian dan filter

## Dependencies

Feature yang di-depend oleh progress:
...
Feature yang depend ke progress:
