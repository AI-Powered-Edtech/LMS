...
Manajemen tenant, konfigurasi modul sekolah, sinkronisasi data. Digunakan oleh admin sekolah untuk mengatur fitur-fitur yang aktif per tenant.
...
Tabel yang digunakan oleh administration dilindungi oleh RLS policy:
...
- `tenants` — Tabel tenants untuk Administrasi
- `tenant_modules` — Tabel tenant modules untuk Administrasi
- `sync_history` — Tabel sync history untuk Administrasi

## RPC / Edge Functions

Fungsi-fungsi database yang terkait dengan administration:

- `get_administration_stats()` — Statistik aggregat
- `search_administration()` — Full-text search

## UI Pages

| Route                           | Deskripsi                        | Role    |
| ------------------------------- | -------------------------------- | ------- |
| `/#/app/student/administration` | Halaman Administrasi untuk siswa | Student |
| `/#/app/teacher/administration` | Halaman Administrasi untuk guru  | Teacher |
| `/#/app/admin/administration`   | Halaman Administrasi untuk admin | Admin   |

## Komponen

- **AdministrationSkeleton** — Loading skeleton
- **AdministrationCard** — Kartu item
- **AdministrationTable** — Tabel data
- **AdministrationStats** — Kartu statistik
- **AdministrationFilterBar** — Bar pencarian dan filter

## Dependencies

Feature yang di-depend oleh administration:
...
Feature yang depend ke administration:
