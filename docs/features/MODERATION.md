...
Moderasi konten user-generated (diskusi, komentar). Filter otomatis dan review queue untuk admin.

Modul ini merupakan bagian dari arsitektur feature-module EduSync LMS, terletak di `src/features/moderation/`. Setiap feature module memiliki struktur standar: api/, queries/, hooks/, types/, components/, dan **tests**/.
...
Tabel yang digunakan oleh moderation dilindungi oleh RLS policy:
...
- `moderation_actions` — Tabel moderation actions untuk Moderasi
- `moderation_queue` — Tabel moderation queue untuk Moderasi

## RPC / Edge Functions

Fungsi-fungsi database yang terkait dengan moderation:

- `get_moderation_stats()` — Statistik aggregat
- `search_moderation()` — Full-text search

## UI Pages

| Route                       | Deskripsi                    | Role    |
| --------------------------- | ---------------------------- | ------- |
| `/#/app/student/moderation` | Halaman Moderasi untuk siswa | Student |
| `/#/app/teacher/moderation` | Halaman Moderasi untuk guru  | Teacher |
| `/#/app/admin/moderation`   | Halaman Moderasi untuk admin | Admin   |

## Komponen

- **ModerationSkeleton** — Loading skeleton
- **ModerationCard** — Kartu item
- **ModerationTable** — Tabel data
- **ModerationStats** — Kartu statistik
- **ModerationFilterBar** — Bar pencarian dan filter

## Dependencies

Feature yang di-depend oleh moderation:
...
Feature yang depend ke moderation:
