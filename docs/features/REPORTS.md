...
Generator laporan akademik, keuangan (SPP), PPDB, dan custom. Mendukung scheduling dan export PDF/Excel.

Modul ini merupakan bagian dari arsitektur feature-module EduSync LMS, terletak di `src/features/reports/`. Setiap feature module memiliki struktur standar: api/, queries/, hooks/, types/, components/, dan **tests**/.
...
Tabel yang digunakan oleh reports dilindungi oleh RLS policy:
...
- `reports` — Tabel reports untuk Laporan
- `report_schedules` — Tabel report schedules untuk Laporan

## RPC / Edge Functions

Fungsi-fungsi database yang terkait dengan reports:

- `get_reports_stats()` — Statistik aggregat
...
| ------------------------ | --------------------------- | ------- |
| `/#/app/student/reports` | Halaman Laporan untuk siswa | Student |
| `/#/app/teacher/reports` | Halaman Laporan untuk guru  | Teacher |
| `/#/app/admin/reports`   | Halaman Laporan untuk admin | Admin   |

## Komponen

- **ReportsSkeleton** — Loading skeleton
- **ReportsCard** — Kartu item
- **ReportsTable** — Tabel data
- **ReportsStats** — Kartu statistik
- **ReportsFilterBar** — Bar pencarian dan filter

## Dependencies

Feature yang di-depend oleh reports:
...
Feature yang depend ke reports:
