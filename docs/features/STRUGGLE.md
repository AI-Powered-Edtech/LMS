# Deteksi Kesulitan (struggle)

## Overview

Deteksi otomatis siswa yang kesulitan berdasarkan pola belajar, waktu per soal, dan penurunan performa.

Modul ini merupakan bagian dari arsitektur feature-module EduSync LMS, terletak di `src/features/struggle/`. Setiap feature module memiliki struktur standar: api/, queries/, hooks/, types/, components/, dan **tests**/.
...
Tabel yang digunakan oleh struggle dilindungi oleh RLS policy:

```sql
CREATE POLICY "tenant_isolation" ON struggle_alerts
  USING (tenant_id = (SELECT get_my_tenant_id()));
```

## Database Tables

- `struggle_alerts` — Tabel struggle alerts untuk Deteksi Kesulitan
- `struggle_config` — Tabel struggle config untuk Deteksi Kesulitan

## RPC / Edge Functions

Fungsi-fungsi database yang terkait dengan struggle:

- `get_struggle_stats()` — Statistik aggregat
- `search_struggle()` — Full-text search

## UI Pages

| Route                     | Deskripsi                             | Role    |
| ------------------------- | ------------------------------------- | ------- |
| `/#/app/student/struggle` | Halaman Deteksi Kesulitan untuk siswa | Student |
| `/#/app/teacher/struggle` | Halaman Deteksi Kesulitan untuk guru  | Teacher |
| `/#/app/admin/struggle`   | Halaman Deteksi Kesulitan untuk admin | Admin   |

## Komponen

- **StruggleSkeleton** — Loading skeleton
- **StruggleCard** — Kartu item
- **StruggleTable** — Tabel data
- **StruggleStats** — Kartu statistik
- **StruggleFilterBar** — Bar pencarian dan filter

## Dependencies

Feature yang di-depend oleh struggle:
...
Feature yang depend ke struggle:
