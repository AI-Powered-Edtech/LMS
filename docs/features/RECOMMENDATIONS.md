...
Engine rekomendasi konten berdasarkan progress, performa, dan pola belajar siswa. Smart next button.

Modul ini merupakan bagian dari arsitektur feature-module EduSync LMS, terletak di `src/features/recommendations/`. Setiap feature module memiliki struktur standar: api/, queries/, hooks/, types/, components/, dan **tests**/.
...
Tabel yang digunakan oleh recommendations dilindungi oleh RLS policy:
...
- `recommendations` — Tabel recommendations untuk Rekomendasi

## RPC / Edge Functions

Fungsi-fungsi database yang terkait dengan recommendations:

- `get_recommendations_stats()` — Statistik aggregat
- `search_recommendations()` — Full-text search

## UI Pages

| Route                            | Deskripsi                       | Role    |
| -------------------------------- | ------------------------------- | ------- |
| `/#/app/student/recommendations` | Halaman Rekomendasi untuk siswa | Student |
| `/#/app/teacher/recommendations` | Halaman Rekomendasi untuk guru  | Teacher |
| `/#/app/admin/recommendations`   | Halaman Rekomendasi untuk admin | Admin   |

## Komponen

- **RecommendationsSkeleton** — Loading skeleton
- **RecommendationsCard** — Kartu item
- **RecommendationsTable** — Tabel data
- **RecommendationsStats** — Kartu statistik
- **RecommendationsFilterBar** — Bar pencarian dan filter

## Dependencies

Feature yang di-depend oleh recommendations:
...
Feature yang depend ke recommendations:
