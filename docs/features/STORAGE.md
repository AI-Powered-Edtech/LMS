...
Manajemen file dan media untuk materi pembelajaran. Upload, preview, dan organisasi file per kursus.

Modul ini merupakan bagian dari arsitektur feature-module EduSync LMS, terletak di `src/features/storage/`. Setiap feature module memiliki struktur standar: api/, queries/, hooks/, types/, components/, dan **tests**/.

## Domain

**Infrastructure** — Modul ini termasuk dalam domain Infrastructure bersama dengan feature terkait lainnya.
...
Tabel yang digunakan oleh storage dilindungi oleh RLS policy:
...
- `storage_files` — Tabel storage files untuk Penyimpanan

## RPC / Edge Functions

Fungsi-fungsi database yang terkait dengan storage:

- `get_storage_stats()` — Statistik aggregat
- `search_storage()` — Full-text search

## UI Pages

| Route                    | Deskripsi                       | Role    |
| ------------------------ | ------------------------------- | ------- |
| `/#/app/student/storage` | Halaman Penyimpanan untuk siswa | Student |
| `/#/app/teacher/storage` | Halaman Penyimpanan untuk guru  | Teacher |
| `/#/app/admin/storage`   | Halaman Penyimpanan untuk admin | Admin   |

## Komponen

- **StorageSkeleton** — Loading skeleton
- **StorageCard** — Kartu item
- **StorageTable** — Tabel data
- **StorageStats** — Kartu statistik
- **StorageFilterBar** — Bar pencarian dan filter

## Dependencies

Feature yang di-depend oleh storage:
...
Feature yang depend ke storage:
