...
Sistem panduan in-app (tooltip, walkthrough, banner, checkpoint). Membantu onboarding dan feature discovery.

Modul ini merupakan bagian dari arsitektur feature-module EduSync LMS, terletak di `src/features/guidance/`. Setiap feature module memiliki struktur standar: api/, queries/, hooks/, types/, components/, dan **tests**/.
...
Tabel yang digunakan oleh guidance dilindungi oleh RLS policy:

```sql
CREATE POLICY "tenant_isolation" ON guides
  USING (tenant_id = (SELECT get_my_tenant_id()));
```

## Database Tables

- `guides` — Tabel guides untuk Panduan
- `guide_completions` — Tabel guide completions untuk Panduan

## RPC / Edge Functions

Fungsi-fungsi database yang terkait dengan guidance:

- `get_guidance_stats()` — Statistik aggregat
- `search_guidance()` — Full-text search

## UI Pages

| Route                     | Deskripsi                   | Role    |
| ------------------------- | --------------------------- | ------- |
| `/#/app/student/guidance` | Halaman Panduan untuk siswa | Student |
| `/#/app/teacher/guidance` | Halaman Panduan untuk guru  | Teacher |
| `/#/app/admin/guidance`   | Halaman Panduan untuk admin | Admin   |

## Komponen

- **GuidanceSkeleton** — Loading skeleton
- **GuidanceCard** — Kartu item
- **GuidanceTable** — Tabel data
- **GuidanceStats** — Kartu statistik
- **GuidanceFilterBar** — Bar pencarian dan filter

## Dependencies

Feature yang di-depend oleh guidance:
...
Feature yang depend ke guidance:
