...
Asisten belajar berbasis AI yang memberikan penjelasan personal kepada siswa. Menggunakan Edge Functions untuk integrasi dengan LLM provider.

Modul ini merupakan bagian dari arsitektur feature-module EduSync LMS, terletak di `src/features/ai-tutor/`. Setiap feature module memiliki struktur standar: api/, queries/, hooks/, types/, components/, dan **tests**/.
...
Tabel yang digunakan oleh ai-tutor dilindungi oleh RLS policy:

```sql
CREATE POLICY "tenant_isolation" ON ai_tutor_sessions
  USING (tenant_id = (SELECT get_my_tenant_id()));
```

## Database Tables

- `ai_tutor_sessions` — Tabel ai tutor sessions untuk AI Tutor
- `ai_tutor_messages` — Tabel ai tutor messages untuk AI Tutor

## RPC / Edge Functions

Fungsi-fungsi database yang terkait dengan ai-tutor:

- `get_ai_tutor_stats()` — Statistik aggregat
- `search_ai_tutor()` — Full-text search

## UI Pages

| Route                     | Deskripsi                    | Role    |
| ------------------------- | ---------------------------- | ------- |
| `/#/app/student/ai-tutor` | Halaman AI Tutor untuk siswa | Student |
| `/#/app/teacher/ai-tutor` | Halaman AI Tutor untuk guru  | Teacher |
| `/#/app/admin/ai-tutor`   | Halaman AI Tutor untuk admin | Admin   |

## Komponen

- **AiTutorSkeleton** — Loading skeleton
- **AiTutorCard** — Kartu item
- **AiTutorTable** — Tabel data
- **AiTutorStats** — Kartu statistik
- **AiTutorFilterBar** — Bar pencarian dan filter

## Dependencies

Feature yang di-depend oleh ai-tutor:
...
Feature yang depend ke ai-tutor:
