...
Repositori soal yang bisa digunakan ulang di berbagai kuis. Mendukung search, tagging, dan import/export.

Modul ini merupakan bagian dari arsitektur feature-module EduSync LMS, terletak di `src/features/question-bank/`. Setiap feature module memiliki struktur standar: api/, queries/, hooks/, types/, components/, dan **tests**/.
...
Tabel yang digunakan oleh question-bank dilindungi oleh RLS policy:
...
- `quiz_questions` — Tabel quiz questions untuk Bank Soal
- `quiz_options` — Tabel quiz options untuk Bank Soal

## RPC / Edge Functions

Fungsi-fungsi database yang terkait dengan question-bank:

- `get_question_bank_stats()` — Statistik aggregat
- `search_question_bank()` — Full-text search

## UI Pages

| Route                          | Deskripsi                     | Role    |
| ------------------------------ | ----------------------------- | ------- |
| `/#/app/student/question-bank` | Halaman Bank Soal untuk siswa | Student |
| `/#/app/teacher/question-bank` | Halaman Bank Soal untuk guru  | Teacher |
| `/#/app/admin/question-bank`   | Halaman Bank Soal untuk admin | Admin   |

## Komponen

- **QuestionBankSkeleton** — Loading skeleton
- **QuestionBankCard** — Kartu item
- **QuestionBankTable** — Tabel data
- **QuestionBankStats** — Kartu statistik
- **QuestionBankFilterBar** — Bar pencarian dan filter

## Dependencies

Feature yang di-depend oleh question-bank:
...
Feature yang depend ke question-bank:
