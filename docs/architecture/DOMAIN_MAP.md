...
| `roles`         | Daftar peran yang tersedia                              | `id`, `name` (STUDENT, TEACHER, ADMIN, PARENT, PRINCIPAL)      |
...
> **`app_role` enum values (Phase 30):** `'student' | 'teacher' | 'admin' | 'parent' | 'principal'`  
> PARENT: read-only akses ke data anak via `student_parent_links`.  
> PRINCIPAL: read-only akses ke seluruh data tenant untuk executive analytics.
...
**Fokus:** Pemantauan orang tua/wali terhadap perkembangan anak secara read-only.

| Entity                    | Purpose                                | Fitur Penting                                                                 |
| ------------------------- | -------------------------------------- | ----------------------------------------------------------------------------- |
| `student_parent_links`    | Relasi orang tua ↔ siswa               | `id`, `tenant_id`, `student_id`, `parent_id`, `relationship`                  |
| `parent_otp_codes`        | Kode OTP untuk registrasi via nomor HP | `id`, `tenant_id`, `phone_number`, `otp_code`, `expires_at`, `verified`       |
| `parent_digest_settings`  | Preferensi notifikasi harian orang tua | `id`, `tenant_id`, `parent_id`, `channel`, `time_preference`, `is_enabled`    |
| `parent_teacher_threads`  | Thread percakapan orang tua ↔ guru     | `id`, `tenant_id`, `parent_id`, `teacher_id`, `student_id`, `last_message_at` |
| `parent_teacher_messages` | Pesan individual dalam thread          | `id`, `tenant_id`, `thread_id`, `sender_id`, `content`, `read_at`             |

**RPCs:**

- `get_my_children()` — Fetch daftar anak yang ter-link ke parent (auth'd)
- `request_parent_otp(p_phone)` — Kirim OTP ke nomor HP untuk registrasi
- `verify_parent_otp(p_phone, p_code)` — Verifikasi OTP dan aktifkan akun parent

**Routes:** `/app/parent/*`  
**Feature module:** `src/features/parent/`  
**Layout:** `src/components/layout/ParentLayout.tsx` (mobile-first)

> **RLS:** Parent hanya bisa mengakses data anak yang ada di `student_parent_links`. Tidak ada cross-tenant access.

---

## 13. 🏫 Principal Domain

**Fokus:** Dashboard eksekutif untuk Kepala Sekolah dengan akses read-only ke seluruh data tenant.

| Entity                    | Purpose                                               | Fitur Penting                                                                |
| ------------------------- | ----------------------------------------------------- | ---------------------------------------------------------------------------- |
| `principal_settings`      | Konfigurasi dashboard kepala sekolah                  | `id`, `tenant_id`, `principal_id`, `report_schedule`, `preferences`          |
| `school_baseline_metrics` | Data baseline sebelum LMS untuk before-after analysis | `id`, `tenant_id`, `metric_name`, `baseline_value`, `recorded_at`            |
| `satisfaction_surveys`    | Definisi survey kepuasan per tenant                   | `id`, `tenant_id`, `title`, `target_role`, `status`, `created_by`            |
| `survey_responses`        | Jawaban survey dari responden                         | `id`, `tenant_id`, `survey_id`, `respondent_id`, `responses`, `submitted_at` |

**RPCs:**

- `get_executive_overview()` — Ringkasan metrics eksekutif (adoption, academic, ROI) untuk seluruh tenant

**Routes:** `/app/principal/*`  
**Feature module:** `src/features/principal/`  
**Layout:** `src/components/layout/PrincipalLayout.tsx`

> **RLS:** Principal mendapat read-only access ke semua data dalam tenant-nya. Tidak bisa modifikasi data akademik.
...
- **Idempotency Guards:** Semua tabel Activity Consumer (`user_points`, `notifications`, `leaderboards`, dll) harus mengecek kolom referensi ke `activity_events` untuk meminimalisasi _duplicate writes_ yang sering terjadi di environment serverless.
...
- **Frontend Restrictions:** Tidak ada perhitungan agregasi berat di frontend. Frontend _hanya_ men-query `user_points` atau `course_progress` tanpa melalukan kalkulasi `COUNT(*)` sama sekali.
