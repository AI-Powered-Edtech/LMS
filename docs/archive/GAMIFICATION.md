...
Sistem gamifikasi lengkap: XP, badge, level, streak counter, dan leaderboard. Meningkatkan motivasi belajar siswa.

Modul ini merupakan bagian dari arsitektur feature-module EduSync LMS, terletak di `src/features/gamification/`. Setiap feature module memiliki struktur standar: api/, queries/, hooks/, types/, components/, dan **tests**/.

## Domain

**Engagement** — Modul ini termasuk dalam domain Engagement bersama dengan feature terkait lainnya.
...
Tabel yang digunakan oleh gamification dilindungi oleh RLS policy:
...
- `xp_events` — Tabel xp events untuk Gamifikasi
- `badges` — Tabel badges untuk Gamifikasi
- `user_badges` — Tabel user badges untuk Gamifikasi
- `streaks` — Tabel streaks untuk Gamifikasi
- `leaderboard_cache` — Tabel leaderboard cache untuk Gamifikasi

## RPC / Edge Functions

Fungsi-fungsi database yang terkait dengan gamification:

- `get_gamification_stats()` — Statistik aggregat
- `search_gamification()` — Full-text search

## UI Pages

| Route                         | Deskripsi                      | Role    |
| ----------------------------- | ------------------------------ | ------- |
| `/#/app/student/gamification` | Halaman Gamifikasi untuk siswa | Student |
| `/#/app/teacher/gamification` | Halaman Gamifikasi untuk guru  | Teacher |
| `/#/app/admin/gamification`   | Halaman Gamifikasi untuk admin | Admin   |

## Komponen

- **GamificationSkeleton** — Loading skeleton
- **GamificationCard** — Kartu item
- **GamificationTable** — Tabel data
- **GamificationStats** — Kartu statistik
- **GamificationFilterBar** — Bar pencarian dan filter

## Dependencies

Feature yang di-depend oleh gamification:
...
Feature yang depend ke gamification:
