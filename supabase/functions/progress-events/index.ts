...
Deno.serve(async (req: Request) => {
    ...
    try {
        ...
        if (!Array.isArray(body)) {
            return new Response(
                JSON.stringify({ error: "Request body must be an array of events" }),
    ...
    try {
        ...
        if (!metricsError && metrics) {
            ...
            if (queueLength > QUEUE_BACKPRESSURE_LIMIT) {
                ...
                return new Response(
                    JSON.stringify({ error: "Server busy. Please retry later." }),
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (result.valid) {
            validEvents.push(events[i] as TelemetryEvent);
        } else {
    ...
    if (validEvents.length > 0) {
        ...
        if (queueError) {
            ...
            return new Response(
                JSON.stringify({ error: "Failed to enqueue events", detail: queueError.message }),
