...
function errorResponse(message: string, status = 500, retryAfter?: number) {
    const headers: Record<string, string> = { 'Content-Type': 'application/json', ...getCorsHeaders() };
...
async function getOrCreateSession(supabase: SupabaseClient, userId: string, lessonId: string, tenantId: string, sessionId?: string): Promise<TutorSession> {
    const start = performance.now();
...
async function fetchHistoryMessages(supabase: SupabaseClient, sessionId: string): Promise<ChatMessage[]> {
    const start = performance.now();
...
async function fetchEnhancedContext(supabase: SupabaseClient, userId: string, lessonId: string, tenantId: string, question: string): Promise<TutorContext> {
    const start = performance.now();
...
function buildMessages(context: TutorContext, difficulty: StudentDifficulty, question: string): ChatMessage[] {
...
Deno.serve(async (req: Request) => {
    ...
    try {
        ...
        if (err.message === 'RATE_LIMIT_MINUTE') return errorResponse('Terlalu banyak permintaan.', 429, err.retryAfter);
