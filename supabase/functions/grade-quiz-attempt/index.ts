...
Deno.serve(async (req: Request) => {
    ...
    try {
        ...
        const { data: ticket, error: checkoutError } = await supabase.rpc('v1_checkout_submission_queue');
        ...
        if (questionIds.length > 0) {
            ...
            for (const aq of attemptQuestions || []) {
                ...
                const bankRec = Array.isArray(questionDef.question_bank) ? questionDef.question_bank[0] : questionDef.question_bank;
                ...
                if (Array.isArray(studentAns)) {
                    ...
                    if (studentAns.length === correctOptionIds.length &&
            ...
            for (const gq of gradedQuestions) {
                await supabase
                    ...
                    .match({ attempt_id: gq.attempt_id, question_id: gq.question_id, started_at: gq.started_at });
            }
        }

        // 6. Update Attempt status and score
        // started_at is part of PK for v2
        const attemptStartedAt = attemptQuestions && attemptQuestions.length > 0 ? attemptQuestions[0].started_at : null;
