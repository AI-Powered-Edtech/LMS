-- ============================================================
-- SP-20: Achievement System (Badges & Certificates)
...
-- pg_cron: :07 every 5 min
-- ============================================================

-- 1. badge_definitions — system + tenant custom badges
CREATE TABLE IF NOT EXISTS badge_definitions (
    id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id   UUID REFERENCES tenants(id),          -- NULL = system badge
    name        TEXT NOT NULL,
    description TEXT NOT NULL,
    icon_emoji  TEXT NOT NULL DEFAULT '🏅',            -- emoji, not URL
...
CREATE INDEX IF NOT EXISTS idx_badge_def_tenant ON badge_definitions(tenant_id) WHERE tenant_id IS NOT NULL;
ALTER TABLE badge_definitions ENABLE ROW LEVEL SECURITY;

-- Students + teachers can read badges (system + own tenant)
DROP POLICY IF EXISTS "badge_def_read" ON badge_definitions;
CREATE POLICY "badge_def_read" ON badge_definitions FOR SELECT
    USING (tenant_id IS NULL OR tenant_id = get_my_tenant_id());

-- Teachers/admins manage tenant badges
DROP POLICY IF EXISTS "badge_def_manage" ON badge_definitions;
CREATE POLICY "badge_def_manage" ON badge_definitions FOR ALL
    USING (tenant_id = get_my_tenant_id() AND has_role('TEACHER'::app_role))
    WITH CHECK (tenant_id = get_my_tenant_id() AND has_role('TEACHER'::app_role));


-- 2. student_badges — earned badges
...
CREATE INDEX IF NOT EXISTS idx_student_badges_user ON student_badges(tenant_id, user_id);
ALTER TABLE student_badges ENABLE ROW LEVEL SECURITY;

-- Students read own badges, teachers read all in tenant
...
-- System inserts via RPCs (SECURITY DEFINER)
...
CREATE INDEX IF NOT EXISTS idx_certificates_user ON certificates(tenant_id, user_id);
...
CREATE POLICY "certificates_manage" ON certificates FOR INSERT
    WITH CHECK (tenant_id = get_my_tenant_id() AND has_role('TEACHER'::app_role));
...
-- get_student_badges: returns earned + all badge definitions for showcase
...
AS $$
    ...
    WHERE bd.is_active = true
      AND (bd.tenant_id IS NULL OR bd.tenant_id = get_my_tenant_id())
    ORDER BY
...
-- check_badge_eligibility: bulk-check and award badges
CREATE OR REPLACE FUNCTION check_badge_eligibility(p_user_id UUID DEFAULT NULL)
RETURNS INTEGER
        ...
        FOREACH v_user IN ARRAY v_users LOOP
            ...
            IF (v_badge.criteria->>'type') = 'lessons_completed' THEN
                IF (
                    ...
                    INSERT INTO student_badges (tenant_id, user_id, badge_id)
            ...
            ELSIF (v_badge.criteria->>'type') = 'course_completed' THEN
                -- Check if user completed any course (all lessons is_completed)
                IF EXISTS (
                    SELECT 1
                    FROM courses c
                    ...
                    HAVING count(*) = count(*) FILTER (
                        WHERE EXISTS (
                            SELECT 1 FROM student_lesson_signals sls
            ...
            ELSIF (v_badge.criteria->>'type') = 'course_master' THEN
                -- Complete a course with avg quiz score >= 90
                IF EXISTS (
                    SELECT 1
                    FROM courses c
                    JOIN lessons l ON l.course_id = c.id
                    ...
                    HAVING
                        ...
                        AND coalesce(avg(
                            (SELECT sls.best_quiz_score FROM student_lesson_signals sls
            ...
            ELSIF (v_badge.criteria->>'type') = 'speed_learner' THEN
                -- Completed a lesson in < 50% of average time
                IF EXISTS (
                    SELECT 1
                    FROM student_lesson_signals sls
                    JOIN lesson_analytics_summary las ON las.lesson_id = sls.lesson_id
                    WHERE sls.user_id = v_user
...
BEGIN
    -- Teacher only
    IF NOT has_role('TEACHER'::app_role) THEN
    ...
    RETURNING certificates.id, certificates.certificate_number, certificates.issued_at
...
CREATE OR REPLACE FUNCTION get_student_certificates(p_user_id UUID)
RETURNS TABLE (
    id                 UUID,
    course_id          UUID,
    course_title       TEXT,
...
-- ============================================================
INSERT INTO badge_definitions (tenant_id, name, description, icon_emoji, badge_type, criteria, xp_reward, rarity)
VALUES
...
-- ============================================================
-- pg_cron: :07 every 5 min — check badge eligibility
-- (XP awarding integrated in migration 822)
-- ============================================================
DO $$ BEGIN
...
SELECT cron.schedule(
    ...
    '7-59/5 * * * *',
    $$SELECT check_badge_eligibility(NULL)$$
);
