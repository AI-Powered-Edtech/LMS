-- =============================================================================
-- Migration 291: Fix v1_submit_quiz_attempt grading logic
--
-- Bugs fixed:
-- 1. jsonb_build_array(UUID[]) wraps entire array as one element → use to_jsonb()
-- 2. Grading read from non-existent quiz_attempt_answers → quiz_attempt_questions_v2
-- 3. Correct options read from non-existent quiz_question_options → quiz_options
-- =============================================================================
...
BEGIN
    ...
    -- FIX 1: Use UNNEST(UUID[]) instead of jsonb_build_array(UUID[])
    -- FIX 2: Read student_answers from quiz_attempt_questions_v2 (not quiz_attempt_answers)
    -- FIX 3: Read correct options from quiz_options (not quiz_question_options)
    ...
    LOOP
        ...
        IF v_question.question_type IN ('MCQ', 'TRUE_FALSE', 'MULTIPLE_SELECT') THEN
            -- Read selected option IDs from quiz_attempt_questions_v2.student_answers (JSONB array)
            SELECT ARRAY(
            ...
            -- INSERT if not exists (student may not have answered this question)
            IF NOT FOUND THEN
    ...
    -- Recalculate from stored records for accuracy
    v_points_earned := (
