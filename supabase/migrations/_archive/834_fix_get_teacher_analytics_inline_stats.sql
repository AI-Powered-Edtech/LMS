-- Fix get_teacher_analytics: compute all stats inline instead of via
-- refresh_course_stats + course_stats, whose schema doesn't match what
-- the function expects (course_stats has avg_score, not avg_progress etc.)
...
BEGIN
    -- Security: Validate role using has_role() (checks user_roles table)
    ...
    -- Get tenant (falls back to profiles table when JWT has no tenant_id)
