  ...
  dbPromise = new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(STORES.QUIZ_ANSWERS)) {
        const answersStore = db.createObjectStore(STORES.QUIZ_ANSWERS, {
          keyPath: "id",
        });
...
export async function addToSyncQueue(
  item: Omit<SyncQueueItem, "attempts">,
): Promise<void> {
  const db = await openDB();
...
export async function saveBuilderDraft(
  ...
  const existing = await wrapRequest<BuilderDraft | undefined>(
    store.get(courseId),
  );
...
export async function getBuilderDraft(
  ...
  const store = tx.objectStore(STORES.BUILDER_DRAFTS);
  const result = await wrapRequest<BuilderDraft | undefined>(
    store.get(courseId),
  );
  return result?.state ?? null;
}

export async function getBuilderDraftRecord(
  courseId: string,
): Promise<BuilderDraft | null> {
  const db = await openDB();
  const tx = db.transaction(STORES.BUILDER_DRAFTS, "readonly");
  const store = tx.objectStore(STORES.BUILDER_DRAFTS);
  const result = await wrapRequest<BuilderDraft | undefined>(
...
export async function getAllDirtyDrafts(): Promise<unknown[]> {
  ...
  const result = await wrapRequest<
    { courseId: string; state: unknown; savedAt: number }[]
  >(store.getAll());
...
export async function getPendingCount(): Promise<number> {
  try {
    ...
    const tx = db.transaction(
      [STORES.SYNC_QUEUE, STORES.UPLOAD_QUEUE],
      "readonly",
    );
 ...
 */
/** @internal Reserved for future use */
export async function cacheAnswerEncrypted(
  answer: CachedAnswer,
  userId: string,
): Promise<void> {
 ...
 */
/** @internal Reserved for future use */
export async function getAnswersEncrypted(
  quizId: string,
  userId: string,
