...
export function computeXPToNextLevel(totalXP: number): { current: number; needed: number; pct: number } {
...
export function rankLeaderboard<T extends { id: string; total_xp?: number; streak_current?: number }>(
  students: T[],
  sortBy: 'xp' | 'streak' = 'xp'
): Array<T & { rank: number }> {
