...
describe('statusTranslations', () => {
  ...
  describe('translateDbError', () => {
    it('should translate app_role error', () => {
      expect(translateDbError('invalid app_role')).toBe('Terjadi kesalahan konfigurasi. Hubungi administrator.')
      expect(translateDbError('invalid input value for enum')).toBe('Terjadi kesalahan konfigurasi. Hubungi administrator.')
    })

    it('should translate Unauthorized error', () => {
      expect(translateDbError('Unauthorized access')).toBe('Anda tidak memiliki akses untuk tindakan ini.')
      expect(translateDbError('Error P0002')).toBe('Anda tidak memiliki akses untuk tindakan ini.')
    })

    it('should translate not found error', () => {
      expect(translateDbError('record not found')).toBe('Data tidak ditemukan.')
      expect(translateDbError('Error P0001')).toBe('Data tidak ditemukan.')
    })

    it('should translate duplicate error', () => {
      expect(translateDbError('duplicate key value')).toBe('Data sudah ada. Tidak bisa membuat duplikat.')
      expect(translateDbError('Error 23505')).toBe('Data sudah ada. Tidak bisa membuat duplikat.')
    })

    it('should translate network error', () => {
      expect(translateDbError('network timeout')).toBe('Gagal terhubung ke server. Periksa koneksi internet Anda.')
      expect(translateDbError('fetch failed')).toBe('Gagal terhubung ke server. Periksa koneksi internet Anda.')
    })

    it('should return a generic error message for unknown errors', () => {
      expect(translateDbError('unknown error')).toBe('Terjadi kesalahan. Silakan coba lagi.')
    })

    it('should return a generic error message for empty strings', () => {
      expect(translateDbError('')).toBe('Terjadi kesalahan. Silakan coba lagi.')
    })

    it('should handle undefined or null gracefully by fallback to generic message if typed poorly', () => {
      // @ts-expect-error testing invalid input
      expect(translateDbError(undefined)).toBe('Terjadi kesalahan. Silakan coba lagi.')
      // @ts-expect-error testing invalid input
      expect(translateDbError(null)).toBe('Terjadi kesalahan. Silakan coba lagi.')
    })
  })
})
