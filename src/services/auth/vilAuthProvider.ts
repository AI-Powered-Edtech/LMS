...
function mapError(
  error: unknown,
  fallback = "Terjadi kesalahan autentikasi.",
): AuthError {
...
function buildSession(payload: VilAuthResponse): AuthSession {
  ...
  return {
    ...
    user: {
      ...
      email_confirmed_at:
        payload.user.email_confirmed_at || new Date().toISOString(),
      app_metadata: payload.user.role ? { role: payload.user.role } : undefined,
      user_metadata: payload.user.tenant_id
...
export function createVilAuthProvider(baseUrl = ""): AuthProvider {
  return {
    ...
    switchTenant: async ({ tenantId }) => {
      ...
      if (!session?.access_token || !session.refresh_token) {
        return {
          data: { session: null },
          error: {
            message: "Sesi tidak tersedia untuk mengganti ruang kerja.",
            status: 401,
          },
        };
      }
    ...
    signUp: async ({ email, password, options }) => {
      const firstName =
        typeof options?.data?.first_name === "string"
          ? options.data.first_name.trim()
          : "";
      const lastName =
        typeof options?.data?.last_name === "string"
          ? options.data.last_name.trim()
          : "";
      ...
      const { data, error } = await requestJson<VilAuthResponse>(
        ...
        "/api/v1/auth/register",
        {
          ...
          body: JSON.stringify({
            ...
            ...(role ? { role } : {}),
            ...(tenantInviteCode
              ? { tenant_invite_code: tenantInviteCode }
              : {}),
    ...
    verifyOtp: async ({ token_hash, type }) => {
      ...
      return {
        error: notImplemented("Tipe OTP ini belum didukung di backend VIL."),
      };
    },
    ...
    resend: async ({ type, email }) => {
      if (type !== "signup") {
        return {
          error: notImplemented("Pengiriman ulang OTP ini belum didukung."),
        };
      }
    ...
    updateUser: async (attributes) => {
      const password =
        typeof attributes.password === "string" ? attributes.password : null;
      if (!password) {
        return {
