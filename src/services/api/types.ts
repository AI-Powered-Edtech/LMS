...
export interface ApiQueryBuilder<T = unknown> extends PromiseLike<
  ApiQueryResult<T>
> {
  select(
    columns: string,
  ...
  or(expression: string): ApiQueryBuilder<T>;
  not(column: string, comparator: string, value: unknown): ApiQueryBuilder<T>;
  is(column: string, value: unknown): ApiQueryBuilder<T>;
  contains(column: string, value: unknown): ApiQueryBuilder<T>;
  match(criteria: Record<string, unknown>): ApiQueryBuilder<T>;
  ...
  then<TResult1 = ApiQueryResult<T>, TResult2 = never>(
    onfulfilled?:
      | ((value: ApiQueryResult<T>) => TResult1 | PromiseLike<TResult1>)
      | null,
