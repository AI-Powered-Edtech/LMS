...
class VilQueryBuilder<T = unknown> implements ApiQueryBuilder<T> {
  ...
  contains(_column: string, _value: unknown): this {
    return this.fail(
      "Filter CONTAINS generik belum didukung oleh VIL query builder",
    );
  }
...
const auth: ApiAuthClient = {
  async signInWithPassword(credentials) {
    const { data, error } = await requestJson<JsonRecord>(
      "/api/v1/auth/login",
      {
        method: "POST",
  ...
  from() {
    return {
      async upload() {
        return {
          data: null,
          error: notImplementedError("VIL storage belum diimplementasikan"),
        };
      },
      async remove() {
        return {
