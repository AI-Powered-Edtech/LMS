...
const STORAGE_PRIMARY = (import.meta.env.VITE_STORAGE_PRIMARY ??
  "s3") as string;
...
function mapHttpError(status: number): { message: string; statusCode: string } {
  switch (status) {
    ...
    default:
      return {
        message: `Permintaan gagal (HTTP ${status})`,
...
class VilBucketClient implements StorageBucketClient {
  ...
  private async _uploadLarge(
    ...
    try {
      ...
      const signResp = await fetch(
        `${this.apiUrl}/api/v1/storage/presign-upload`,
        {
          method: "POST",
          headers: {
      ...
      if (!upload_url) {
        return {
          data: null,
          error: {
            message: "URL unggah tidak valid",
            statusCode: "PRESIGN_ERROR",
          },
        };
      }
      ...
      if (!putResp.ok) {
        return {
          data: null,
          error: {
            message: "Unggah file besar gagal",
            statusCode: String(putResp.status),
          },
        };
      }
  ...
  async download(path: string): Promise<StorageDownloadResponse> {
    ...
    try {
      const resp = await fetch(
        `${this.apiUrl}/api/v1/storage/object/${this.bucket}/${path}`,
        {
  ...
  async remove(paths: string[]): Promise<StorageRemoveResponse> {
    ...
    try {
      const resp = await fetch(
        `${this.apiUrl}/api/v1/storage/object/${this.bucket}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
    ...
    try {
      ...
      if (!json.signed_url) {
        return {
          data: null,
          error: {
            message: "URL bertanda tidak valid",
            statusCode: "SIGN_ERROR",
          },
        };
      }
