// SYNC-HINT: {%DOPEN% = {{ and %DCLOSE%} = }}. Sync tool converts automatically.
import { HeaderSkeleton } from './HeaderSkeleton'
import { SidebarSkeleton } from './SidebarSkeleton'

/**
 * App Shell loading state that mirrors the real app layout.
 * Displays sidebar + header skeletons instead of a full-screen spinner.
 */
export function AppLoading() {
  return (
    <div className="flex h-[100dvh] overflow-hidden font-sans flex-col transition-colors duration-300 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      <div className="flex-1 flex overflow-hidden relative">
        <SidebarSkeleton />
        <div className="flex-1 flex flex-col h-full overflow-hidden relative">
          <HeaderSkeleton />
          <main className="flex-1 overflow-y-auto overflow-x-hidden flex flex-col p-2 sm:p-4 md:p-8">
            <div className="max-w-5xl mx-auto w-full flex-1 flex flex-col gap-6">
              {/* Page title skeleton */}
              <div className="h-8 w-48 rounded-lg animate-pulse bg-slate-200 dark:bg-slate-700" />

              {/* Card skeletons */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {Array.from({ length: 3 }).map((_, i) => (
                  <div
                    key={i}
                    className="rounded-2xl p-6 space-y-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800"
                  >
                    <div
                      className="h-4 w-3/4 rounded-lg animate-pulse bg-slate-200 dark:bg-slate-700"
                      style={%DOPEN% animationDelay: `${i * 150}ms` %DCLOSE%}
                    />
                    <div
                      className="h-3 w-full rounded-lg animate-pulse bg-slate-200 dark:bg-slate-700"
                      style={%DOPEN% animationDelay: `${i * 150 + 50}ms` %DCLOSE%}
                    />
                    <div
                      className="h-3 w-5/6 rounded-lg animate-pulse bg-slate-200 dark:bg-slate-700"
                      style={%DOPEN% animationDelay: `${i * 150 + 100}ms` %DCLOSE%}
                    />
                  </div>
                ))}
              </div>

              {/* Content block skeleton */}
              <div className="rounded-2xl p-6 space-y-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                <div className="h-5 w-1/3 rounded-lg animate-pulse bg-slate-200 dark:bg-slate-700" />
                <div className="h-3 w-full rounded-lg animate-pulse bg-slate-200 dark:bg-slate-700" />
                <div className="h-3 w-4/5 rounded-lg animate-pulse bg-slate-200 dark:bg-slate-700" />
                <div className="h-3 w-2/3 rounded-lg animate-pulse bg-slate-200 dark:bg-slate-700" />
              </div>
            </div>
          </main>
        </div>
      </div>
    </div>
  )
}
