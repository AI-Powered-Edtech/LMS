...
function HeaderAvatar({ name, avatarUrl }: { name: string; avatarUrl: string | null | undefined }) {
  const initials = name
    .split(' ')
    .slice(0, 2)
...
export function ParentLayout() {
  ...
  const displayName = profile
    ? `${profile.first_name ?? ''} ${profile.last_name ?? ''}`.trim() || 'Orang Tua'
    : 'Orang Tua'
    ...
    >
      ...
      >
        {NAV_ITEMS.map((item) => {
          ...
          return <BottomNavItem key={item.to} item={item} isActive={isActive} />
        })}
      </nav>
    </div>
  )
