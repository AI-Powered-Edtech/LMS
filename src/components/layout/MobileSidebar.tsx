...
import { X, LogOut, ChevronDown, Plus, Check } from 'lucide-react'
          ...
          >
            ...
            <div className="p-spacing-md flex items-center justify-between border-b border-neutral-200 dark:border-neutral-800">
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-xl font-bold text-neutral-900 dark:text-neutral-50 tracking-tight">EduSync</h1>
              </div>
              <button
                onClick={onClose}
              ...
              {role === 'teacher' && (
                <div className="mb-spacing-lg relative">
                  ...
                  >
                    ...
                    <ChevronDown className={cn('w-4 h-4 transition-transform', isClassroomDropdownOpen && 'rotate-180')} />
                  </button>
                  ...
                  {isClassroomDropdownOpen && (
                    <div className="mt-spacing-xs bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-radius-lg overflow-hidden">
                      {classrooms.map((classroom) => (
                        ...
                        >
                          <span className={cn('text-sm font-medium', activeClassroomId === classroom.id ? 'text-primary-600' : 'text-neutral-700 dark:text-neutral-300')}>
                            {classroom.name}
                          </span>
                          {activeClassroomId === classroom.id && <Check className="w-4 h-4 text-primary-600" />}
                        </button>
                      ))}
                        ...
                        {isAddingClassroom ? (
                          <form onSubmit={handleAddClassroom} className="flex flex-col gap-2">
                            ...
                            <div className="flex gap-2">
                              <button type="submit" className="flex-1 bg-primary-600 text-white text-xs font-bold py-2 rounded-radius-sm">Simpan</button>
                              <button type="button" onClick={() => setIsAddingClassroom(false)} className="flex-1 bg-neutral-200 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-300 text-xs font-bold py-2 rounded-radius-sm">Batal</button>
                            </div>
                          </form>
                        ) : (
                          <button
