        ...
        <div className="space-y-4">
            ...
            {assignments.length === 0 ? (
                <div className="text-center p-6 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50">
                    ...
                    <p className="text-xs text-slate-400 mt-1">Klik tombol di atas untuk menyebarkan kuis ini.</p>
                </div>
            ) : (
                <div className="space-y-3">
                    {assignments.map(assignment => {
                        ...
                        return (
                            <div key={assignment.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 border border-slate-200 rounded-xl bg-white shadow-sm">
                                <div>
                                    <h4 className="font-bold text-slate-800 flex items-center gap-2">
                                        {(assignment as any).classes?.name || 'Kelas Tidak Diketahui'}
                                        <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full border ${statusColors[displayStatus as keyof typeof statusColors]}`}>
                                    ...
                                    <div className="flex items-center gap-3 mt-1.5 text-xs text-slate-500">
                                        <div className="flex items-center gap-1">
                                            ...
                                            {assignment.available_from 
                                                ? format(new Date(assignment.available_from), 'd MMM yyyy HH:mm', { locale: id })
                                                : 'Mulai sekarang'}
                                        </div>
                                        <span>—</span>
                                        <div className="flex items-center gap-1">
