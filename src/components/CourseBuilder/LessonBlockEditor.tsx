...
export function LessonBlockEditor() {
  ...
  // ⚡ Perf: Memoize activeLesson lookup to prevent repeated O(n) array allocations (flatMap) on every render (e.g. typing)
  const activeLessonFromModules = useMemo(() => {
    if (!state.activeLesson?.id) return null;
    for (const m of state.modules) {
      const lesson = m.lessons.find((l) => l.id === state.activeLesson?.id);
