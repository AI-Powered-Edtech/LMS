...
export function LessonBlockEditor() {
  ...
  // Memoize the deep lookup to avoid O(N) intermediate array allocations on every render
  ...
  useEffect(() => {
    if (state.activeLesson?.id !== activeLessonIdRef.current) {
      ...
      setLocalTitle(memoizedActiveLesson?.title ?? "");
    }
  }, [state.activeLesson?.id, memoizedActiveLesson?.title]);
