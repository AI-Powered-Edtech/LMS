...
export function LessonBlockEditor() {
  ...
  // ⚡ Bolt: Memoize deep module lookup to prevent O(N) array allocation on render
  const fullActiveLesson = useMemo(() => {
    if (!state.activeLesson?.id) return null;
  ...
  const activeLessonTitle = fullActiveLesson?.title ?? "";
  ...
  useEffect(() => {
    if (state.activeLesson?.id !== activeLessonIdRef.current) {
      ...
      setLocalTitle(fullActiveLesson?.title ?? "");
    }
  }, [state.activeLesson?.id, fullActiveLesson?.title]);
  ...
  const activeLesson = fullActiveLesson;
