...
export function LessonBlockEditor() {
  ...
  // Memoize lesson lookup to prevent O(N) array allocation (.flatMap) on every render
  ...
  const activeLessonTitle = activeLessonData?.title ?? "";
  ...
  useEffect(() => {
    if (state.activeLesson?.id !== activeLessonIdRef.current) {
      ...
      setLocalTitle(activeLessonData?.title ?? "");
    }
  }, [state.activeLesson?.id, activeLessonData?.title]);
  ...
  const activeLesson = activeLessonData;
