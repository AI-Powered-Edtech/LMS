...
export function LessonBlockEditor() {
  ...
  const activeLessonTitle = activeLessonFull?.title ?? "";
  ...
  useEffect(() => {
    if (state.activeLesson?.id !== activeLessonIdRef.current) {
      ...
      setLocalTitle(activeLessonFull?.title ?? "");
    }
  }, [state.activeLesson?.id, activeLessonFull?.title]);
  ...
  const activeLesson = activeLessonFull;
