...
export const CourseEnrollmentGuard: React.FC<CourseEnrollmentGuardProps> = ({ children }) => {
    ...
    useEffect(() => {
        const verifyEnrollment = async () => {
            ...
            try {
                ...
                setError(err.message || 'Gagal memverifikasi status pendaftaran');
                setIsEnrolled(false);
            } finally {
                setLoading(false);
            }
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-screen p-4 text-center">
                <div className="bg-red-50 text-red-600 p-4 rounded-xl max-w-md">
                    ...
                    <p>{error}</p>
                    <button onClick={() => window.location.reload()} className="mt-4 px-4 py-2 bg-red-600 text-white rounded-lg font-medium">Coba Lagi</button>
                </div>
            </div>
        );
    }
    ...
    if (!isEnrolled) {
        ...
        return <Navigate to="/courses" state={{ from: location, error: 'You are not enrolled in this course.' }} replace />;
    }
