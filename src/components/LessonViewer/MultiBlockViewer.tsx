...
import {
  BLOCK_REGISTRY,
  isValidBlockType,
} from "@/features/lessons/blockRegistry";
...
export function MultiBlockViewer({
  ...
  const markBlockComplete = useCallback(
    (blockId: string) => {
      ...
      setCompletedSet((prev) => {
        ...
        const pct =
          total > 0 ? Math.min(Math.round((done / total) * 100), 100) : 100;
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        let mostVisible: { id: string; index: number; ratio: number } | null =
          null;
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting && entry.intersectionRatio >= 0.3) {
            ...
            const blockIndex = parseInt(
              entry.target.getAttribute("data-block-index") || "0",
              10,
            );
        ...
        const visible = mostVisible as {
          id: string;
          index: number;
          ratio: number;
        } | null;
        if (visible) {
          if (activeBlockRef.current?.id !== visible.id) {
            const blockMeta = blocks.find((b) => b.id === visible.id);
          ...
          activeBlockRef.current = { id: visible.id, index: visible.index };
        }
      },
    ...
    const saveInterval = setInterval(() => {
      if (activeBlockRef.current && !isCompleted) {
        const blockEl = document.getElementById(
          `block-${activeBlockRef.current.id}`,
        );
        if (blockEl) {
              ...
              {mountedBlocks.has(block.id) ? (
                <BlockRenderer
                  ...
                  onVideoTimeUpdate={(seconds) =>
                    onVideoTimeUpdate?.(block.id, seconds)
                  }
                  onCompletionMet={
                    isActiveBlock
                      ? () => markBlockComplete(block.id)
                  ...
                  onProgressUpdate={
                    isActiveBlock
                      ? (pct) => {
                          ...
                          onProgressUpdate(
                            Math.min(Math.round(basePct + blockPct), 100),
                          );
                        }
                      : undefined
                  }
