...
const FlashcardBlock = lazy(() =>
  import("@/features/interactive-blocks/components/FlashcardBlock").then(
    (m) => ({
...
const DragDropBlock = lazy(() =>
  import("@/features/interactive-blocks/components/DragDropBlock").then(
    (m) => ({
...
const TimelineBlock = lazy(() =>
  import("@/features/interactive-blocks/components/TimelineBlock").then(
    (m) => ({
...
const FillBlankBlock = lazy(() =>
  import("@/features/interactive-blocks/components/FillBlankBlock").then(
    (m) => ({
...
export function BlockRenderer({
  ...
  switch (type) {
    ...
    case "scorm": {
      const scormPackageId = block.metadata?.scorm_package_id as
        | string
        | undefined;
      if (!scormPackageId)
        return (
          <div className="px-6 py-4 text-slate-500 text-sm">
            Paket SCORM tidak ditemukan.
          </div>
        );
      return (
        <ScormPlayer
