...
import type {
  QuizAttemptQuestion,
  SubmitAnswer,
} from "@/features/quizzes/types/quizzes.types";
...
export function LessonQuizPlayer({
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      ...
      const attemptQuestions = await quizPlayerService.getAttemptQuestions(
        result.attempt_id,
      );
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm p-8 text-center space-y-6">
          ...
          <div className="mt-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl">
            ...
            <p className="text-sm text-red-600 dark:text-red-400 mb-3">
              Nilai kamu:{" "}
              <strong>{state.score !== null ? `${state.score}%` : "—"}</strong>.
              Nilai minimum: <strong>{passingScore}%</strong>.
            </p>
            <button
              onClick={handleRetry}
