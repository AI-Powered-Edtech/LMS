...
export function VideoViewer({
  ...
  const metaHlsUrl =
    metadata &&
    typeof (metadata as Record<string, unknown>).hls_url === "string"
      ...
      {!videoUrl ? (
        <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-8 text-center min-h-[400px]">
          ...
          <p className="text-slate-500 dark:text-slate-400 max-w-md mb-6">
            Materi video untuk pelajaran ini belum ditambahkan. Jika Anda adalah
            instruktur, silakan masukkan URL video terlebih dahulu.
          </p>
        </div>
      ) : (
        <div className="flex-1 flex flex-col min-w-0">
                  ...
                  >
                    <div className="w-full max-w-4xl p-6 bg-white dark:bg-slate-900 rounded-2xl max-h-full overflow-y-auto">
                      ...
                      <QuizViewer
                        ...
                        instructions={
                          loadedQuizzes[activeEvent.quizId].instructions
                        }
                        questions={
                          loadedQuizzes[activeEvent.quizId].quiz_questions
                        }
                        maxAttempts={
                          loadedQuizzes[activeEvent.quizId].max_attempts
                        }
                        passingScore={
                          loadedQuizzes[activeEvent.quizId].passing_score ?? 0
                        }
                        isCompleted={false}
              ...
              {isStalled && (
                ...
                >
                  ...
                  <p className="text-sm text-slate-300">
                    Menunggu jaringan kembali stabil. Video akan otomatis
                    dilanjutkan.
                  </p>
                </motion.div>
              )}
          ...
          >
            ...
            <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
              Pastikan Anda menonton hingga akhir agar sistem mencatat progres
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            {transcripts.map((transcript, idx) => {
              ...
              const isNext = transcripts[idx + 1]
                ? currentTime < transcripts[idx + 1].time
                : true;
