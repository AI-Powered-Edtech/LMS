...
export function CommentSection({ entityId, entityType, className }: CommentSectionProps) {
    ...
    const loadComments = useCallback(async () => {
        try {
            const data = await discussionService.fetchDiscussions({
    ...
    const CommentItem = ({ comment, isReply = false }: { comment: Discussion, isReply?: boolean }) => {
            ...
            <div className={cn("flex gap-3", isReply && "ml-11 mt-4")}>
                <div className="w-8 h-8 bg-slate-200 rounded-full shrink-0 overflow-hidden">
                    {comment.author?.avatar_url ? (
                        ...
                        <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${comment.author?.full_name || comment.author_id}`} alt="" className="w-full h-full object-cover" />
                    )}
                </div>
                <div className="flex-1">
                    <div className="bg-slate-50 p-3 rounded-2xl rounded-tl-none border border-slate-100 group relative">
                        <div className="flex items-center justify-between mb-1">
                            <span className="text-sm font-bold text-slate-900">{comment.author?.full_name || 'User'}</span>
                        ...
                        {editingComment === comment.id ? (
                            <form onSubmit={(e) => { e.preventDefault(); handleSubmit(undefined, comment.parent_id); }} className="mt-2">
                                <textarea
                                    autoFocus
                                    value={editContent}
                                ...
                                <div className="flex justify-end gap-2 mt-2">
                                    <button type="button" onClick={() => setEditingComment(null)} className="text-xs text-slate-500 hover:text-slate-700">Batal</button>
                                    <button type="submit" className="text-xs px-3 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Simpan</button>
                                </div>
                            </form>
                        ) : (
                            <p className={cn("text-sm text-slate-700 whitespace-pre-wrap", comment.is_deleted && "text-slate-400 italic")}>
                                {comment.content}
                            </p>
                        )}
                        ...
                        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                            {isAdmin && !comment.is_deleted && (
                                <button onClick={() => handleTogglePin(comment.id, !!comment.is_pinned)} className="p-1 text-slate-400 hover:text-blue-600 rounded" title={comment.is_pinned ? "Lepaskan sematan" : "Sematkan"}>
                                    <Pin className={cn("w-4 h-4", comment.is_pinned && "fill-blue-500 text-blue-500")} />
                                </button>
                            )}
                            {!comment.is_deleted && canManage && (
                                <div className="relative">
                                    <button onClick={() => setOpenMenuId(openMenuId === comment.id ? null : comment.id)} className="p-1 text-slate-400 hover:text-slate-600 rounded">
                                        <MoreVertical className="w-4 h-4" />
                                    </button>
                                    {openMenuId === comment.id && (
                                        <div className="absolute right-0 mt-1 w-32 bg-white rounded-lg shadow-lg border border-slate-100 py-1 z-10">
                                            {isAuthor && (
                                                <button onClick={() => { setEditingComment(comment.id); setEditContent(comment.content); setOpenMenuId(null); }} className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex flex-center gap-2">
                                                    <Edit2 className="w-3.5 h-3.5" /> Edit
                                                </button>
                                            )}
                                            <button onClick={() => handleDelete(comment.id)} className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2">
                    ...
                    <AnimatePresence>
                        {replyingTo === comment.id && !isReply && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-3 overflow-hidden">
                                <form onSubmit={(e) => { e.preventDefault(); handleSubmit(undefined, comment.id); }} className="flex gap-3">
                                    <input
                                        type="text"
                                        autoFocus
                                        value={newComment}
    ...
    return (
        <div className={cn("space-y-6", className)}>
            ...
            <div className="space-y-4">
                ...
                {comments.length === 0 && (
                    <p className="text-center text-sm text-slate-500 py-4">Belum ada komentar. Jadilah yang pertama!</p>
                )}
            </div>

            <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="mt-6 flex gap-3 pt-4 border-t border-slate-100">
