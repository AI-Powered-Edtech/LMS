...
export function ProgressSummary({ totalLessons, completedLessons, totalDurationMinutes }: ProgressSummaryProps) {
    ...
    >
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className={`text-sm font-bold ${isComplete ? "text-emerald-600" : "text-blue-600"}`}>
            {percentage}%
          </span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-slate-700">
              {completedLessons}/{totalLessons} <span className="font-normal text-slate-400">selesai</span>
            </p>
          </div>
        </div>
          ...
          <div>
            ...
            <p className="text-sm font-bold text-slate-700">
              {isComplete ? (
                ...
                <>{remainingMinutes > 60 ? `${Math.floor(remainingMinutes / 60)} jam ${remainingMinutes % 60} min` : `~${remainingMinutes} min`}</>
              )}
            </p>
          </div>
        </div>
