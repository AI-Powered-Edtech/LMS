...
const mockGroupData = {
  ...
  description:
    'Buatlah presentasi kolaboratif mengenai dampak Revolusi Industri 4.0. Setiap anggota harus berkontribusi pada slide yang berbeda.',
  ...
  chat: [
    ...
    { id: 2, sender: 'Citra', text: 'Oke, saya bantu cari gambar ilustrasinya.', time: '10:05' },
    {
      id: 3,
      sender: 'Ahmad (Anda)',
      text: 'Mantap, saya fokus ke materi pendahuluan.',
      time: '10:10',
    },
  ],
}
...
export function GroupAssignment() {
  ...
  const handleAddTask = () => {
    ...
    setTasks([
      ...tasks,
      { id: Date.now(), title: newTaskTitle, assignee: 'Unassigned', status: 'pending' },
    ])
    setNewTaskTitle('')
  }
                  ...
                  <div key={i} className="flex items-start gap-3 p-3 bg-slate-50 rounded-xl">
                    ...
                    <div>
                      <p className="text-sm text-slate-700">
                        <span className="font-bold">{log.group}</span> {log.action}
                      </p>
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        Sistem akan membuat salinan template untuk setiap kelompok. Semua anggota
                        dalam kelompok yang sama dapat mengedit dokumen tersebut.
                      </p>
                    </div>
                  </label>
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        Buat folder Google Drive khusus untuk tiap kelompok agar mereka bisa
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-start justify-between gap-6">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              <Link
                to="/assignments"
                className="p-1 -ml-1 hover:bg-slate-100 rounded-lg transition-colors text-slate-500 hover:text-slate-900"
              >
              ...
              <span className="px-3 py-1 bg-amber-100 text-amber-700 text-xs font-bold rounded-full flex items-center gap-1.5">
                <Clock className="w-3.5 h-3.5" /> Tenggat:{' '}
                {new Date(mockGroupData.dueDate).toLocaleDateString('id-ID')}
              </span>
            </div>
              ...
              <div className="p-6 flex flex-col h-full">
                ...
                <div className="flex-1 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50 flex flex-col items-center justify-center p-8 text-center">
                  ...
                  <p className="text-sm text-slate-500 mb-6">
                    Terakhir diedit oleh {mockGroupData.sharedDoc.lastEditedBy} •{' '}
                    {mockGroupData.sharedDoc.lastEditedAt}
                  </p>
              ...
              <div className="p-6 flex flex-col h-full">
                ...
                <div className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                  {tasks.map((task) => (
                    <div
                      key={task.id}
                      className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-xl hover:border-indigo-200 transition-colors group"
                    >
              ...
              <div className="p-6 flex flex-col h-full">
                <div className="mb-6">
                  ...
                  <p className="text-sm text-slate-500 mt-1">
                    Nilai kontribusi anggota kelompok Anda. Penilaian ini bersifat rahasia dan hanya
                    dilihat oleh guru.
                  </p>
                </div>
                        ...
                        <div className="space-y-3">
                          <div>
                            <label className="text-xs font-bold text-slate-700 mb-1 block">
                              Tingkat Kontribusi (1-5)
                            </label>
                            <div className="flex gap-2">
                              {[1, 2, 3, 4, 5].map((score) => (
                                <button
                                  key={score}
                                  className="w-8 h-8 rounded-lg border border-slate-200 bg-white hover:border-indigo-500 hover:text-indigo-600 text-sm font-bold text-slate-600 transition-colors"
                                >
                                  {score}
                                </button>
                              ))}
                          ...
                          <div>
                            ...
                            <textarea
                              rows={2}
                              className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                              placeholder="Bagaimana kinerja rekan Anda?"
                            ></textarea>
                          </div>
                        </div>
                      </div>
