...
const STATUS_BUTTONS: Array<{ key: AttendanceStatus; label: string; cls: string }> = [
  { key: 'hadir', label: 'H', cls: 'bg-emerald-100 text-emerald-800 hover:bg-emerald-200' },
  { key: 'sakit', label: 'S', cls: 'bg-amber-100 text-amber-800 hover:bg-amber-200' },
  { key: 'izin', label: 'I', cls: 'bg-blue-100 text-blue-800 hover:bg-blue-200' },
...
export function RombelAttendance() {
  ...
  const { data: students = [] } = useQuery({
    queryKey: ['rombel_attendance_students', rombelId, members.map((m) => m.student_id).join(',')],
    queryFn: async () => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      void qc.invalidateQueries({ queryKey: ['rombel_attendance_day', rombelId, date] })
    } catch (err) {
      addToast({
        type: 'error',
    ...
    <div className="w-full max-w-5xl mx-auto px-4 md:px-8 pt-8 pb-20 space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Catat kehadiran harian per rombel. H = Hadir, S = Sakit, I = Izin, A = Alpa.
          </p>
        </div>
        <Button
          variant="primary"
      ...
      <Card>
        <div className="flex flex-wrap items-center gap-4 mb-4">
          <select value={rombelId} onChange={(e) => setRombelId(e.target.value)} className="w-60">
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4 text-center text-sm">
              ...
              <div className="rounded-lg bg-amber-50 dark:bg-amber-900/20 p-3">
                ...
                <p className="text-2xl font-bold text-amber-700">{summary.sakit}</p>
              </div>
            ...
            <ul className="divide-y divide-slate-100 dark:divide-slate-800">
              {students.map((stu) => (
                <li key={stu.id} className="flex items-center justify-between py-2.5">
                  <div>
