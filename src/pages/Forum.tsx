...
const ParticipationChart = lazy(() =>
  import("@/features/discussions/components/forum/ParticipationChart").then(
    (module) => ({
...
export function Forum() {
  ...
  const [activeTab, setActiveTab] = useState<"discussions" | "participation">(
    "discussions",
  );
  ...
  const availableClassrooms = useMemo(() => {
    ...
    return allClassrooms.filter(
      (classroom: any) => classroom.course_id === selectedCourseId,
    );
  ...
  const { data: rawDiscussions = [] } = useQuery({
    ...
    queryFn: () =>
      discussionService.fetchForumPosts(
        tenantId!,
        forumPage,
        PAGE_SIZE,
        selectedCourseId,
  ...
  const posts = useMemo(
    () => buildForumPosts(rawDiscussions),
    [rawDiscussions],
  );
  ...
  const createPost = useMutation({
    ...
    onSuccess: (data, vars) => {
      const isAiSuspect =
        ...
        (vars.content.includes("tentu") ||
          vars.content.includes("sebagai model bahasa"));
      if (isAiSuspect) {
        const authorName = vars.isAnon
          ? "Anonim"
      ...
      {activeTab === "participation" && isTeacher && (
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm p-6">
            ...
            >
              <option value="">
                Pilih kursus untuk melihat partisipasi...
              </option>
              {courses.map((course) => (
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  disabled={
                    ...
                    participationData.participants.length === 0
                  }
          ...
          <Suspense
            fallback={
              <div className="h-64 animate-pulse">Loading chart...</div>
            }
          >
