...
export function ReviewQueue() {
  ...
  const pendingQuery = useQuery({
    ...
    queryFn: async (): Promise<{
      ...
      teachers: Record<string, TeacherProfile>;
    }> => {
      ...
      const { data: coursesData, error } = await getDbClient()
        .from("courses")
        .select(
          "id, title, description, subject, level, updated_at, created_by, tenant_id",
        )
  ...
  const requestChangesMutation = useMutation({
    ...
    onError: (err: Error) => {
      addToast({
        type: "error",
        message: "Gagal mengirim catatan",
        description: err.message,
      });
    },
    onSettled: () => setActiveCourseId(null),
      ...
      <header className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Kursus yang diajukan guru untuk ditinjau di tenant ini. Setujui
            untuk mengaktifkan tombol Terbitkan, atau minta perubahan dengan
            catatan.
          </p>
        </div>
        <button
      ...
      {pendingQuery.isLoading ? (
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-white dark:from-emerald-900/20 dark:to-slate-800/80 border border-emerald-100 dark:border-emerald-800/40 rounded-2xl p-8 text-center">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Semua kursus yang diajukan sudah ditinjau. Guru akan muncul di sini
            ketika mengajukan kursus baru.
          </p>
        </div>
      ) : (
              ...
              >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                      ...
                      <span className="font-medium">
                        {teacherLabel(course.created_by)}
                      </span>
                      {" · "}
                ...
                <div className="mt-4 space-y-2">
                  <label className="flex items-center gap-1.5 text-xs font-medium text-slate-600 dark:text-slate-400">
                    ...
                    Catatan untuk guru (opsional saat setuju, wajib saat minta
                    perubahan)
                  </label>
                  <textarea
                    value={draft}
                      ...
                      onClick={() => {
                        if (!draft.trim()) {
                          addToast({
                            type: "error",
                            message:
                              "Isi catatan terlebih dahulu untuk minta perubahan.",
                          });
                          return;
                        }
