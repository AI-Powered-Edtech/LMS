...
export function AdminAnalyticsDashboard() {
  ...
  if (
    ...
    (analytics.overview.totalCourses === 0 &&
      analytics.overview.totalEnrolled === 0)
  ) {
    return (
  ...
  const { overview, activityMetrics, courseEngagement, activityTimeline } =
    analytics;
        ...
        <div className="flex items-center gap-3">
          {overview.lastRefreshedAt && (
            <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
              ...
              <span>
                Diperbarui: {formatDateTime(overview.lastRefreshedAt)}
              </span>
            </div>
          )}
          <button
