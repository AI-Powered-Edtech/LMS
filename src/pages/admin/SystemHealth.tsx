  ...
  async function loadData() {
    ...
    try {
      ...
      summary.push({
        ...
        unit: "ms",
        status:
          dbLatency < 200 ? "good" : dbLatency < 500 ? "warning" : "critical",
      });

      // Aggregate from stored metrics
      if (recentMetrics) {
        const byName = recentMetrics.reduce<Record<string, number[]>>(
          (acc, m) => {
        ...
        <>
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <p
                  className={`text-lg font-bold ${statusColor(overallStatus)}`}
                >
                  {overallStatus === "healthy"
                    ? "Semua Sistem Normal"
                    : overallStatus === "degraded"
