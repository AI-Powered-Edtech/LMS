  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-2">Kelola preferensi akun, notifikasi, dan tampilan aplikasi.</p>
      </div>
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-100 dark:border-slate-700">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Perbarui informasi dasar akun Anda.</p>
            </div>
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-20 h-20 bg-slate-100 dark:bg-slate-700 rounded-full flex items-center justify-center overflow-hidden border-4 border-white dark:border-slate-800 shadow-md">
                  <img src={profile?.avatar_url ?? `https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.id ?? 'default'}`} alt="Avatar" className="w-full h-full object-cover" />
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input type="text" defaultValue={displayName} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-bold text-slate-700 dark:text-slate-300">Email</label>
                  <input type="email" defaultValue={displayEmail} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-bold text-slate-700 dark:text-slate-300">Peran Akun</label>
                  <input type="text" value={role === 'teacher' ? 'Guru' : 'Siswa'} disabled className="w-full px-4 py-2.5 bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl text-slate-500 dark:text-slate-400 cursor-not-allowed" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-bold text-slate-700 dark:text-slate-300">Nomor Induk</label>
                  <input type="text" value={(profile as any)?.employee_id ?? (profile as any)?.student_id ?? ''} disabled placeholder={role === 'teacher' ? 'NIP' : 'NIS'} className="w-full px-4 py-2.5 bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl text-slate-500 dark:text-slate-400 cursor-not-allowed" />
                </div>
              </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
            ...
            <div className="p-6">
              <div className="grid grid-cols-3 gap-4">
                ...
                >
                  <Sun className={cn("w-8 h-8", theme === 'light' ? "text-blue-600" : "text-slate-400 dark:text-slate-500")} />
                  <span className={cn("text-sm font-bold", theme === 'light' ? "text-blue-700 dark:text-blue-400" : "text-slate-600 dark:text-slate-400")}>Terang</span>
                </button>
                <button
                ...
                >
                  <Moon className={cn("w-8 h-8", theme === 'dark' ? "text-blue-600" : "text-slate-400 dark:text-slate-500")} />
                  <span className={cn("text-sm font-bold", theme === 'dark' ? "text-blue-700 dark:text-blue-400" : "text-slate-600 dark:text-slate-400")}>Gelap</span>
                </button>
                <button
                ...
                >
                  <Monitor className={cn("w-8 h-8", theme === 'system' ? "text-blue-600" : "text-slate-400 dark:text-slate-500")} />
                  <span className={cn("text-sm font-bold", theme === 'system' ? "text-blue-700 dark:text-blue-400" : "text-slate-600 dark:text-slate-400")}>Sistem</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl border border-red-200 dark:border-red-900/50 shadow-sm overflow-hidden">
            <div className="p-6">
              <h2 className="text-lg font-bold text-red-600 dark:text-red-400 mb-2">Zona Berbahaya</h2>
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">Tindakan di bawah ini tidak dapat dibatalkan.</p>
