    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6 pb-24 md:pb-8">
        ...
        <div className="px-6 md:px-8 pb-8">
          <div className="flex flex-col md:flex-row gap-6 items-start md:items-end -mt-12 md:-mt-16 mb-6">
            <div className="relative">
              <div className="w-24 h-24 md:w-32 md:h-32 rounded-full bg-white dark:bg-slate-800 p-1.5 shadow-lg">
                <OptimizedImage
                  src={profile.avatar}
                  alt={profile.name}
                  className="w-full h-full rounded-full bg-slate-100 dark:bg-slate-700 object-cover"
                />
              </div>
              {isTeacherProfile && teacherProfile.isOnline && (
                <div
                  className="absolute bottom-2 right-2 w-5 h-5 bg-green-500 border-4 border-white dark:border-slate-800 rounded-full"
                  title="Daring"
                ></div>
              )}
            </div>
            ...
            <div className="flex-1">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    className="p-2.5 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600 rounded-xl transition-colors shadow-sm"
                    title="Kirim Pesan"
                  >
                    <MessageSquare className="w-5 h-5" />
                  </button>
                  {isTeacherProfile && (
                    <button className="px-4 py-2.5 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 font-bold text-sm rounded-xl transition-colors shadow-sm hidden sm:flex items-center gap-2">
          ...
          <div className="max-w-2xl">
            {isTeacherProfile ? (
              ...
              <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-lg text-sm text-slate-700 dark:text-slate-300">
          ...
          {isTeacherProfile ? (
            <>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
                <h2 className="font-bold text-slate-900 dark:text-slate-100">Informasi Akademik</h2>
                <div className="space-y-3 text-sm">
                  <div>
                    <div className="text-slate-500 dark:text-slate-400 mb-1">Mata Pelajaran</div>
                    <div className="flex flex-wrap gap-2">
                      {teacherProfile.subjects.map((sub) => (
                        <span
                          key={sub}
                          className="px-2.5 py-1 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 rounded-lg font-medium"
                        >
                          {sub}
                        </span>
                      ))}
                  ...
                  <div className="pt-3 border-t border-slate-100 dark:border-slate-700">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-slate-500 dark:text-slate-400">
                        ({teacherProfile.totalReviews} ulasan)
                      </span>
                    </div>
                  </div>
                </div>
              ...
              {/* Official Links */}
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
                <h2 className="font-bold text-slate-900 dark:text-slate-100">Tautan Resmi</h2>
                <div className="space-y-2">
                  {teacherProfile.links.map((link, i) => (
                    <a
                      key={i}
                      href={link.url}
                      className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 hover:bg-blue-50 dark:hover:bg-blue-900/30 hover:text-blue-700 dark:hover:text-blue-300 rounded-xl transition-colors text-sm font-medium text-slate-700 dark:text-slate-300 group"
                    >
                      {link.title}
            ...
            <>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
                ...
                {currentUserRole === 'student' && studentProfile.mutualClasses > 0 && (
                  <div className="p-3 bg-blue-50 dark:bg-blue-900/30 border border-blue-100 dark:border-blue-800 rounded-xl flex items-start gap-3 mt-4">
              ...
              {/* Interests */}
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
                <h2 className="font-bold text-slate-900 dark:text-slate-100">Minat & Keahlian</h2>
                <div className="flex flex-wrap gap-2">
                  {studentProfile.interests.map((tag) => (
                    <span
                      key={tag}
                      className="px-3 py-1.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg text-xs font-bold hover:bg-slate-200 dark:hover:bg-slate-600 cursor-pointer transition-colors"
                    >
                      {tag}
                    </span>
                  ))}
            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
              ...
              <div className="flex gap-4 overflow-x-auto pb-2 hide-scrollbar">
                {studentProfile.badges.map((badge) => (
                  <div
                    key={badge.id}
                    className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-700 rounded-2xl min-w-[200px]"
                  >
                    <div
                      className={cn(
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
            ...
            <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 dark:before:via-slate-700 before:to-transparent">
              {profile.recentActivity.map((activity) => (
                ...
                >
                  <div className="flex items-center justify-center w-10 h-10 rounded-full border border-white dark:border-slate-700 bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10">
                    <MessageSquare className="w-4 h-4" />
                  </div>
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-2xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm hover:shadow-md transition-shadow">
      ...
      {showReportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                {[
                  ...
                  <label
                    key={reason}
                    className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer transition-colors"
                  >
                    <input
                      type="radio"
                      name="report_reason"
