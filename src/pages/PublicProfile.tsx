...
import { 
  UserPlus, MessageSquare, ShieldCheck, Award, Star, 
  BookOpen, Clock, Flag, CheckCircle, Users, ExternalLink,
  ChevronLeft, MoreVertical, Flame
} from "lucide-react";
...
export function PublicProfile() {
  ...
  const studentProfile = {
    ...
    badges: [
      { id: 1, name: "Top Contributor", icon: Flame, color: "text-orange-500", bg: "bg-orange-100" },
    ...
    recentActivity: [
      { id: 1, action: "Menjawab pertanyaan", topic: "Cara menghitung limit fungsi trigonometri?", time: "2 jam yang lalu" },
      { id: 2, action: "Mendapatkan lencana", topic: "Problem Solver", time: "1 hari yang lalu" },
      { id: 3, action: "Bertanya", topic: "Rekomendasi buku Fisika Dasar untuk pemula?", time: "3 hari yang lalu" },
    ]
  };
  ...
  const teacherProfile = {
    ...
    recentActivity: [
      { id: 1, action: "Mengunggah materi baru", topic: "Modul 4: Neural Networks", time: "5 jam yang lalu" },
      { id: 2, action: "Menjawab pertanyaan", topic: "Penjelasan tentang algoritma Dijkstra", time: "1 hari yang lalu" },
    ]
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6 pb-24 md:pb-8">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden relative">
        ...
        <div className={cn(
          "h-32 md:h-48 w-full",
          isTeacherProfile ? "bg-gradient-to-r from-emerald-500 to-teal-600" : "bg-gradient-to-r from-blue-500 to-indigo-600"
        )}></div>
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start md:items-end -mt-12 md:-mt-16 mb-6">
            <div className="relative">
              <div className="w-24 h-24 md:w-32 md:h-32 rounded-full bg-white p-1.5 shadow-lg">
                <img src={profile.avatar} alt={profile.name} className="w-full h-full rounded-full bg-slate-100 object-cover" />
              </div>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <h1 className="text-2xl md:text-3xl font-bold text-slate-900 flex items-center gap-2">
                    {profile.name}
                    {isTeacherProfile && <span title="Verified Teacher"><ShieldCheck className="w-6 h-6 text-emerald-500" /></span>}
                  </h1>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button className="p-2.5 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 rounded-xl transition-colors shadow-sm" title="Kirim Pesan">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {teacherProfile.subjects.map(sub => (
                        <span key={sub} className="px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-lg font-medium">{sub}</span>
                      ))}
                    </div>
                  </div>
                  <div className="pt-3 border-t border-slate-100">
                    <div className="text-slate-500 mb-1 flex items-center gap-1.5"><Clock className="w-4 h-4" /> Office Hours</div>
                    <div className="font-medium text-slate-900">{teacherProfile.officeHours}</div>
                  </div>
                  <div className="pt-3 border-t border-slate-100">
                    <div className="text-slate-500 mb-1 flex items-center gap-1.5"><Star className="w-4 h-4 text-yellow-500" /> Rating Komunitas</div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-slate-900 text-lg">{teacherProfile.rating}</span>
              ...
              <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
                ...
                <div className="space-y-2">
                  {teacherProfile.links.map((link, i) => (
                    <a key={i} href={link.url} className="flex items-center justify-between p-3 bg-slate-50 hover:bg-blue-50 hover:text-blue-700 rounded-xl transition-colors text-sm font-medium text-slate-700 group">
                      {link.title}
              ...
              <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 bg-slate-50 rounded-xl text-center">
                    <div className="text-2xl font-black text-slate-900">{studentProfile.stats.bestAnswers}</div>
                    <div className="text-[10px] font-bold text-slate-500 uppercase">Jawaban Terbaik</div>
                  </div>
                  <div className="p-3 bg-slate-50 rounded-xl text-center">
                    <div className="text-2xl font-black text-slate-900">{studentProfile.stats.reputation}</div>
                ...
                {currentUserRole === 'student' && studentProfile.mutualClasses > 0 && (
                  <div className="p-3 bg-blue-50 border border-blue-100 rounded-xl flex items-start gap-3 mt-4">
                    ...
                    <p className="text-xs text-blue-800 font-medium">
                      Berada di <strong className="font-bold">{studentProfile.mutualClasses} kelas</strong> yang sama dengan Anda.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-6">
              ...
              <div className="flex gap-4 overflow-x-auto pb-2 hide-scrollbar">
                {studentProfile.badges.map(badge => (
                  <div key={badge.id} className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-100 rounded-2xl min-w-[200px]">
                    <div className={cn("w-10 h-10 rounded-full flex items-center justify-center shrink-0", badge.bg, badge.color)}>
              ...
              <div className="relative overflow-hidden rounded-2xl bg-slate-900 text-white group cursor-pointer">
                <img src={studentProfile.highlightedCertificate.image} alt="Certificate" className="absolute inset-0 w-full h-full object-cover opacity-40 group-hover:opacity-50 transition-opacity duration-500" />
                ...
                <div className="relative p-6 flex flex-col h-48 justify-end">
                  ...
                  <h3 className="font-bold text-lg leading-tight mb-1">{studentProfile.highlightedCertificate.title}</h3>
                  <p className="text-sm text-slate-300">{studentProfile.highlightedCertificate.issuer} • {studentProfile.highlightedCertificate.date}</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              <p className="text-sm text-slate-600">
                Mengapa Anda melaporkan profil ini? Laporan Anda akan ditinjau oleh tim admin secara anonim.
              </p>
              <div className="space-y-2">
                {['Foto profil tidak pantas', 'Bio mengandung kata kasar/SARA', 'Spam atau penipuan', 'Lainnya'].map(reason => (
                  <label key={reason} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 hover:bg-slate-50 cursor-pointer transition-colors">
                    <input type="radio" name="report_reason" className="w-4 h-4 text-blue-600 border-slate-300 focus:ring-blue-500" />
