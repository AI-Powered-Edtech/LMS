...
function mergeStudentAssignment(
  ...
  const attempts = [...bundle.attempts]
    ...
    .map((attempt) => ({
      ...
      fileName: attempt.file_url
        ? attempt.file_url.split("/").pop() || "file"
        : null,
...
export function Assignments() {
  ...
  usePageTitle(t("assignmentsPage.pageTitle"));
  ...
  const [activeBundle, setActiveBundle] =
    useState<AssignmentSubmissionBundle | null>(null);
  const [activeGradingQueue, setActiveGradingQueue] =
    useState<AssignmentGradingQueue | null>(null);
  const [activeAnalytics, setActiveAnalytics] =
    useState<AssignmentAnalytics | null>(null);
      ...
      try {
        if (role === "student") {
          ...
          const [queue, analytics] = await Promise.all([
            assignmentService.getAssignmentGradingQueue(
              activeAssignment.id,
              tenantId,
            ),
        ...
        if (!cancelled) {
          addToast({
            type: "error",
            message: t("assignmentsPage.toasts.loadDetailError"),
          });
        }
      } finally {
  ...
  }, [activeAssignment, addToast, role, tenantId, user, t]);
  ...
  const activeSelectedFile = activeAssignment
    ? (selectedFiles[activeAssignment.id] ?? null)
    : null;
  const activeDraftText = activeAssignment
    ? (draftTexts[activeAssignment.id] ?? "")
    : "";
  const activeDraftLink = activeAssignment
    ? (draftLinks[activeAssignment.id] ?? "")
    : "";
  ...
  const handleTurnIn = async (assignmentId: string) => {
    ...
    try {
      ...
      await assignmentService.submitAssignmentAttempt(
        assignmentId,
        user.id,
        tenantId,
        {
      ...
      addToast({
        type: "success",
        message: t("assignmentsPage.toasts.submitSuccess"),
      });
    } catch (error) {
      addToast({
        type: "error",
        message:
          error instanceof Error
            ? error.message
            : t("assignmentsPage.toasts.submitError"),
      });
    } finally {
  ...
  const handleUnsubmit = async (assignmentId: string) => {
    ...
    try {
      await assignmentService.unsubmitAssignment(
        assignmentId,
        user.id,
        tenantId,
      );
      ...
      addToast({
        type: "success",
        message: t("assignmentsPage.toasts.unsubmitSuccess"),
      });
    } catch (error) {
      addToast({
        type: "error",
        message:
          error instanceof Error
            ? error.message
            : t("assignmentsPage.toasts.unsubmitError"),
      });
    }
  };
  ...
  const handleCreateAssignment = async (data: NewAssignmentData) => {
    ...
    try {
      ...
      addToast({
        type: "success",
        message: t("assignmentsPage.toasts.createSuccess"),
      });
    } catch (error) {
      addToast({
        type: "error",
        message:
          error instanceof Error
            ? error.message
            : t("assignmentsPage.toasts.createError"),
      });
    }
  };
  ...
  const handleSendReminder = async () => {
    ...
    try {
      const result = await assignmentService.sendAssignmentReminders(
        activeAssignment.id,
        tenantId,
      );
      ...
      addToast({
        type: "success",
        message: t("assignmentsPage.toasts.reminderSuccess").replace(
          "{count}",
          String(result.recipient_count),
        ),
      });
    } catch (error) {
      addToast({
        ...
        message:
          error instanceof Error
            ? error.message
            : t("assignmentsPage.toasts.reminderError"),
      });
    } finally {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2">
            {role === "teacher"
              ? t("assignmentsPage.teacherSubtitle")
              : t("assignmentsPage.studentSubtitle")}
          </p>
        </div>
        {role === "teacher" && (
          <button
          ...
          {activeAssignment ? (
            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col h-[calc(100vh-12rem)] min-h-[600px]">
              <div className="p-6 sm:p-8 border-b border-slate-100 dark:border-slate-800 shrink-0">
                <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                  {getStatusBadge(
                    (activeStudentAssignment ?? activeAssignment).status,
                  )}
                  <div className="flex flex-wrap items-center gap-4 text-sm font-bold text-slate-600 dark:text-slate-400">
                    <span className="flex items-center gap-1.5">
                      <Clock className="w-4 h-4 text-slate-400" />
                      {t("assignmentsPage.dueDate")}:{" "}
                      {new Date(activeAssignment.dueDate).toLocaleString(
                        i18n.language === "en" ? "en-US" : "id-ID",
                      )}
                    </span>
                    {(activeStudentAssignment ?? activeAssignment).grade !==
                      null && (
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="space-y-4">
                    ...
                    {activeAssignment.description ? (
                      ...
                      <p className="text-slate-400 dark:text-slate-500 italic text-sm">
                        {t("assignmentsPage.noInstructions")}
                      </p>
                    )}
                  </div>
                  ...
                  {role !== "student" && (
                    ...
                    >
                      ...
                      {t("assignmentsPage.sendReminder").replace(
                        "{count}",
                        String(reminderTargetCount),
                      )}
                    </button>
                  )}
                </div>
                ...
                {(activeAssignment.attachments || []).length > 0 && (
                  <div className="mt-6">
                    <h4 className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">
                      {t("assignmentsPage.teacherAttachments")}
                    </h4>
                  ...
                  <div className="space-y-8">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-4">
                          <div className="text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                            {t("assignmentsPage.analytics.submissionRate")}
                          </div>
                        ...
                        <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-4">
                          <div className="text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                            {t("assignmentsPage.analytics.graded")}
                          </div>
                          <div className="mt-2 text-2xl font-black text-slate-900 dark:text-white">
                        ...
                        <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-4">
                          <div className="text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                            {t("assignmentsPage.analytics.late")}
                          </div>
                          <div className="mt-2 text-2xl font-black text-slate-900 dark:text-white">
                        ...
                        <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-4">
                          <div className="text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                            {t("assignmentsPage.analytics.averageScore")}
                          </div>
                          <div className="mt-2 text-2xl font-black text-slate-900 dark:text-white">
                      ...
                      activeAnalytics.score_distribution.length > 0 && (
                        <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-6">
                          <h3 className="font-bold text-slate-900 dark:text-white mb-4">
                            {t("assignmentsPage.analytics.scoreDistribution")}
                          </h3>
                          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                            {activeAnalytics.score_distribution.map(
                              (bucket) => (
                                <div
                                  key={bucket.bucket}
                    ...
                    {isLoadingContext && (
                      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 flex items-center gap-3 text-sm text-slate-500 dark:text-slate-400">
                        ...
                        {t("assignmentsPage.loadingLatest")}
                      </div>
                    )}
                            ...
                            <div className="rounded-2xl border border-amber-200 dark:border-amber-900/40 bg-amber-50 dark:bg-amber-950/30 p-4 flex gap-3">
                              ...
                              <div>
                                <p className="text-sm font-bold text-amber-800 dark:text-amber-300">
                                  {t(
                                    "assignmentsPage.reminder.notSubmitted",
                                  ).replace(
                                    "{count}",
                                    String(reminderTargetCount),
                                  )}
                                ...
                                <p className="text-sm text-amber-700 dark:text-amber-400">
                                  {t("assignmentsPage.reminder.description")}
                                </p>
                              </div>
                            </div>
                          )}
            ...
            <div className="h-[calc(100vh-12rem)] min-h-[600px] bg-slate-50 dark:bg-slate-900 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-3xl flex flex-col items-center justify-center text-center p-8">
              ...
              <h3 className="text-xl font-bold text-slate-700 dark:text-slate-300 mb-2">
                {t("assignmentsPage.empty.title")}
              </h3>
              <p className="text-slate-500 dark:text-slate-400 max-w-sm">
                {t("assignmentsPage.empty.description")}
              </p>
            </div>
          )}
        </div>
