...
} from "@/features/dossier/api/dossierService";
...
export function StudentDossier() {
  ...
  const { data: dossier, isLoading } = useQuery({
    ...
    queryFn: () =>
      profileId ? dossierService.get(profileId) : Promise.resolve(null),
    enabled: !!profileId,
  });
  ...
  function setField<K extends keyof DossierType>(
    key: K,
    value: DossierType[K],
  ) {
  ...
  if (!profileId) {
    return (
      <div className="p-8 text-sm text-slate-500">
        Profile ID tidak ditemukan di URL.
      </div>
    );
  }
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <select
            value={form.gender ?? ""}
            onChange={(e) =>
              setField("gender", (e.target.value || null) as "L" | "P" | null)
            }
          >
            <option value="">—</option>
