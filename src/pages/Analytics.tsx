    ...
    <div className="max-w-7xl mx-auto space-y-8 p-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={
              refreshMutation.isPending || isLoading || !selectedCourseId
            }
          ...
          <button
            onClick={() => {
              if (selectedCourseId) {
                void navigate(
                  `/app/teacher/course-analytics?courseId=${selectedCourseId}`,
                );
              }
            }}
      ...
      {data && (
        <div className="flex items-center gap-2 text-sm text-slate-500 bg-slate-50 px-4 py-2 rounded-lg">
          ...
          <span>
            Terakhir diperbarui:{" "}
            {formatLastUpdated(data.overview.last_calculated_at)}
          </span>
        </div>
      )}
      ...
      {isLoading && !refreshMutation.isPending ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-slate-500">
            Memuat data agregasi dari warehouse...
          </p>
        </div>
      ) : errorMessage ? (
        ...
        <div className="p-10 text-center bg-slate-50 rounded-2xl border border-slate-200">
          ...
          <p className="text-slate-500">
            Pilih kursus untuk melihat data analitik.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-bold text-slate-900 mt-1">
                  {data.overview.total_enrolled > 0
                    ? Math.round(data.overview.avg_progress)
                    : 0}
                  %
                </h3>
                <p className="text-xs text-slate-400 mt-1">
                  Tingkat penyelesaian{" "}
                  {Math.round(data.overview.lesson_completion_rate)}%
                </p>
              </div>
