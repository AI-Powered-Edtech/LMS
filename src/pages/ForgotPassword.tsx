...
export function ForgotPassword() {
  ...
  const onSubmit = async (data: ForgotPasswordFormData) => {
    ...
    const { allowed, retryAfterMs } =
      passwordResetRateLimiter.check("password-reset");
    if (!allowed) {
      const seconds = Math.ceil(retryAfterMs / 1000);
      setError(
        t("auth.pages.tooManyAttempts").replace("{seconds}", String(seconds)),
      );
      return;
    }
    ...
    try {
      const { error: resetError } =
        await getAuthProvider().resetPasswordForEmail(data.email, {
      ...
      setError(
        err instanceof Error ? err.message : t("auth.pages.genericRetryError"),
      );
    }
  };
        ...
        {submitted ? (
          <div className="bg-emerald-50 dark:bg-emerald-900/20 p-6 rounded-xl border border-emerald-200 dark:border-emerald-800/50 text-center">
            ...
            <p className="text-emerald-700 dark:text-emerald-400 font-bold mb-2">
              {t("auth.pages.resetEmailSent")}{" "}
              <strong>{getValues("email")}</strong>.{" "}
              {t("auth.pages.checkInboxSpam")}
            </p>
