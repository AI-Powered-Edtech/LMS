...
const LOADING_STEPS = [
  { labelKey: "creator.progress.extracting", icon: "📄" },
  { labelKey: "creator.progress.analyzing", icon: "🔍" },
  { labelKey: "creator.progress.generating", icon: "🤖" },
  { labelKey: "creator.progress.saving", icon: "💾" },
] as const;
        ...
        return (
          ...
          >
            ...
            <span className="flex-1">
              {t("creator.progress.stepPrefix").replace(
                "{step}",
                String(i + 1),
              )}
              : {t(step.labelKey)}
...
export function Creator(): React.JSX.Element {
  ...
  usePageTitle(t("creator.pageTitle"));
  ...
  const [resultGeneratedAt, setResultGeneratedAt] = useState<string | null>(
    null,
  );
  ...
  const validateAndSetFile = useCallback(
    (f: File) => {
      if (f.size > 10 * 1024 * 1024) {
        addToast({ type: "error", message: t("creator.toasts.fileTooLarge") });
        return;
      }
      if (
        ...
        addToast({
          type: "error",
          message: t("creator.toasts.unsupportedFormat"),
        });
        return;
      }
      setFile(f);
  ...
  const handleGenerate = (): void => {
    ...
    if (curriculumRef.trim())
      formData.append("curriculumRef", curriculumRef.trim());
  ...
  const handleAddToCourse = (): void => {
    ...
    if (selectedQs.length === 0) {
      addToast({
        type: "error",
        message: t("creator.toasts.selectQuestionToAdd"),
      });
      return;
    }
    ...
    useCreatorBridgeStore.getState().setPendingQuiz({
      title:
        file?.name.replace(/\.[^/.]+$/, "") || t("creator.defaults.aiContent"),
      type: resultType,
    ...
    void navigate(
      getPath("/app/teacher/course-builder", "/app/admin/course-builder"),
    );
  };
  ...
  const handleSaveToCalendar = (): void => {
    ...
    addEvent({
      title: `${resultType === "quiz" ? t("creator.assignmentTypes.quizShort") : resultType === "reading" ? t("creator.assignmentTypes.reading") : t("creator.assignmentTypes.writing")}: ${file?.name.replace(/\.[^/.]+$/, "") || t("creator.defaults.newMaterial")}`,
      date: dueDate,
      time: "23:59",
      type: "quiz",
      location: "Online",
    ...
    void sendNotification.mutate({
      userId: user!.id,
      type: "assignment",
      title: t("creator.notifications.assignmentScheduledTitle"),
      message: t("creator.notifications.assignmentScheduledMessage"),
    });

    addToast({ type: "success", message: t("creator.toasts.addedToCalendar") });
  ...
  const handleExportCSV = (): void => {
    ...
    if (selectedQs.length === 0) {
      addToast({
        type: "error",
        message: t("creator.toasts.selectQuestionToExport"),
      });
      return;
    }
    exportQuestionsToCSV(
      ...
      resultType,
      file?.name.replace(/\.[^/.]+$/, "") ||
        t("creator.defaults.aiQuestionsFilename"),
    );
    addToast({
      type: "success",
      message: t("creator.toasts.exportSuccess").replace(
        "{count}",
        String(selectedQs.length),
      ),
    });
  };
  ...
  const handleSaveToBank = async (): Promise<void> => {
    ...
    if (selectedQs.length === 0) {
      addToast({
        type: "error",
        message: t("creator.toasts.selectQuestionToSave"),
      });
      return;
    }
    ...
    try {
      ...
      if (result.saved > 0) {
        addToast({
          type: "success",
          message: t("creator.toasts.saveToBankSuccess")
            .replace("{saved}", String(result.saved))
            .replace(
              "{failedPart}",
              result.failed > 0
                ? ` (${t("creator.toasts.failedCount").replace("{count}", String(result.failed))})`
                : "",
            ),
        });
      } else {
        addToast({
          type: "error",
          message: t("creator.toasts.saveToBankError"),
        });
      }
    } catch {
      addToast({ type: "error", message: t("creator.toasts.saveToBankError") });
    } finally {
  ...
  const ASSIGNMENT_TYPES: { id: AssignmentType; label: string }[] = [
    { id: "quiz", label: t("creator.assignmentTypes.quiz") },
    { id: "reading", label: t("creator.assignmentTypes.reading") },
    { id: "writing", label: t("creator.assignmentTypes.writing") },
  ];
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl text-sm text-blue-700 dark:text-blue-300">
            <strong>{t("creator.infoTitle")}:</strong>{" "}
            {t("creator.infoDescription")}
          </div>
        </div>
        {/* History Button */}
        <button
                ...
                {file ? (
                  ...
                  <>
                    ...
                    <p className="font-bold text-slate-700 dark:text-slate-200">
                      {t("creator.upload.dropTitle")}
                    </p>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
                      {t("creator.upload.dropHint")}
                    </p>
                    <input
                      type="file"
                      className="hidden"
              ...
              <div className="space-y-7">
                ...
                <div>
                  <label className="font-bold text-slate-700 dark:text-slate-200 block mb-3">
                    {t("creator.config.assignmentType")}
                  </label>
                  ...
                  <div className="flex justify-between mb-2">
                    <label className="font-bold text-slate-700 dark:text-slate-200">
                      {assignmentType === "writing"
                        ? t("creator.config.topicCount")
                        : t("creator.config.questionCount")}
                    </label>
                ...
                <div>
                  <label className="font-bold text-slate-700 dark:text-slate-200 block mb-1">
                    {t("creator.config.bloomDifficulty")}
                  </label>
                  {difficulty && (
                    ...
                    <div className="mt-3 space-y-3 pl-1">
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">
                          {t("creator.config.subject")}
                        </label>
                        <input
                          type="text"
                          value={subject}
                          onChange={(e) => setSubject(e.target.value)}
                          placeholder={t("creator.config.subjectPlaceholder")}
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">
                          {t("creator.config.gradeLevel")}
                        </label>
                        <input
                          type="text"
                          value={gradeLevel}
                          onChange={(e) => setGradeLevel(e.target.value)}
                          placeholder={t("creator.config.gradePlaceholder")}
                          className="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                          disabled={generateMutation.isPending}
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">
                          {t("creator.config.curriculumRef")}
                        </label>
                        <input
                          type="text"
                          value={curriculumRef}
                          ...
                          placeholder={t(
                            "creator.config.curriculumPlaceholder",
                          )}
                          className="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                          disabled={generateMutation.isPending}
                        />
                ...
                >
                  {generateMutation.isPending ? (
                    <>
                      ...
                      {t("creator.generate.processing")}
                    </>
                  ) : (
                    <>
                      <Sparkles className="w-5 h-5" />
                      {t("creator.generate.button")}
                    </>
                  )}
                </button>
              </div>
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-2 px-4 py-2.5 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 text-xs text-slate-500 dark:text-slate-400">
            <span className="flex items-center gap-1.5 font-medium text-slate-700 dark:text-slate-300">
              ...
              {resultFileName || t("creator.results.unknownDocument")}
            </span>
            ...
            <span className="text-slate-300 dark:text-slate-600">·</span>
            <span>
              {t("creator.results.created")}{" "}
              {resultGeneratedAt
                ? new Date(resultGeneratedAt).toLocaleString(
            ...
            <div className="md:col-span-1 bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200">
                  {resultType === "reading"
                    ? t("creator.results.readingText")
                    : resultType === "writing"
                      ? t("creator.results.topicContext")
                      : t("creator.results.summary")}
                </h2>
                {resultId && (
            ...
            <div className="md:col-span-2 bg-slate-100 dark:bg-slate-900 p-4 md:p-6 rounded-3xl border border-slate-200 dark:border-slate-700 flex flex-col min-h-[500px]">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
                <div className="flex items-center gap-3">
                  <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200">
                    {resultType === "writing"
                      ? t("creator.results.topicList")
                      : t("creator.results.questionList")}{" "}
                    ({questions.length})
                  </h2>
                  {/* Select all toggle */}
                  <button
                    ...
                    title={
                      allSelected
                        ? t("creator.selection.deselectAll")
                        : t("creator.selection.selectAll")
                    }
                  >
                    {allSelected ? (
                  ...
                  {selectedCount > 0 && selectedCount < questions.length && (
                    <span className="text-xs text-blue-600 dark:text-blue-400 font-medium">
                      {t("creator.selection.selectedCount").replace(
                        "{count}",
                ...
                <div className="flex gap-2 w-full sm:w-auto items-center">
                  <input
                    ...
                    title={t("creator.results.dueDate")}
                  />
                  <button
                    type="button"
                  ...
                  >
                    ...
                    {isSavingToBank
                      ? t("creator.results.saving")
                      : t("creator.results.questionBank")}
                  </button>
                  <button
                    type="button"
              ...
              <div className="flex items-center gap-4 mb-2">
                ...
                <div>
                  <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100">
                    {t("creator.progress.workingTitle")}
                  </h3>
                  <p className="text-slate-500 dark:text-slate-400 text-sm">
                    {t("creator.progress.workingDescription")}
                  </p>
                </div>
              </div>
