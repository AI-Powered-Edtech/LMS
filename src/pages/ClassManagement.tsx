...
import {
  ArrowLeft, Plus, Users, Copy, Link as LinkIcon,
  Pencil, Trash2, Loader2, X, Check, Search,
...
export function ClassManagement() {
  ...
  const { classrooms, addClassroom, updateClassroom, setActiveClassroomId, loading: classLoading } = useClassroom();
  ...
  useEffect(() => {
    ...
    const fetchCounts = async () => {
      ...
      if (!error && data) {
        ...
        const finalCounts = classrooms.reduce((acc, cls) => {
    ...
    <div className="max-w-7xl mx-auto pb-20">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-sm transition-all shadow-sm shrink-0"
        >
      ...
      {/* Create Class Form */}
      {showCreateForm && (
        <div className="bg-indigo-50 border border-indigo-200 rounded-2xl p-5 mb-6 flex flex-col sm:flex-row items-start sm:items-end gap-3">
          <div className="flex-1 w-full">
            <label className="block text-xs font-bold text-indigo-600 mb-1.5 uppercase tracking-wider">Nama Kelas Baru</label>
            <input
              type="text"
              value={newClassName}
          ...
          <div className="flex gap-2 shrink-0">
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2.5 bg-indigo-600 text-white font-bold text-sm rounded-xl hover:bg-indigo-700 disabled:opacity-50 transition-colors"
            >
            ...
            <button
              ...
              className="p-2.5 text-slate-400 hover:text-slate-600 hover:bg-white rounded-xl transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <h3 className={cn(
                      ...
                      selectedClassId === cls.id ? 'text-indigo-700' : 'text-slate-800'
                    )}>
                      {cls.name}
                    </h3>
                    <div className={cn(
                      'w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-bold shrink-0',
                      selectedClassId === cls.id ? 'bg-indigo-200 text-indigo-700' : 'bg-slate-100 text-slate-500'
                    )}>
        ...
        <div className="flex-1 min-w-0">
          {!selectedClass ? (
            <div className="flex flex-col items-center justify-center py-24 text-slate-400 bg-white border border-slate-200 rounded-2xl">
              <Settings2 className="w-12 h-12 mb-3 opacity-30" />
              <p className="font-medium text-slate-500">Pilih kelas untuk melihat detail</p>
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                  ...
                  <div className="flex-1 min-w-0">
                    {renamingClassId === selectedClass.id ? (
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <button onClick={() => handleRename(selectedClass.id)} className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-lg">
                          <Check className="w-4 h-4" />
                        </button>
                        <button onClick={() => setRenamingClassId(null)} className="p-2 text-slate-400 hover:bg-slate-100 rounded-lg">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => { setRenamingClassId(selectedClass.id); setRenameValue(selectedClass.name); }}
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                      Dibuat {new Date(selectedClass.created_at).toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' })}
                    </p>
                  </div>
                  <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-5">
                  <div className="bg-slate-50 rounded-xl p-4">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Siswa Aktif</p>
                    <div className="flex items-center gap-2">
                      <Users className="w-4 h-4 text-indigo-500" />
                      <span className="text-2xl font-black text-slate-800">{studentCounts[selectedClass.id] ?? 0}</span>
                    </div>
                  </div>

                  <div className="bg-slate-50 rounded-xl p-4 sm:col-span-2">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Kode Gabung</p>
                    <div className="flex items-center gap-3">
                      <span className="text-2xl font-black text-slate-800 tracking-[0.2em]">{selectedClass.join_code}</span>
                      <div className="flex gap-1.5 ml-auto">
                        <button
                          onClick={() => handleCopy(selectedClass.join_code, 'code-' + selectedClass.id)}
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 hover:border-indigo-300 text-slate-600 hover:text-indigo-600 font-medium text-xs rounded-lg transition-colors"
                        >
                          {copiedId === 'code-' + selectedClass.id ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}
                          {copiedId === 'code-' + selectedClass.id ? 'Tersalin!' : 'Salin Kode'}
                        </button>
                        <button
                          onClick={() => handleCopy(`${window.location.origin}/dashboard?join=${selectedClass.join_code}`, 'link-' + selectedClass.id)}
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 hover:border-indigo-300 text-slate-600 hover:text-indigo-600 font-medium text-xs rounded-lg transition-colors"
                        >
                          {copiedId === 'link-' + selectedClass.id ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <LinkIcon className="w-3.5 h-3.5" />}
                ...
                <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-slate-100">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 bg-indigo-50 text-indigo-700 font-bold text-xs rounded-lg hover:bg-indigo-100 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 bg-blue-50 text-blue-700 font-bold text-xs rounded-lg hover:bg-blue-100 transition-colors"
                  >
                    <BookOpen className="w-3.5 h-3.5" />
                    Tugas Kelas
                    <ChevronRight className="w-3 h-3" />
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 bg-emerald-50 text-emerald-700 font-bold text-xs rounded-lg hover:bg-emerald-100 transition-colors"
                  >
                    <BookOpen className="w-3.5 h-3.5" />
                    Analitik
                    <ChevronRight className="w-3 h-3" />
              ...
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                ...
                {loadingStudents ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-slate-400">
                    ...
                    <p className="text-sm font-medium text-slate-500">Belum ada siswa</p>
                  ...
                  <div className="divide-y divide-slate-100">
                    {students.map((student, idx) => (
                      ...
                      >
                        ...
                        <p className="text-[10px] text-slate-400 font-medium shrink-0 hidden sm:block">
                          Bergabung {new Date(student.enrolled_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                        </p>
                        <button
                          onClick={async () => {
