...
async function listVa(tenantId: string): Promise<BankVa[]> {
  const { data, error } = await db
    ...
    .select(
      "id, tenant_id, student_id, bank_code, va_number, is_active, created_at",
    )
...
export function BankVa() {
  ...
  const create = useMutation({
    ...
    onSuccess: () => {
      addToast({
        type: "success",
        message: `VA ${bankCode} ${vaNumber} ditambahkan`,
      });
      setIsOpen(false);
      setStudentId("");
      setVaNumber("");
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Tetapkan nomor VA per siswa untuk pembayaran SPP via direct bank
            (alternatif Midtrans).
          </p>
        </div>
        <Button
