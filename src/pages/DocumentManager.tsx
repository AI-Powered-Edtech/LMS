...
import { FileText, Edit3, CheckCircle, PenTool, FileSignature, Send, Search, FileDown, AlertCircle } from "lucide-react";
import { cn } from "@/src/utils/cn";
import { motion, AnimatePresence } from "motion/react";

const templates = [
  { id: "sk", name: "Surat Keputusan (SK)", icon: FileSignature, color: "text-purple-600", bg: "bg-purple-50" },
  { id: "aktif", name: "Surat Keterangan Aktif", icon: FileText, color: "text-blue-600", bg: "bg-blue-50" },
  { id: "panggilan", name: "Surat Panggilan Ortu", icon: AlertCircle, color: "text-red-600", bg: "bg-red-50" },
];
...
export function DocumentManager() {
  ...
  const handleAutocomplete = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setNisn(val);
    if (val === "12345") {
      setStudentData({ name: "Budi Santoso", className: "XII IPA 1", address: "Jl. Merdeka No. 10" });
      setContent(`SURAT KETERANGAN AKTIF\n\nYang bertanda tangan di bawah ini menerangkan bahwa:\n\nNama: Budi Santoso\nNISN: 12345\nKelas: XII IPA 1\nAlamat: Jl. Merdeka No. 10\n\nAdalah benar siswa aktif di sekolah ini pada tahun ajaran berjalan.`);
    } else {
      setStudentData(null);
      setContent("");
    }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 h-full flex flex-col">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Manajemen Surat & Dokumen</h1>
        ...
        <div className="lg:col-span-1 space-y-6 flex flex-col">
              ...
              {templates.map(t => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedTemplate === t.id ? "border-blue-500 bg-blue-50 shadow-sm" : "border-slate-100 bg-slate-50 hover:bg-slate-100"
                  )}
                >
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex-1">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Cari NISN Siswa (Ketik: 12345)</label>
                ...
                {studentData && (
                  ...
                  >
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between"><span className="text-slate-500">Nama:</span><span className="font-bold text-slate-900">{studentData.name}</span></div>
                      <div className="flex justify-between"><span className="text-slate-500">Kelas:</span><span className="font-bold text-slate-900">{studentData.className}</span></div>
                      <div className="flex justify-between"><span className="text-slate-500">Alamat:</span><span className="font-bold text-slate-900 truncate max-w-[150px]">{studentData.address}</span></div>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
                ...
                return (
                  <div key={step} className="relative flex flex-col items-center gap-2 z-10">
                    <div className={cn(
                      ...
                      isCompleted ? "bg-blue-500 border-blue-500 text-white shadow-md shadow-blue-200" : "bg-white border-slate-300 text-slate-400"
                    )}>
                      {isCompleted ? <CheckCircle className="w-4 h-4" /> : index + 1}
                    </div>
                    <span className={cn("text-xs font-bold absolute -bottom-6 whitespace-nowrap", isActive ? "text-blue-600" : "text-slate-500")}>
                      {step}
                    </span>
                  </div>
                );
          ...
          <div className="flex-1 p-6 flex flex-col min-h-0">
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-16 h-16 bg-white rounded-xl border border-slate-200 flex items-center justify-center shadow-sm">
                      <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=EduSyncSignature" alt="QR" className="w-12 h-12 opacity-80" />
                    </div>
                    <div>
                      <p className="text-sm font-bold text-slate-500">Ditandatangani secara elektronik oleh:</p>
                      <p className="text-xl font-black text-slate-900 font-serif italic mt-1">Dr. H. Ahmad Dahlan, M.Pd.</p>
                      <p className="text-xs text-slate-400 mt-1">Kepala Sekolah • {new Date().toLocaleDateString('id-ID')}</p>
                    </div>
                  </div>
            ...
            <div className="mt-6 flex flex-col sm:flex-row justify-end gap-3 md:gap-4">
              {currentStep < steps.length - 1 ? (
                ...
                >
                  {currentStep === 0 ? "Kirim ke Reviewer" : currentStep === 1 ? "Setujui & Lanjut TTD" : "Terbitkan Dokumen"}
