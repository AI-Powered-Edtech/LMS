...
const CATEGORY_KEYS: Record<DocumentCategory, string> = {
  surat_masuk: "documents.categories.incomingLetter",
  surat_keluar: "documents.categories.outgoingLetter",
  sk: "documents.categories.decree",
  pengumuman: "documents.categories.announcement",
  rapor: "documents.categories.reportCard",
  umum: "documents.categories.general",
};
...
const VISIBILITY_VALUES: DocumentVisibility[] = ["admin", "teacher", "all"];
...
function formatRelativeDate(
  ...
  if (diffMin < 1) return t("documents.time.justNow");
  if (diffMin < 60)
    return t("documents.time.minutesAgo").replace("{count}", String(diffMin));
  if (diffHour < 24)
    return t("documents.time.hoursAgo").replace("{count}", String(diffHour));
  if (diffDay < 7)
    return t("documents.time.daysAgo").replace("{count}", String(diffDay));
  if (diffDay < 30)
    return t("documents.time.weeksAgo").replace(
      "{count}",
      String(Math.floor(diffDay / 7)),
    );
...
function UploadModal({ open, onClose, onSuccess, tenantId }: UploadModalProps) {
  ...
  const categoryUploadOptions = CATEGORY_VALUES.map((value) => ({
    value,
  ...
  const handleFileSelect = useCallback(
    (f: File) => {
      if (f.size > MAX_FILE_SIZE) {
        addToast({
          type: "error",
          message: t("documents.toasts.fileTooLarge"),
        });
        return;
      }
      setFile(f);
  ...
  const handleSubmit = useCallback(async () => {
    if (!file || !title.trim()) {
      addToast({
        type: "error",
        message: t("documents.toasts.titleAndFileRequired"),
      });
      return;
    }
    ...
    try {
      ...
      addToast({
        type: "success",
        message: t("documents.toasts.uploadSuccess"),
      });
      handleClose();
      onSuccess();
    } catch (err) {
      const msg =
        err instanceof Error ? err.message : t("documents.toasts.uploadError");
  ...
  return (
    <Modal open={open} onClose={handleClose} size="lg">
      <ModalHeader title={t("documents.upload.title")} onClose={handleClose} />
      <ModalBody>
        <div className="space-y-4">
          {/* Drop zone */}
          <div
            className={cn(
              "border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500",
              dragActive
            ...
            {file ? (
              <div className="flex items-center justify-center gap-3">
                ...
                <button
                  ...
                  className="ml-2 p-1 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-500"
              ...
              <>
                ...
                <p className="text-sm text-slate-600 dark:text-slate-300">
                  {t("documents.upload.dropzoneTitle")}
                </p>
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
                  {t("documents.upload.dropzoneHint")}
                </p>
              </>
            )}
          </div>
...
function CategoryCard({ count, active, label, onClick }: CategoryCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
...
interface DocumentRowProps {
  ...
  visibilityLabels: Record<DocumentVisibility, string>;
      ...
      <div className="flex items-center gap-1 shrink-0">
        {doc.file_url && (
          <a
            ...
            className="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-blue-600 dark:hover:text-blue-400 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
            title={downloadTitle}
        ...
        <button
          ...
          className="p-1.5 rounded-lg text-slate-500 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500"
          title={deleteTitle}
...
export function DocumentManager() {
  ...
  usePageTitle(t("documents.pageTitle"));
  ...
  const categoryLabels = Object.fromEntries(
    CATEGORY_VALUES.map((value) => [value, t(CATEGORY_KEYS[value])]),
  ) as Record<DocumentCategory, string>;
  const categoryOptions = [
    { value: "all", label: t("documents.categories.all") },
  ...
  const visibilityLabels = Object.fromEntries(
    VISIBILITY_VALUES.map((value) => [
      value,
      t(`documents.visibility.${value}`),
    ]),
  ) as Record<DocumentVisibility, string>;
  const formatDocumentDate = useCallback(
    (date: string) => formatRelativeDate(date, t, locale),
    [locale, t],
  );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const msg =
        err instanceof Error ? err.message : t("documents.toasts.loadError");
      addToast({ type: "error", message: msg });
    } finally {
      setLoading(false);
    }
  }, [filter, addToast, tenantId, t]);
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      ...
      addToast({
        type: "success",
        message: t("documents.toasts.deleteSuccess"),
      });
      void fetchData();
    } catch (err) {
      const msg =
        err instanceof Error ? err.message : t("documents.toasts.deleteError");
      addToast({ type: "error", message: msg });
    } finally {
      setDeleting(false);
      setDeleteTarget(null);
    }
  }, [deleteTarget, addToast, fetchData, t]);
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            {t("documents.totalStored").replace("{count}", String(totalDocs))}
          </p>
        </div>
        {canManage && (
          <Button
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700">
          <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200">
            {filter.category && filter.category !== "all"
              ? categoryLabels[filter.category as DocumentCategory]
              : t("documents.allDocuments")}
            {filter.search && (
              <span className="font-normal text-slate-400 dark:text-slate-500">
                {" "}
                &middot; {t("documents.searchResultLabel")}: &ldquo;
                {filter.search}&rdquo;
              </span>
            )}
          </h2>
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description={
              filter.search
                ? t("documents.emptySearchDescription").replace(
                    "{query}",
                    filter.search,
                  )
          ...
          <div>
            {documents.map((doc) => (
              <DocumentRow
                ...
                downloadTitle={t("documents.actions.download")}
                deleteTitle={t("documents.actions.delete")}
              />
            ))}
          </div>
        )}
      ...
      >
        ...
        <ModalFooter>
          ...
          >
            {t("documents.actions.deletePermanent")}
          </Button>
        </ModalFooter>
      </Modal>
    </div>
