...
export function useSessionManagement(): UseSessionManagementResult {
  ...
  const signIn = useCallback(async (email: string, password: string) => {
    ...
    setAuthError(null);
    const { error } = await getAuthProvider().signInWithPassword({
      email,
      password,
    });
    if (error) {
  ...
  useEffect(() => {
    getAuthProvider()
      ...
      .catch((err) => {
        ...
        setAuthError(
          err instanceof Error ? err.message : "Gagal memulihkan sesi login.",
        );
        setAuthStatus("auth_error");
        setLoading(false);
      });
    ...
    const {
      ...
      if (s?.user) {
        wasAuthenticatedRef.current = true;
        addBreadcrumb(`Auth state changed: ${_event}`, "auth", {
          userId: s.user.id,
        });
        clearOAuthRedirectPending();
        setAuthError(null);
  ...
  useEffect(() => {
    ...
    const checkAndRefresh = async () => {
      const currentSession = (await getAuthProvider().getSession()).data
        .session;
      ...
      if (remainingS <= REFRESH_THRESHOLD_S) {
        ...
        try {
          ...
          if (error) {
            ...
            addBreadcrumb(
              "Proactive token refresh failed — signing out",
              "auth",
              {
                error: error.message,
              },
          ...
          addBreadcrumb(
            "Proactive token refresh exception — signing out",
            "auth",
            {
