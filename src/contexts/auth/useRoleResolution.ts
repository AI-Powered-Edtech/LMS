...
export function useRoleResolution(
  ...
  const fetchUserData = useCallback(async (userId: string) => {
    ...
    try {
      ...
      const loadedMemberships: TenantMembership[] = bootstrap.memberships.map(
        (membership) => ({
      ...
      const message =
        err instanceof Error ? err.message : "Gagal memuat akses ruang kerja.";
      setError(message);
      setBootstrapReady(true);
    ...
    async (userId: string) => {
      ...
      try {
        ...
        const retryCount = parseInt(
          localStorage.getItem("pendingInviteRetryCount") ?? "0",
        );
        if (!isPermanentFailure && retryCount < 1) {
          ...
          if (import.meta.env.DEV)
            logger.warn(
              "[Auth] Transient invite error — will retry on next login",
            );
          return;
        }
  ...
  const processPendingJoinCode = useCallback(async (userId: string) => {
    ...
    try {
      ...
      if (import.meta.env.DEV)
        logger.error("[Auth] Failed to enroll with pending join code:", e);
