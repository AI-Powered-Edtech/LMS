...
export const QuestionEditor: React.FC<QuestionEditorProps> = ({
    ...
    const [formData, setFormData] = useState<CreateQuestionPayload | UpdateQuestionPayload>(emptyQuestion);
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
                ...
                >
                    ...
                    <div className="flex items-center justify-between p-5 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50">
                        ...
                        <button onClick={onClose} className="p-2 text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg">
                            <X className="w-5 h-5" />
                        </button>
                    </div>
                        ...
                        {loading ? (
                            ...
                            <>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-semibold mb-1 text-slate-700 dark:text-slate-300">Tipe Soal</label>
                                        <select
                                            value={formData.type}
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold mb-1 text-slate-700 dark:text-slate-300">Tingkat Kesulitan</label>
                                        <select
                                ...
                                <div>
                                    <label className="block text-sm font-semibold mb-1 text-slate-700 dark:text-slate-300">Pertanyaan</label>
                                    <textarea
                                        value={formData.text}
                                ...
                                {(formData.type !== 'SHORT_ANSWER' && formData.type !== 'ESSAY') && (
                                    <div className="space-y-3">
                                        <div className="flex items-center justify-between">
                                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">Pilihan Jawaban</label>
                                        ...
                                        <div className="space-y-2">
                                            {formData.options.map((opt, idx) => (
                                                <div key={idx} className="flex items-center gap-2 p-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                                                    <button
                                                    ...
                                                    >
                                                        {opt.is_correct && <span className="block w-2.5 h-2.5 bg-white rounded-full scale-100" />}
                                                    </button>
                                ...
                                <div>
                                    <label className="block text-sm font-semibold mb-1 text-slate-700 dark:text-slate-300">Penjelasan (Opsional)</label>
                                    <textarea
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-2">
                                        {formData.tags.map(tag => (
                                            <span key={tag} className="flex items-center gap-1 px-2.5 py-1 text-sm bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-md">
                                                {tag}
