...
const getDifficultyLabel = (level: number) => {
    switch (level) {
        case 1: return { text: 'Sangat Mudah', color: 'bg-green-100 text-green-700' };
        case 2: return { text: 'Mudah', color: 'bg-emerald-100 text-emerald-700' };
        case 3: return { text: 'Sedang', color: 'bg-yellow-100 text-yellow-700' };
        case 4: return { text: 'Sulit', color: 'bg-orange-100 text-orange-700' };
        case 5: return { text: 'Sangat Sulit', color: 'bg-red-100 text-red-700' };
        default: return { text: 'Menengah', color: 'bg-gray-100 text-gray-700' };
    }
};

const getTypeIcon = (type: string) => {
    switch (type) {
        case 'MCQ': return <List className="w-4 h-4" />;
    ...
    return (
        <div 
            className={`bg-white dark:bg-gray-800 border rounded-xl overflow-hidden shadow-sm transition-all ${
                selected ? 'border-indigo-500 ring-1 ring-indigo-500' : 'border-gray-200 dark:border-gray-700 hover:shadow-md'
            }`}
        >
            <div className="p-5 flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                        ...
                        <span className={`text-xs font-semibold px-2.5 py-1 rounded-md border ${defaultDiff.color.replace('bg-', 'border-').replace('text-', 'border-opacity-30 text-')} ${defaultDiff.color}`}>
                            {defaultDiff.text}
                        </span>
                            ...
                            <div className="flex items-center gap-1.5 ml-auto text-gray-500">
                                ...
                                <div className="flex gap-1">
                                    {question.tags.map((tag, idx) => (
                                        <span key={`${question.id}-tag-${idx}`} className="text-xs bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300 px-1.5 py-0.5 rounded">
                                            {tag}
                                        </span>
                                    ))}
                                </div>
