...
export function parseCSVQuestions(csvContent: string): ParseResult {
  ...
  if (lines.length < 2) {
    return {
      questions: [],
      errors: [{ row: 0, message: 'CSV harus memiliki header dan minimal 1 baris data' }],
      totalRows: 0,
    }
  }
...
function parseQuestionRow(fields: string[], rowNum: number): ParsedQuestion {
  ...
  const type = VALID_TYPES.includes(rawType as QuestionType) ? (rawType as QuestionType) : 'MCQ'
  ...
  for (let j = 5; j < fields.length && options.length < MAX_OPTIONS; j += 2) {
    ...
    const isCorrect = TRUTHY_VALUES.has(isCorrectRaw) || isCorrectRaw === 'true'
