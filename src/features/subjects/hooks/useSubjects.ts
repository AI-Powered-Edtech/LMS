...
export function useCurriculumItems(subjectId: string | null) {
  ...
  return useQuery<CurriculumItem[]>({
    ...
    queryFn: () =>
      subjectId ? subjectService.listCurriculumItems(tenantId!, subjectId) : Promise.resolve([]),
...
export function useCreateSubject() {
  ...
  return useMutation({
    mutationFn: (input: { code: string; name: string; schoolBand: 'SD' | 'SMP' | 'SMA'; phase: Subject['is_kurmer_phase']; description?: string }) =>
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: subjectKeys.all(tenantId!) }),
  })
}
...
export function useCreateCurriculumItem() {
  ...
  return useMutation({
    ...
    onSuccess: (_, _vars) => qc.invalidateQueries({ queryKey: ciKeys.all(tenantId!) }).then(() => undefined),
  })
}
