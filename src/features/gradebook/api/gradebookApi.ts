...
export async function fetchGradebookEntries(
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      assignment_id:
        entityType === "assignment" ? (row.entity_id as string) : null,
...
export async function addGradebookItem(data: {
  ...
  const latestOrderRaw =
    existingColumns && (existingColumns as Array<{ order: number }>).length > 0
      ? (existingColumns as Array<{ order: number }>)[0].order
      : null;
  const latestOrder =
...
export async function fetchGradebookSettings(
  ...
  const { data, error } = await db
    .from<any>("gradebook_settings")
    .select(
      `id, tenant_id, course_id, grading_scale, weight_quizzes, weight_assignments`,
    )
    .eq("course_id", courseId)
    .eq("tenant_id", tenantId)
    .maybeSingle();
  ...
  const students: GradebookStudent[] = (
    ...
    (p: {
      ...
      return {
        id: p.id,
        name:
          profile?.name || `${p.first_name} ${p.last_name}`.trim() || p.email,
        nis: p.email.split("@")[0],
        avatarSeed: p.id,
      };
    },
...
export function exportGradebookCSV(
  ...
  for (const [, student] of studentMap) {
    ...
    row["Rata-rata (%)"] =
      gradedCount > 0 ? Number((totalPct / gradedCount).toFixed(1)) : "-";
    rows.push(row);
  }
