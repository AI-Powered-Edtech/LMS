...
function SummaryCard({ average, rank, totalStudents }: SummaryProps) {
  ...
  return (
    ...
    >
      ...
      >
        <span className={cn("text-4xl font-black", letterColorClass(letter))}>
          {letter}
        </span>
      </div>
      ...
      <div className="flex-1 space-y-1">
        ...
        <p className={cn("text-3xl font-black", letterColorClass(letter))}>
          {average.toFixed(1)}%
        </p>
...
function EntryRow({ entry }: { entry: GradebookEntry }) {
  const letter =
    entry.grade_letter ??
    (entry.score != null ? deriveLetter(entry.percentage) : null);
...
export function StudentGradeView({ courseId }: Props) {
  ...
  const myRank = useMemo(() => {
    ...
    for (const e of allEntries) {
      ...
      studentTotals.set(e.student_id, {
        sum: prev.sum + e.percentage,
        count: prev.count + 1,
      });
    }
