...
const calendarKeys = {
  ...
  userEvents: (tenantId: string) => [...base.all(tenantId), 'userEvents'] as const,
}
...
export function useUserCalendarEvents() {
  ...
  return useQuery({
    ...
    queryFn: () => calendarEventService.fetchUserCalendarEvents(tenantId!, { from, to }),
    enabled: !!tenantId,
    staleTime: STALE.DYNAMIC,
  })
}
...
export function usePersistCalendarEvent() {
  ...
  return useMutation({
      ...
      await calendarEventService.createCalendarEvent(
        {
          ...
          event_type:
            eventData.type === 'exam' || eventData.type === 'assignment' ? 'personal' : 'personal',
          color: undefined,
        },
        tenantId,
        user.id
    ...
    onSuccess: () => {
      if (tenantId) {
        void qc.invalidateQueries({ queryKey: calendarKeys.userEvents(tenantId) })
      }
    },
    onError: (err) => {
