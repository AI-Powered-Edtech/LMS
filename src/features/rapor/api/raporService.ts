...
export type RaporStatus = 'draft' | 'guru_signed' | 'wali_signed' | 'kepsek_signed' | 'published'
...
export const raporService = {
  async list(tenantId: string, rombelId?: string | null, semesterId?: string | null): Promise<RaporDocument[]> {
    let q = db
  ...
  async getSubjectGrades(raporId: string): Promise<RaporSubjectGrade[]> {
    const { data, error } = await db
      ...
      .select('id, rapor_id, subject_id, tenant_id, subject_name, nilai_akhir, descriptor, deskripsi_capaian, teacher_id, created_at')
      .eq('rapor_id', raporId)
  ...
  async sign(input: {
    ...
    if (error) {
      ...
      if (error.code === 'P0003') throw new Error('Tahap tanda tangan tidak valid (urutan: guru → wali kelas → kepsek)')
      throw error
    }
