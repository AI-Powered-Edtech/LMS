...
export const dapodikCsvExport = {
  async exportStudents(tenantId: string): Promise<void> {
    ...
    const rows = ((data ?? []) as Array<Record<string, unknown>>).map((p) => {
      const d =
        (p as { student_dossier?: Record<string, unknown> }).student_dossier ??
        {};
      return [
    ...
    downloadCsv(
      `dapodik-siswa-${new Date().toISOString().slice(0, 10)}.csv`,
      csv,
    );
  },
  ...
  async exportStaff(tenantId: string): Promise<void> {
    ...
    const rows = ((data ?? []) as Array<Record<string, unknown>>).map((p) => {
      const d =
        (p as { staff_dossier?: Record<string, unknown> }).staff_dossier ?? {};
      return [
        p.full_name,
        d.nip,
        d.nuptk,
    ...
    downloadCsv(
      `dapodik-staf-${new Date().toISOString().slice(0, 10)}.csv`,
      csv,
    );
  },
  ...
  async exportRombel(tenantId: string): Promise<void> {
    const { data, error } = await db
      .from("rombel")
      .select(
        "id, code, name, capacity, status, wali_kelas_id, academic_year_id",
      )
      .eq("tenant_id", tenantId);
    if (error) throw error;
    ...
    downloadCsv(
      `dapodik-rombel-${new Date().toISOString().slice(0, 10)}.csv`,
      csv,
    );
  },
};
