...
function formatPhoneDisplay(phone: string): string {
  ...
  if (d.length <= 11)
    return `+62 ${d.slice(0, 3)}-${d.slice(3, 7)}-${d.slice(7)}`;
...
function OtpInput({ value, onChange, disabled }: OtpInputProps) {
  ...
  return (
    <div
      ...
      role="group"
      aria-label={t("auth.pages.parentOtpGroupLabel")}
    >
...
export function ParentRegisterPage() {
  ...
  usePageTitle(t("auth.pages.parentRegisterPageTitle"));
  ...
  const handleRequestOtp = async () => {
    ...
    if (!phone || phone.replace(/[^0-9]/g, "").length < 9) {
      setError(t("auth.pages.parentInvalidPhoneError"));
      return;
    }
    ...
    try {
      ...
      if (!result.success) {
        setError(result.error ?? t("auth.pages.parentOtpSendRetryError"));
        return;
      }
      ...
      const msg =
        err instanceof Error ? err.message : t("auth.pages.parentOtpSendError");
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleVerifyOtp = async () => {
    setError("");
    if (otp.length !== 6) {
      setError(t("auth.pages.parentOtpLengthError"));
      return;
    }
    ...
    try {
      ...
      if (!result.success) {
        setError(result.error ?? t("auth.pages.parentOtpInvalidError"));
        return;
      }
      ...
      const msg =
        err instanceof Error
          ? err.message
          : t("auth.pages.parentOtpVerifyError");
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleCompleteRegistration = async () => {
    ...
    if (!profile.fullName.trim()) {
      setError(t("auth.pages.parentFullNameRequiredError"));
      return;
    }
    if (profile.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(profile.email)) {
      setError(t("auth.pages.parentInvalidEmailError"));
      return;
    }
    ...
    try {
      ...
      if (!signUpData.user)
        throw new Error(t("auth.pages.parentCreateAccountError"));
      ...
      if (signInError) {
        ...
        logger.warn(
          "[ParentRegister] Auto sign-in skipped:",
          signInError.message,
        );
      }
      ...
      const msg =
        err instanceof Error
          ? err.message
          : t("auth.pages.parentCompleteRegistrationError");
      setError(msg);
    } finally {
      setLoading(false);
    }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-blue-300/70 text-sm mt-1">
            {t("auth.pages.parentPortalSubtitle")}
          </p>
        </div>
          ...
          {import.meta.env.DEV && step !== 4 && (
            <div className="mb-5 p-3 bg-yellow-500/15 border border-yellow-500/30 rounded-xl">
              <p className="text-yellow-300 text-xs font-semibold uppercase tracking-wide mb-1">
                {t("auth.pages.parentDevModeTitle")}
              </p>
              <p className="text-yellow-200/70 text-xs leading-relaxed">
                {t("auth.pages.parentDevModeDescription")}
                {devOtp && (
                  <>
                    {" "}
                    {t("auth.pages.parentYourOtpCode")}{" "}
          ...
          {step === 4 ? (
            <div className="text-center py-4">
              ...
              <h2 className="text-white font-bold text-xl mb-2">
                {t("auth.pages.parentRegistrationSuccessTitle")}
              </h2>
              <p className="text-white/60 text-sm leading-relaxed mb-6">
                {t("auth.pages.parentRegistrationSuccessDescription")}
              </p>
              <button
                type="button"
              ...
              >
                {t("auth.pages.parentDashboardButton")}
              </button>
              <button
                type="button"
                ...
                <div>
                  <div className="mb-6">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <h2 className="text-white font-bold text-lg">
                        {t("auth.pages.parentRegisterTitle")}
                      </h2>
                    </div>
                    <p className="text-white/50 text-sm">
                      {t("auth.pages.parentPhoneStepDescription")}
                    </p>
                  </div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <input
                        ...
                        type="tel"
                        inputMode="numeric"
                        placeholder={t("auth.pages.parentPhonePlaceholder")}
                        value={phone}
                        onChange={(e) => {
                    ...
                    <p className="text-white/30 text-xs mt-2">
                      {t("auth.pages.parentPhoneExample")}
                    </p>
                  </div>
                  ...
                  >
                    {loading ? (
                      ...
                      t("auth.pages.parentSendVerificationCode")
                    )}
                  </button>

                  <div className="mt-6 text-center">
                    <span className="text-white/40 text-sm">
                      {t("auth.pages.parentAlreadyHaveAccount")}{" "}
                    </span>
                    <Link
                      to="/login"
                      className="text-blue-400 hover:text-blue-300 text-sm font-medium transition-colors"
                    >
                      {t("auth.pages.parentLoginHere")}
                    </Link>
                  </div>
                </div>
              )}
              ...
              {step === 2 && (
                <div>
                  ...
                  >
                    ...
                    {t("auth.pages.parentChangePhone")}
                  </button>
                  ...
                  <div className="mb-6">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <h2 className="text-white font-bold text-lg">
                        {t("auth.pages.parentOtpStepTitle")}
                      </h2>
                    </div>
                    <p className="text-white/50 text-sm">
                      {t("auth.pages.parentCodeSentTo")}{" "}
                ...
                <div>
                  <div className="mb-6">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <h2 className="text-white font-bold text-lg">
                        {t("auth.pages.parentCompleteProfileTitle")}
                      </h2>
                    </div>
                    <p className="text-white/50 text-sm">
                      {t("auth.pages.parentCompleteProfileDescription")}
                    </p>
                  </div>
                  ...
                  <div className="mb-4">
                    <label
                      ...
                      className="block text-white/60 text-xs font-medium mb-1.5 uppercase tracking-wide"
                    >
                      {t("auth.pages.parentFullNameLabel")}{" "}
                      <span className="text-red-400">*</span>
                    </label>
                    <input
                      id="fullName"
                      type="text"
                      placeholder={t("auth.pages.parentFullNamePlaceholder")}
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      type="email"
                      inputMode="email"
                      placeholder={t("auth.pages.parentEmailPlaceholder")}
                      value={profile.email}
                      onChange={(e) => {
                  ...
                  <div className="mb-6">
                    <label className="block text-white/60 text-xs font-medium mb-2 uppercase tracking-wide">
                      {t("auth.pages.parentRelationshipLabel")}{" "}
                      <span className="text-red-400">*</span>
                    </label>
                    <div className="grid grid-cols-2 gap-2">
                      {(
                        [
                          {
                            value: "ayah",
                            label: t("auth.pages.parentRelationshipFather"),
                          },
                          {
                            value: "ibu",
                            label: t("auth.pages.parentRelationshipMother"),
                          },
                          {
                            value: "wali",
                            label: t("auth.pages.parentRelationshipGuardian"),
                          },
                          {
                            value: "kakak",
                            label: t("auth.pages.parentRelationshipSibling"),
                          },
                        ] as const
                      ).map(({ value, label }) => (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            setProfile((p) => ({ ...p, relationship: value }))
                          }
                          className={cn(
                  ...
                  >
                    {loading ? (
                      <>
                        <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                        {t("auth.pages.parentRegistering")}
                      </>
                    ) : (
                      t("auth.pages.parentCompleteRegistrationButton")
                    )}
                  </button>
                </div>
              )}
