...
import {
  type AuthBootstrap as ProviderAuthBootstrap,
  getAuthProvider,
} from "@/services/auth";
...
export const authService = {
  ...
  async exchangeOAuthCode(urlOrCode: string) {
    ...
    const { data, error } =
      await getAuthProvider().exchangeCodeForSession(authCode);
    if (error) throw error;
  ...
  async getAuthBootstrap(): Promise<AuthBootstrap> {
    ...
    const normalized = {
      profile:
        (bootstrap.profile as AuthBootstrapProfile | null | undefined) ?? null,
      memberships:
        (bootstrap.memberships as AuthBootstrapMembership[] | undefined) ?? [],
      default_tenant_id: bootstrap.default_tenant_id ?? null,
      requires_email_verification:
        bootstrap.requires_email_verification ?? false,
    };
  ...
  resolvePostAuthDestination(
    ...
    if (bootstrap.requires_email_verification) {
      ...
      if (activeMemberships.length === 0) {
        ...
        const preferredRedirect = sanitizeRedirectTarget(
          fallbackPath ?? storedRedirect,
        );
  ...
  async acceptInvitation(token: string): Promise<{ success: boolean }> {
    const { data, error } = await getApiClient().rpc("accept_invitation", {
      p_token: token,
    });
    if (error) throw error;
  ...
  async enrollStudent(joinCode: string): Promise<void> {
    const { error } = await getApiClient().rpc("enroll_student", {
  ...
  async validateInvitation(token: string): Promise<InvitationInfo | null> {
    const { data } = await getApiClient().rpc("validate_invitation", {
      p_token: token,
    });
  ...
  async publicLookupClass(joinCode: string): Promise<ClassInfo | null> {
    const { data } = await getApiClient().rpc("public_lookup_class", {
      p_join_code: joinCode,
    });
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status >= 500) {
          logDevError(
            "auth",
            "Rate limit health check failed (5xx) - fail-semi-open:",
            response.status,
          );
          captureError(
            new Error(`rate_limit_health_check_failed_${response.status}`),
            {
        ...
        logDevError(
          "auth",
          "Rate limit health check failed - fail-closed:",
          response.status,
        );
      ...
      logDevError(
        "auth",
        "Rate limit unreachable (network error) - fail-semi-open:",
        err,
      );
      captureError(err, {
        context: "checkRateLimit",
   ...
   */
  async onboardStudentJoinClass(
    params: JoinClassParams,
  ): Promise<JoinClassResult> {
    const { data, error } = await getApiClient().rpc(
      "onboard_student_join_class",
      {
