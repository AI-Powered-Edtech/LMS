...
export function sanitizeRedirectTarget(target: string | null | undefined): string | null {
  ...
  try {
    ...
    if (normalized.startsWith('/auth/callback') || normalized.startsWith('/auth/error')) {
      return null
    }
    ...
    if (target.startsWith('/auth/callback') || target.startsWith('/auth/error')) return null
    return target
  }
}
...
export function normalizeLegacyHashUrl(url: Pick<Location, 'hash'>): string | null {
  const hash = url.hash ?? ''
