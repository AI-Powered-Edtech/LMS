...
describe('struggleService.getStruggleAlerts', () => {
  ...
  it('passes unreadOnly option', async () => {
    ...
    expect(mockRpc).toHaveBeenCalledWith('get_struggle_alerts', expect.objectContaining({
      p_unread_only: true,
    }));
  });
...
describe('struggleService.updateStruggleConfig', () => {
  ...
  it('calls update_struggle_config RPC with updates', async () => {
    ...
    expect(mockRpc).toHaveBeenCalledWith('update_struggle_config', expect.objectContaining({
