...
export const struggleService = {
  ...
  async getStruggleConfig(_tenantId: string): Promise<StruggleConfig | null> {
    ...
    if (error) {
      if (import.meta.env.DEV)
        logger.error("[struggleService] getStruggleConfig:", error);
      throw error;
    }
   ...
   */
  async updateStruggleConfig(
    _tenantId: string,
    updates: Partial<StruggleConfig>,
  ): Promise<void> {
    ...
    if (error) {
      if (import.meta.env.DEV)
        logger.error("[struggleService] updateStruggleConfig:", error);
      throw error;
    }
  },
  ...
  async getStruggleAlerts(
    ...
    if (error) {
      if (import.meta.env.DEV)
        logger.error("[struggleService] getStruggleAlerts:", error);
      throw error;
    }
  ...
  async markAlertsRead(_tenantId: string, alertIds: string[]): Promise<void> {
    ...
    if (error) {
      if (import.meta.env.DEV)
        logger.error("[struggleService] markAlertsRead:", error);
      throw error;
    }
  },
  ...
  async getMyLessonStatus(
    ...
    if (error) {
      ...
      if (
        ...
        return null;
      if (import.meta.env.DEV)
        logger.error("[struggleService] getMyLessonStatus:", error);
      return null;
    }
