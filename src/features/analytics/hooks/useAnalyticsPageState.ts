...
export function useAnalyticsPageState() {
  ...
  const { data, isLoading, error, refetch } =
    useTeacherAnalytics(selectedCourseId);
  ...
  useEffect(() => {
    if (error) {
      if (error instanceof AnalyticsError) {
        ...
        setErrorMessage(
          "Gagal memuat analitik. Pastikan module dan quiz terhubung ke progress.",
        );
      }
    } else {
    ...
    try {
      ...
      if (err instanceof AnalyticsError) {
        switch (err.code) {
          case "PERMISSION_DENIED":
            refreshError =
              "Anda tidak memiliki akses untuk memperbarui analitik.";
            break;
          case "RPC_NOT_FOUND":
            refreshError =
              "Fungsi refresh belum tersedia. Hubungi administrator.";
            break;
          case "NETWORK_ERROR":
