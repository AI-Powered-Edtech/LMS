...
export function RetentionHeatmap({ data }: RetentionHeatmapProps) {
    ...
    if (cohortWeeks.length === 0) {
        return <p className="py-8 text-center text-sm text-slate-400">Belum ada data retensi tersedia.</p>;
    }
                    ...
                    {cohortWeeks.map(week => {
                        ...
                        return (
                            <tr key={week}>
                                ...
                                <td className="px-2 py-1 text-center font-medium text-slate-900 dark:text-white">{cohortSize}</td>
