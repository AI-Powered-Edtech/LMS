...
export function FunnelComparison({ courseId }: FunnelComparisonProps) {
    ...
    const { data: results, isLoading: resultsLoading } = useFunnelResults(selectedFunnelId ?? undefined);
        ...
        <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
            ...
            <AnimatePresence>
                {showBuilder && (
                    ...
                    >
                        <FunnelBuilder courseId={courseId} onSaved={handleSaved} onCancel={() => setShowBuilder(false)} />
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            {isLoading ? (
                ...
                <div className="space-y-1">
                    {funnels.map(f => (
                        <div
                            ...
                            onClick={() => setSelectedFunnelId(prev => prev === f.funnel_id ? null : f.funnel_id)}
                        >
                            <div className="flex items-center gap-2">
                                <ChevronRight className={`h-4 w-4 text-slate-400 transition-transform ${selectedFunnelId === f.funnel_id ? 'rotate-90' : ''}`} />
                                <div>
                            ...
                            <button
                                onClick={e => { e.stopPropagation(); deleteFunnel.mutate(f.funnel_id); }}
                ...
                <p className="py-6 text-center text-sm text-slate-400">Belum ada funnel. Buat yang pertama!</p>
            ) : null}
                ...
                {selectedFunnelId && (
                    ...
                    >
                        {resultsLoading ? (
                                ...
                                <div className="mt-2 grid grid-cols-3 gap-2">
                                    {results.map((step, i) => (
                                        <div key={i} className="rounded-lg bg-white p-2 text-center dark:bg-slate-900">
                                            ...
                                            <p className="text-lg font-bold text-slate-900 dark:text-white">{step.user_count}</p>
                            ...
                            <p className="py-4 text-center text-sm text-slate-400">Belum ada data untuk funnel ini.</p>
                        )}
                    </motion.div>
                )}
            </AnimatePresence>
