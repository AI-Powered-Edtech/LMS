...
export function FunnelComparison({ courseId }: FunnelComparisonProps) {
  ...
  const [pendingDeleteFunnelId, setPendingDeleteFunnelId] = useState<
    string | null
  >(null);
  ...
  return (
    <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
          ...
          {funnels.map((f) => (
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === " ") {
                  ...
                  setSelectedFunnelId((prev) =>
                    prev === f.funnel_id ? null : f.funnel_id,
                  );
                }
              }}
            ...
            >
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setPendingDeleteFunnelId(f.funnel_id);
                }}
      ...
      <ConfirmDialog
        open={pendingDeleteFunnelId !== null}
        title="Hapus corong?"
        description="Aksi ini tidak bisa dibatalkan. Data corong tidak akan tampil lagi."
        confirmLabel="Hapus"
        variant="danger"
        isLoading={deleteFunnel.isPending}
        onCancel={() => setPendingDeleteFunnelId(null)}
        onConfirm={() => {
          if (!pendingDeleteFunnelId) return;
          deleteFunnel.mutate(pendingDeleteFunnelId, {
            onSettled: () => setPendingDeleteFunnelId(null),
          });
        }}
      />
              ...
              <div className="mt-2 rounded-xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800">
                ...
                <div className="mt-2 grid grid-cols-3 gap-2">
                  {results.map((step, i) => (
                    <div
                      key={i}
                      className="rounded-lg bg-white p-2 text-center dark:bg-slate-900"
                    >
