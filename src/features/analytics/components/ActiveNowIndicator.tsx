...
export function ActiveNowIndicator({ count, className }: ActiveNowIndicatorProps) {
  return (
    <div className={cn(
      ...
      <Users className={cn('h-3.5 w-3.5', count > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400')} />
      <span className={cn(
        'text-xs font-bold',
        count > 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400'
      )}>
