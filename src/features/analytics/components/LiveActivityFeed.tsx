...
import {
  analyticsService,
  type LiveEvent,
} from "@/features/analytics/api/analyticsService";
...
export function LiveActivityFeed({
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      if (isPaused) return;
      try {
        const data = await analyticsService.fetchLatestEvents(
          String(activeTenant),
          10,
        );
        ...
        if (isMounted && data) {
          setEvents((prev) => {
            const newEvents = data.filter(
              (d: LiveEvent) => !prev.some((p) => p.id === d.id),
            );
    ...
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-800">
        <div className="flex items-center gap-2">
          <Radio
            className={`h-4 w-4 ${isConnected ? "text-emerald-500" : "text-slate-400"}`}
          />
