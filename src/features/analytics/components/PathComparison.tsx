        ...
        <thead>
          <tr className="border-b border-slate-200 dark:border-slate-700">
            ...
            <th className="py-2 pr-4 text-left font-medium text-indigo-600 dark:text-indigo-400">
              Jalur A — {pathA.user_count} siswa (
              {pathA.avg_completion_rate.toFixed(0)}%)
              {pathA.is_optimal && " ⭐"}
            </th>
            <th className="py-2 text-left font-medium text-purple-600 dark:text-purple-400">
              Jalur B — {pathB.user_count} siswa (
              {pathB.avg_completion_rate.toFixed(0)}%)
              {pathB.is_optimal && " ⭐"}
            </th>
          </tr>
        </thead>
        <tbody>
          {Array.from({ length: maxSteps }).map((_, i) => {
            ...
            return (
              ...
              >
                <td className="py-2 pr-4 text-slate-400 dark:text-slate-500 font-mono">
                  {i + 1}
                </td>
