...
export function StruggleAlertBanner({
  ...
  // ⚡ Perf: consolidate multiple chained passes (.filter then .reduce) into a single, standard for loop to minimize CPU overhead and O(N) intermediate allocations.
  const { lessonsWithStruggle, totalStruggling, totalHighRisk } =
    useMemo(() => {
      const lessonsWithStruggle: LessonAnalytics[] = [];
