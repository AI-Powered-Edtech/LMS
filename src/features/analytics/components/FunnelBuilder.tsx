...
export function FunnelBuilder({ courseId, onSaved, onCancel }: FunnelBuilderProps) {
    ...
    return (
        <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
            <div>
                <label className="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400">Nama Funnel</label>
                <input
                    type="text"
                    value={name}
            ...
            <div className="space-y-2">
                <label className="block text-xs font-medium text-slate-600 dark:text-slate-400">Langkah-Langkah ({steps.length})</label>
                {steps.map((step, i) => (
                ...
                {steps.length < 6 && (
                    <button onClick={addStep} className="flex items-center gap-1.5 text-xs font-medium text-indigo-600 hover:text-indigo-800 dark:text-indigo-400">
