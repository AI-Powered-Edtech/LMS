...
export function EngagementDashboard({ courseId }: EngagementDashboardProps) {
    ...
    const { data: summary, isLoading: sumLoading } = useEngagementSummary(courseId);
            ...
            {!isLoading && summary && summary.length > 0 && (
                <div className="grid grid-cols-4 gap-2">
                    {(['high', 'medium', 'low', 'at_risk'] as const).map(seg => {
                        ...
                        return (
                            <div key={seg} className="rounded-lg border border-slate-100 p-2 text-center dark:border-slate-700">
                                <p className="text-xs text-slate-500">
                                    {seg === 'high' ? 'Tinggi' : seg === 'medium' ? 'Sedang' : seg === 'low' ? 'Rendah' : 'Berisiko'}
                                </p>
                                <p className="text-xl font-bold text-slate-900 dark:text-white">{row?.student_count ?? 0}</p>
                                <p className="text-xs text-slate-400">{row ? `${row.avg_score}` : '–'}</p>
                            </div>
                        );
                    })}
                </div>
