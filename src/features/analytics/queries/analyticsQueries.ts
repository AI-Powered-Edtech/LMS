...
export function useSaveFunnel() {
    ...
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ name, steps, courseId, funnelId }: { name: string; steps: string[]; courseId?: string; funnelId?: string }) =>
...
export function useStudentPath(userId?: string, courseId?: string) {
    const { tenantId } = useAuth();
    return useQuery({
        queryKey: analyticsKeys.studentPath(tenantId!, userId!, courseId!),
        queryFn: () => analyticsService.getStudentPath(userId!, courseId!),
        enabled: !!tenantId && !!userId && !!courseId,
...
export function useStudentPrediction(userId?: string, courseId?: string) {
    const { tenantId } = useAuth();
    return useQuery({
        queryKey: analyticsKeys.studentPrediction(tenantId!, userId!, courseId!),
        queryFn: () => analyticsService.getStudentPrediction(userId!, courseId!),
        enabled: !!tenantId && !!userId && !!courseId,
        staleTime: 5 * 60 * 1000,
    });
}
