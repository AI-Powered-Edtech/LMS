 ...
 */
export function useTenantAnalytics(): UseQueryResult<
  TenantAnalyticsData,
  Error
> {
  const { tenantId } = useAuth();
 ...
 */
export function useRefreshCourseStats(): UseMutationResult<
  void,
  AnalyticsError,
  string
> {
...
export function useCourseDashboard(
  ...
  return useQuery({
    ...
    queryFn: () =>
      analyticsService.getCourseAnalyticsDashboard(courseId!, tenantId!),
    enabled: !!tenantId && !!courseId,
...
export function useLessonDashboard(
  courseId?: string,
): UseQueryResult<LessonAnalytics[], Error> {
  const { tenantId } = useAuth();
  return useQuery({
    queryKey: analyticsKeys.lessonDashboard(tenantId!, courseId!),
    queryFn: () =>
      analyticsService.getLessonAnalyticsDashboard(courseId!, tenantId!),
    enabled: !!tenantId && !!courseId,
    staleTime: STALE.MODERATE,
  });
}
...
export function useStudentSignals(
  ...
  return useQuery({
    ...
    queryFn: () =>
      analyticsService.getStudentSignalsDashboard(
        courseId!,
        tenantId!,
        lessonId,
      ),
...
export function useFunnelResults(
  funnelId?: string,
): UseQueryResult<FunnelStepResult[], Error> {
  const { tenantId } = useAuth();
...
export function useDeleteFunnel(): UseMutationResult<void, Error, string> {
  ...
  return useMutation({
    mutationFn: (funnelId: string) =>
      analyticsService.deleteFunnelDefinition(funnelId),
    onSuccess: () => {
      if (tenantId)
        void queryClient.invalidateQueries({ queryKey: base.all(tenantId) });
    },
