...
export const academicYearService = {
  ...
  async setActive(tenantId: string, yearId: string): Promise<AcademicYear> {
    ...
    if (error) {
      if (error.code === 'P0002') throw new Error('Tahun ajaran tidak ditemukan')
      if (error.code === 'P0003') throw new Error('Tahun ajaran yang sudah diarsipkan tidak dapat diaktifkan kembali')
      throw error
    }
    return (data as unknown) as AcademicYear
  },
