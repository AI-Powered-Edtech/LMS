...
export function FillBlankBlock({
  ...
  const allAnswered = (data?.answers ?? []).every(
    (a) => (answers[a.id] ?? "").trim() !== "",
  );
      ...
      <div className="text-base leading-loose text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-800/50 rounded-xl px-5 py-4 border border-slate-200 dark:border-slate-700">
        {segments.map((seg, i) => {
          ...
          return (
            <span
              key={i}
              className="inline-flex items-baseline gap-0.5 mx-1 relative"
            >
              <span
                ref={inputWidthRef}
              ...
              <input
                ...
                style={{
                  width: `${Math.max(60, (answers[blankId]?.length ?? 0) * 10 + 30)}px`,
                }}
              />
              {isCk && result === true && (
