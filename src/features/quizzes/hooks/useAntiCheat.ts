  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const isBlocked = BLOCKED_COMBOS.some((combo) => {
        ...
        const ctrlMatch = 'ctrlKey' in combo ? e.ctrlKey === combo.ctrlKey : true
        const shiftMatch = 'shiftKey' in combo ? e.shiftKey === combo.shiftKey : !e.shiftKey
    ...
    const devToolsInterval = setInterval(() => {
      ...
      if (open) {
        ...
        if (consecutiveDevToolsCountRef.current >= 2) {
          ...
          const now = Date.now()
          if (now - lastDevToolsSignalRef.current >= DEVTOOLS_SIGNAL_COOLDOWN_MS) {
