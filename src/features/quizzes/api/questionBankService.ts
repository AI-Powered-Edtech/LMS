 ...
 */
export async function deletePoolConfig(
  id: string,
  tenantId: string,
): Promise<void> {
...
export async function getQuestionsInBank(
  ...
  const { data, error } = await db
    .from<any>("question_bank_members")
    .select(
      `
      question_bank!inner (
        id,
  ...
  const safeData = data as Array<{
    question_bank: BankQuestion | BankQuestion[];
  }> | null;
  return (safeData ?? []).map((row) => {
    const q = Array.isArray(row.question_bank)
      ? row.question_bank[0]
      : row.question_bank;
    return {
      id: q?.id ?? "",
 ...
 */
export async function hasPoolConfigs(
  quizId: string,
  tenantId: string,
): Promise<boolean> {
