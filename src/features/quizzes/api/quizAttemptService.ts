...
export async function startQuizAttempt(
  ...
  const assignmentId =
    typeof input === "string" ? null : (input.assignmentId ?? null);
...
async function awardQuizXp(
  ...
  try {
    ...
    if (attemptError || !attemptRow) {
      logDevError(
        "quizPlayer",
        "Failed to fetch attempt for XP award:",
        attemptError,
      );
      return;
    }
    ...
    if (quizError || !quizRow) {
      logDevError(
        "quizPlayer",
        "Failed to fetch quiz for XP award:",
        quizError,
      );
      return;
    }
    ...
    if (score >= passingScore && lessonId) {
      await db.rpc("award_quiz_xp", {
        ...
        p_quiz_id: attemptRow.quiz_id,
        p_score: score,
        p_passing_score: passingScore,
        p_tenant_id: attemptRow.tenant_id,
      });
    }
  } catch (err) {
