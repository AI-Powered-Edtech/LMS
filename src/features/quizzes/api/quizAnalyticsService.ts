...
export const quizAnalyticsService = {
  ...
  async getQuestionDifficulty(
    ...
    if (error) {
      logDevError(
        "quizAnalytics",
        "Error fetching question difficulty:",
        error,
      );
      throw error;
    }
...
export async function getQuestionStats(
  quizId: string,
): Promise<QuestionStatsWithQuestion[]> {
  // Get question stats
  const { data: stats, error } = await db
    .from<any>("question_stats")
    .select(
  ...
  const safeStats = stats as Array<{
    id: string;
    question_id: string;
    quiz_id: string;
    tenant_id: string;
