...
export async function getAssignmentsByQuiz(quizId: string, tenantId: string) {
  const { data, error } = await db
    ...
    .select(
      "id, quiz_id, class_id, tenant_id, status, available_from, due_at, max_attempts",
    )
    .eq("quiz_id", quizId)
  ...
  const { data: classes, error: classError } =
    classIds.length > 0
      ? await db
          .from<Array<{ id: string; name: string | null }>>("classes")
          .select("id, name")
          .eq("tenant_id", tenantId)
          .in("id", classIds)
      : { data: [], error: null };
  ...
  const classMap = new Map(
    ((classes ?? []) as Array<{ id: string; name: string | null }>).map(
...
export async function getAssignmentsByClass(classId: string, tenantId: string) {
  ...
  const classRecord =
    ((classes ?? []) as Array<{ id: string; name: string | null }>)[0] ?? null;
  const questionMap = new Map<string, Array<{ id: string }>>();
  ((questions ?? []) as Array<{ id: string; quiz_id: string }>).forEach(
    (question) => {
 ...
 */
export async function removeQuizAssignment(
  assignmentId: string,
  tenantId: string,
) {
  const { error } = await db
