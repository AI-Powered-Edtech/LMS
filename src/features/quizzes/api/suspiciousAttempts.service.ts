...
export async function getSuspiciousAttempts(
  ...
  if (attemptError) {
    if (import.meta.env.DEV)
      logger.error("Error fetching quiz attempts:", attemptError);
    throw attemptError;
  }
  ...
  if (signalError) {
    if (import.meta.env.DEV)
      logger.error("Error fetching cheating signals:", signalError);
    throw signalError;
  }
  ...
  const quizTitle =
    ((quizzes ?? []) as Array<{ id: string; title: string | null }>)[0]
      ?.title ?? "-";

  for (const row of signalsArray as Array<{
    attempt_id: string;
    id: string;
    signal_type: string;
    metadata: Record<string, unknown> | null;
...
export async function getSuspiciousAttemptCount(
  ...
  if (attemptsError) {
    if (import.meta.env.DEV)
      logger.error("Error fetching attempts for count:", attemptsError);
    return 0;
  }

  const typedAttempts = attempts as { id: string }[] | null;
  ...
  if (error) {
    if (import.meta.env.DEV)
      logger.error("Error counting suspicious attempts:", error);
    return 0;
  }
