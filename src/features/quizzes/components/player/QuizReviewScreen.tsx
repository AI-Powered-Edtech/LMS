    ...
    <div className="max-w-3xl mx-auto space-y-6 flex-1 w-full pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className={cn(
          ...
          <div>
            <p className="text-3xl font-black text-slate-800 dark:text-slate-100">{answeredCount}</p>
            <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Dijawab</p>
          </div>
        </div>
        ...
        <div className={cn(
          ...
          <div>
            <p className="text-3xl font-black text-slate-800 dark:text-slate-100">{unansweredCount}</p>
            <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Belum Dijawab</p>
          </div>
        </div>
        ...
        <div className={cn(
          ...
          <div>
            ...
            <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Ditandai</p>
          </div>
        </div>
      </div>
        ...
        <div className="max-h-80 overflow-y-auto space-y-2 pr-1">
          {questions.map((q, i) => {
            ...
            if (!isAnswered) {
              ...
              const selectedOptions = q.quiz_options?.filter((opt: QuizOptionSnapshot) => selectedIds.includes(opt.id)) || [];
              answerPreview = selectedOptions.map((opt: QuizOptionSnapshot) => opt.text).join(', ');
            }

            const questionText = q.text?.substring(0, 60) || `Soal ${i + 1}`;
            const truncatedQuestion = (q.text?.length ?? 0) > 60 ? questionText + '...' : questionText;
        ...
        {showConfirm && (
          ...
          >
            ...
            <p className="text-sm text-red-600 dark:text-red-400 mb-6">
              Anda masih memiliki soal yang belum dijawab. Jawaban tidak dapat diubah setelah kuis dikirim.
            </p>
