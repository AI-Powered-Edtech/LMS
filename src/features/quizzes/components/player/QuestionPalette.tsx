          ...
          return (
            <button
              ...
              className={cn(
                ...
                isCurrent &&
                  "bg-blue-600 dark:bg-blue-500 text-white border-blue-600 dark:border-blue-500 ring-2 ring-blue-400 ring-offset-1 dark:ring-offset-slate-900 shadow-md shadow-blue-200 dark:shadow-blue-900/40",
                // Flagged (may overlap answered)
                !isCurrent &&
                  isFlagged &&
                  "bg-amber-400 dark:bg-amber-500 text-white border-amber-400 dark:border-amber-500 shadow-sm shadow-amber-100 dark:shadow-amber-900/30",
                // Answered
                !isCurrent &&
                  !isFlagged &&
                  isAnswered &&
                  "bg-emerald-500 dark:bg-emerald-600 text-white border-emerald-500 dark:border-emerald-600 shadow-sm shadow-emerald-100 dark:shadow-emerald-900/30",
                // Unanswered
                !isCurrent &&
                  !isFlagged &&
                  !isAnswered &&
                  "bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-300 dark:border-slate-600 hover:border-slate-400 dark:hover:border-slate-500",
              )}
            >
              {i + 1}
              {isFlagged && !isCurrent && (
