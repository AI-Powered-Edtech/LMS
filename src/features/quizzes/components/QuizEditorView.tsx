...
export function QuizEditorView({
  ...
  const handleImportFromBank = async (bankQuestions: QuestionBankItem[]) => {
    ...
    try {
      ...
      const existingIds = new Set(currentQuestions.map((q) => q.id).filter(Boolean))
      const toImport = bankQuestions.filter((q) => !existingIds.has(q.id))

      if (toImport.length === 0) {
        addToast({ type: 'warning', message: 'Semua soal yang dipilih sudah ada di kuis ini.' })
      ...
      addToast({ type: 'success', message: `${toImport.length} soal berhasil diimpor ke kuis.` })
    ...
    <div className="max-w-4xl mx-auto space-y-6 px-4 md:px-6 lg:px-8 pb-20">
      ...
      {error && (
        <div className="p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 text-sm rounded-xl flex items-start gap-2">
          ...
          <span>{error}</span>
          <button onClick={() => setError(null)} className="ml-auto" aria-label="Tutup peringatan">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm space-y-4">
        ...
        <div className="space-y-4">
          <div>
            ...
            />
            {errors.title && (
              <p className="text-[10px] text-red-500 mt-1 font-medium">{errors.title.message}</p>
            )}
          </div>
                ...
                {watch(`questions.${qIdx}.question_type`) !== 'SHORT_ANSWER' &&
                  watch(`questions.${qIdx}.question_type`) !== 'ESSAY' && (
                    <div className="pl-8 space-y-2">
                      {watch(`questions.${qIdx}.question_type`) === 'MULTIPLE_SELECT' && (
                        <p className="text-[10px] text-slate-400 italic">
                          Klik untuk toggle jawaban benar (bisa lebih dari 1)
                        </p>
                      )}
                      {watch(`questions.${qIdx}.options`)?.map((opt: any, oIdx: number) => (
                        <div key={oIdx} className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            disabled={
                              isPublished ||
                              watch(`questions.${qIdx}.question_type`) === 'TRUE_FALSE'
                            }
                            className={cn(
                          ...
                          {!isPublished &&
                            ...
                            watch(`questions.${qIdx}.question_type`) !== 'TRUE_FALSE' && (
                              <button
                                type="button"
                                onClick={() => {
                                  const currentOptions = getValues(`questions.${qIdx}.options`)
                                  setValue(
                                    `questions.${qIdx}.options`,
                                    currentOptions.filter((_: any, i: number) => i !== oIdx)
                                  )
                                }}
      ...
      {editingQuizId && (
        <ImportFromQuestionBank
          ...
          existingQuestionIds={questionFields.map((q) => q.id).filter(Boolean) as string[]}
