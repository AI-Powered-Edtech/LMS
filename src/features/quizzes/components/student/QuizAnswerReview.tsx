...
export function QuizAnswerReview({
  ...
  const getQuestionTypeLabel = (type: QuestionType): string => {
    switch (type) {
      ...
      case 'MULTIPLE_SELECT': return 'Pilihan Banyak';
  ...
  // Check if a specific option is correct
  const isOptionCorrect = (optionId: string, correctOptionIds: string[]): boolean => {
    ...
    <div className="max-w-4xl mx-auto space-y-6 flex-1 w-full pb-10">
      ...
      <div className="space-y-6">
        {questions.map((question, index) => {
          ...
          const correctOptionIds = question.question_snapshot?.options
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <span className={cn(
                        ...
                        qType === 'ESSAY' ? 'bg-purple-100 text-purple-700' :
                        qType === 'SHORT_ANSWER' ? 'bg-amber-100 text-amber-700' :
                        qType === 'MULTIPLE_SELECT' ? 'bg-cyan-100 text-cyan-700' :
                        qType === 'TRUE_FALSE' ? 'bg-teal-100 text-teal-700' :
              ...
              <div className="p-4 md:p-6">
                ...
                {['MCQ', 'TRUE_FALSE', 'MULTIPLE_SELECT'].includes(qType) && (
                  <div className="space-y-3">
                    {/* Student's Answer */}
                    <div>
                      <p className="text-sm font-bold text-slate-700 mb-2">Jawaban Anda:</p>
                        ...
                        {question.quiz_options?.map((option) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className={cn(
                                "font-medium",
                                isSelected ? "text-slate-900" : "text-green-700"
                              )}>
                                {option.text}
                              </span>
                              {isSelected && !isOptionCorrectAnswer && (
                                <span className="text-xs text-red-600 ml-auto">Jawaban Anda</span>
                              )}
                              {!isSelected && isOptionCorrectAnswer && (
                                <span className="text-xs text-green-600 ml-auto">Jawaban Benar</span>
                              )}
                            </div>
                          );
                        })}
                    ...
                    {!isCorrect && showCorrectAnswers && correctOptionIds.length > 0 && !studentOptionIds.includes(correctOptionIds[0]) && (
                      <div className="mt-3 p-3 bg-green-50 rounded-xl border border-green-200">
                        <p className="text-sm font-bold text-green-800 mb-1">Jawaban yang benar:</p>
                        {question.question_snapshot?.options
                          ?.filter((opt) => opt.is_correct)
                          ?.map((opt) => (
                            <p key={opt.id} className="text-sm text-green-700">{opt.text}</p>
                          ))}
                      </div>
                    )}
                  </div>
