  ...
  return (
    ...
    >
      <div className="p-5 flex-1 flex flex-col">
        ...
        <h3 className="text-lg font-bold text-slate-900 dark:text-white leading-tight mb-1">{quiz.title}</h3>
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mb-4">
           <span className="text-sm font-medium text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-md">{quiz.class_name}</span>
           <span className="text-sm text-slate-500 dark:text-slate-400">{quiz.quiz_questions?.length || 0} Soal {timeLimitMin > 0 ? `• ${timeLimitMin} Menit` : ''}</span>
        </div>

        {availableUntil && (
           <p className={cn("text-xs font-bold mb-3", isExpired ? "text-red-500" : "text-amber-600")}>
             Tenggat: {availableUntil.toLocaleDateString('id-ID', { month: 'short', day: 'numeric' })}
             {' '}{availableUntil.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}
           </p>
        )}

        <p className="text-sm text-slate-600 dark:text-slate-300 mb-6 line-clamp-2 flex-1">{quiz.description}</p>
        ...
        <div className="flex items-center gap-4 text-sm font-medium text-slate-500 dark:text-slate-400 mb-6">
          {maxAttempts > 0 && (
            <div className="flex items-center gap-1.5 text-xs font-bold px-2 py-1 bg-slate-50 dark:bg-slate-700 border border-slate-100 dark:border-slate-600 rounded-md">
              <span>Percobaan: {Math.min(attemptsCount + (activeAttempt ? 0 : 1), maxAttempts)} / {maxAttempts}</span>
            </div>
          )}
        </div>
