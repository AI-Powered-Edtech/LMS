...
import { quizAnalyticsService, type QuizLiveStatus } from '../../api/quizAnalyticsService'
...
function isInactive(lastHeartbeat: string | null, thresholdMinutes = 5): boolean {
...
function StatusBadge({ row }: { row: QuizLiveStatus }) {
  const inactive = isInactive(row.last_heartbeat_at) && row.status === 'in_progress'
...
export function QuizLiveMonitor({ assignmentId, tenantId, className = '' }: QuizLiveMonitorProps) {
  const {
    data: liveStatus = [],
    isLoading,
    dataUpdatedAt,
  ...
  const done = liveStatus.filter(
    (s) => s.status.toLowerCase() === 'submitted' || s.status.toLowerCase() === 'graded'
  ).length
  const suspicious = liveStatus.filter(
    (s) => s.is_suspicious || (isInactive(s.last_heartbeat_at) && s.status === 'in_progress')
  ).length
          ...
          <tbody className="divide-y divide-gray-100 bg-white dark:divide-gray-700 dark:bg-gray-900">
            {isLoading ? (
              ...
              liveStatus.map((row) => {
                ...
                return (
                  <tr key={row.student_id} className={`transition-colors ${rowClass}`}>
                    {/* Nama */}
                    ...
                    {/* Progres */}
                    <td className="px-4 py-3">
                      <ProgressBar answered={row.answered_count} total={row.total_questions} />
                    </td>
                    ...
                    <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
                      {row.last_heartbeat_at ? (
                        <span
                          className={
                            inactive ? 'font-semibold text-amber-600 dark:text-amber-400' : ''
                          }
                        >
