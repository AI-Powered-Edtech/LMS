  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700">
        <p className="font-bold text-slate-800 dark:text-white mb-1">{data.questionText}</p>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-4 text-xs">
          <div className="flex items-center gap-1">
            ...
            <span className="text-slate-600 dark:text-slate-400">Mudah (&gt;70%)</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded-full bg-yellow-500" />
            <span className="text-slate-600 dark:text-slate-400">Sedang (40-70%)</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded-full bg-red-500" />
            <span className="text-slate-600 dark:text-slate-400">Sulit (&lt;40%)</span>
          </div>
        </div>
      </div>
      ...
      {hardQuestions.length > 0 && (
        <div className="mt-4 p-3 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-800">
          ...
          <div className="space-y-1">
            {hardQuestions.map((q) => (
              <p key={q.questionId} className="text-sm text-red-700 dark:text-red-300">
