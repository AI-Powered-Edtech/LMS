...
import type { QuizAttempt, StartQuizAttemptInput, SubmitAnswer } from '../types/quizzes.types'
...
export function useSubmitQuizAttempt() {
  ...
  return useMutation({
    ...
    onMutate: async ({ attemptId }) => {
      ...
      await queryClient.cancelQueries({ queryKey: QuizKeys.userAttempts(tenantId) })
      ...
      queryClient.setQueryData<QuizAttempt[]>(QuizKeys.userAttempts(tenantId), (old) => {
        if (!old) return old
        return old.map((attempt) =>
          attempt.id === attemptId
            ? { ...attempt, status: 'SUBMITTED', submitted_at: new Date().toISOString() }
            : attempt
        )
      })
    ...
    onError: (err, variables, context) => {
      ...
      if (tenantId && context?.previousAttempts !== undefined) {
        queryClient.setQueryData(QuizKeys.userAttempts(tenantId), context.previousAttempts)
      }
