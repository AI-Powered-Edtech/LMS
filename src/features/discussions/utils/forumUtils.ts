...
export function mapToPost(
  d: Discussion,
  repliesMap: Record<string, Discussion[]>,
): ForumPost {
  const isAnon = d.is_anonymous ?? false;
  const replies = (repliesMap[d.id] ?? []).sort(
    (a, b) =>
      (b.is_best_answer ? 1 : 0) - (a.is_best_answer ? 1 : 0) ||
  ...
  return {
    ...
    avatar: isAnon
      ...
      : (d.author?.avatar_url ??
        `https://api.dicebear.com/7.x/avataaars/svg?seed=${d.author_id}`),
    role: "Siswa",
    points: 0,
    badges: [],
    ...
    comments: replies.map((r) => ({
      ...
      avatar:
        r.author?.avatar_url ??
        `https://api.dicebear.com/7.x/avataaars/svg?seed=${r.author_id}`,
      role: "Siswa",
      points: 0,
      badges: [],
      content: r.content,
