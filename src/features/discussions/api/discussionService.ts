...
export const discussionService = {
  ...
  async togglePostVote(
    ...
    if (error) {
      if (error.code === "PGRST202" || error.code === "42883") {
        if (import.meta.env.DEV)
          logger.warn(
            "[discussionService] toggle_post_vote RPC not found — migration needed.",
          );
  ...
  async setBestAnswer(
    ...
    if (postError) {
      if (import.meta.env.DEV)
        logger.error(
          "[discussionService] setBestAnswer pre-verify error:",
          postError,
        );
      throw postError;
    }
    if (!post) {
      throw new Error(
        "Post tidak ditemukan atau tidak ada akses ke tenant ini.",
      );
    }
    ...
    if (error) {
      ...
      if (error.code === "PGRST202") {
        if (import.meta.env.DEV)
          logger.warn(
            "[discussionService] set_best_answer RPC not found — migration needed.",
          );
        return;
      }
      if (import.meta.env.DEV)
        logger.error("Error setting best answer:", error);
      throw error;
    }
  },
};
