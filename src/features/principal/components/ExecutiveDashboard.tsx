  ...
  if (data.length === 0) {
    return (
      <div className="flex items-center justify-center h-64 text-slate-400 dark:text-slate-500">
        <p className="text-sm">Belum ada data tren aktivitas.</p>
      </div>
    )
  }
        ...
        <Legend
          ...
          formatter={(value) => {
            const labels: Record<string, string> = {
              ...
              lesson_completions: 'Penyelesaian Pelajaran',
...
function ROICard({ paperSheets, paperCost, teacherHours, adoptionScore }: ROICardProps) {
  return (
        ...
        <div className="flex items-start gap-3 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/30">
          ...
          <div>
            <p className="text-sm font-semibold text-emerald-800 dark:text-emerald-300">
              ~{fmtNumber(paperSheets)} lembar dihemat
            </p>
            <p className="text-xs text-emerald-600 dark:text-emerald-400">
              Setara {fmtCurrency(paperCost)} per bulan
            </p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 p-3 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/30">
          ...
          <div>
            <p className="text-sm font-semibold text-blue-800 dark:text-blue-300">
              ~{teacherHours.toFixed(1)} jam/minggu lebih efisien
            </p>
            <p className="text-xs text-blue-600 dark:text-blue-400">Dari penilaian tugas digital</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between p-3 rounded-xl bg-violet-50 dark:bg-violet-900/20 border border-violet-100 dark:border-violet-800/30">
          <div>
            ...
            <p className="text-lg font-bold text-violet-800 dark:text-violet-300">
              ...
              <span className="text-sm font-normal text-violet-600 dark:text-violet-400">/100</span>
            </p>
          </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          <div className="flex justify-between items-center mb-1.5">
            <span className="text-sm text-slate-600 dark:text-slate-400">Rata-rata Nilai</span>
          ...
          <div className="w-full h-2.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-500 ${
                avgScore >= 75 ? 'bg-emerald-500' : avgScore >= 60 ? 'bg-amber-500' : 'bg-red-500'
              }`}
        ...
        >
          <div>
            ...
            >
              {fmtNumber(needsAttention)} siswa
            </p>
          </div>
          {needsAttention > 0 && (
            <button
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
      ...
      <div>
        <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">Belum Ada Data</h3>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 max-w-sm">
          Data akan muncul setelah guru dan siswa mulai menggunakan EduSync secara aktif.
        </p>
      </div>
    </div>
  )
...
export function ExecutiveDashboard() {
  ...
  const { overview, monthlyTrend, roiMetrics, settings, isLoading } = useExecutiveData()
  const { surveys } = useSurveys()

  const schoolName = settings?.school_name ?? activeTenant?.name ?? 'Sekolah'
    ...
    return [
      {
        ...
        sub: `${fmtNumber(overview.active_students)} / ${fmtNumber(overview.total_students)} siswa`,
        badgeText: fmtPercent(studentPct),
        badgeVariant: studentPct >= 70 ? 'success' : studentPct >= 40 ? 'warning' : 'neutral',
      ...
      },
      {
        ...
        sub: `${fmtNumber(overview.active_teachers)} / ${fmtNumber(overview.total_teachers)} guru`,
        badgeText: fmtPercent(teacherPct),
        badgeVariant: teacherPct >= 70 ? 'success' : teacherPct >= 40 ? 'warning' : 'neutral',
  ...
  const quickActions: QuickAction[] = [
    ...
    {
      icon: '📊',
      label: 'Export untuk Yayasan',
      description: 'Format Excel/CSV',
      onClick: () => setShowReportGenerator(true),
    },
    {
      icon: '⚙️',
      label: 'Pengaturan Dashboard',
      description: 'Konfigurasi tampilan',
      onClick: () => setShowSettingsModal(true),
    },
    {
      icon: '📋',
      label: 'Jadwalkan Laporan',
      description: 'Laporan otomatis bulanan',
      onClick: () => setShowReportScheduler(true),
    },
    {
      icon: '📈',
      label: 'Analitik Sebelum & Sesudah',
      description: 'Dampak nyata implementasi LMS',
      onClick: () => navigate('/app/principal/analytics'),
    },
    {
      icon: '📋',
      label: 'Kelola Survey Kepuasan',
      description: 'Survey guru, siswa & orang tua',
      onClick: () => navigate('/app/principal/survey'),
    },
  ]

  const hasData = overview && (overview.total_students > 0 || overview.total_courses > 0)
  ...
  return (
    <div className="min-h-full space-y-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-base font-bold text-slate-800 dark:text-slate-100">
                Tren Aktivitas 6 Bulan
              </h2>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Siswa aktif, penyelesaian pelajaran, dan percobaan kuis
              </p>
            </div>
          </div>
          {isLoading ? (
        ...
        >
          ...
          <div>
            <p className="text-sm font-bold text-slate-800 dark:text-slate-100 group-hover:text-blue-700 dark:group-hover:text-blue-300 transition-colors">
              Lihat Analitik Sebelum &amp; Sesudah LMS
            </p>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Bandingkan kondisi sekolah sebelum dan sesudah implementasi EduSync
            </p>
          </div>
        </button>
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              {activeSurveys.length > 0
                ? `${activeSurveys.length} survey aktif — klik untuk lihat respons`
                : 'Buat survey untuk guru, siswa, dan orang tua'}
            </p>
          </div>
          {activeSurveys.length > 0 && (
            <Badge variant="success" size="sm" className="flex-shrink-0 ml-auto">
              {activeSurveys.length} Aktif
            </Badge>
          )}
        </button>
      </div>
      ...
      {/* ── Modals ── */}
      <ReportGenerator open={showReportGenerator} onClose={() => setShowReportGenerator(false)} />
      <ReportScheduler open={showReportScheduler} onClose={() => setShowReportScheduler(false)} />
    </div>
  )
}
