...
//   7. Footer: generated by EduSync LMS
//
...
const TD_RIGHT_CLS =
  "border border-slate-300 px-3 py-2 text-slate-700 text-right tabular-nums";
const SECTION_TITLE =
...
function ReportHeader({ data }: { data: ExecutiveReportData }) {
  ...
  const reportTypeLabel =
    data.reportType === "monthly"
      ? t("reportPreview.reportTypes.monthly")
      : data.reportType === "academic"
        ? t("reportPreview.reportTypes.academic")
        : t("reportPreview.reportTypes.roi");
  return (
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {t("reportPreview.header.academicYear").replace(
              "__YEAR__",
              data.academicYear,
            )}
          </p>
        ...
        <div className="text-right">
          ...
          <div className="text-sm text-slate-500 mt-1">
            {t("reportPreview.header.period").replace(
              "__PERIOD__",
              data.period,
            )}
          </div>
          <div className="text-xs text-slate-400 mt-0.5">
            {t("reportPreview.header.generated").replace(
              "__DATE__",
...
function ExecutiveSummarySection({ data }: { data: ExecutiveReportData }) {
  ...
  return (
    <section>
      <h2 className={SECTION_TITLE}>{t("reportPreview.summary.title")}</h2>
      <table className={TABLE_CLS}>
        <thead>
          <tr>
            <th className={TH_CLS}>{t("reportPreview.summary.metric")}</th>
            <th className={`${TH_CLS} text-right`}>
              {t("reportPreview.summary.value")}
            </th>
            <th className={TH_CLS}>{t("reportPreview.summary.note")}</th>
          </tr>
        </thead>
        <tbody>
          {topMetrics.map((m) => (
...
function ActivityTrendSection({ data }: { data: ExecutiveReportData }) {
  ...
  return (
    <section>
      <h2 className={SECTION_TITLE}>{t("reportPreview.activity.title")}</h2>
      <table className={TABLE_CLS}>
        <thead>
          <tr>
            <th className={TH_CLS}>{t("reportPreview.activity.month")}</th>
            <th className={`${TH_CLS} text-right`}>
              {t("reportPreview.activity.activeStudents")}
            </th>
            <th className={`${TH_CLS} text-right`}>
              {t("reportPreview.activity.lessonCompletions")}
            </th>
            <th className={`${TH_CLS} text-right`}>
              {t("reportPreview.activity.quizAttempts")}
            </th>
          </tr>
        </thead>
        <tbody>
          {data.monthlyTrend.map((row) => (
            <tr key={row.month}>
              ...
              <td className={TD_RIGHT_CLS}>
                {fmtNumber(row.lesson_completions)}
              </td>
          ...
          {data.monthlyTrend.length === 0 && (
            <tr>
              <td
                colSpan={4}
                className={`${TD_CLS} text-center text-slate-400`}
              >
                {t("reportPreview.activity.empty")}
              </td>
            </tr>
          )}
        </tbody>
...
function AcademicSection({ data }: { data: ExecutiveReportData }) {
  ...
  return (
    <section>
      <h2 className={SECTION_TITLE}>{t("reportPreview.academic.title")}</h2>
      <table className={TABLE_CLS}>
        <thead>
          <tr>
            <th className={TH_CLS}>{t("reportPreview.common.indicator")}</th>
            <th className={`${TH_CLS} text-right`}>
              {t("reportPreview.summary.value")}
            </th>
            <th className={TH_CLS}>{t("reportPreview.common.status")}</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td className={TD_CLS}>
              {t("reportPreview.academic.avgQuizScore")}
            </td>
            <td className={TD_RIGHT_CLS}>{fmtNumber(academic.avgScore)}/100</td>
            <td className={TD_CLS}>
              {academic.avgScore >= 75
                ? t("reportPreview.status.good")
                : academic.avgScore >= 60
                  ? t("reportPreview.status.fair")
                  : t("reportPreview.status.needsAttention")}
            </td>
          </tr>
          <tr>
            <td className={TD_CLS}>
              {t("reportPreview.academic.projectedPassRate")}
            </td>
            <td className={TD_RIGHT_CLS}>
              {fmtPercent(academic.projectedPassRate)}
            </td>
            <td className={TD_CLS}>
              {academic.projectedPassRate >= 80
                ? t("reportPreview.status.targetReached")
                : t("reportPreview.status.needsImprovement")}
            </td>
          </tr>
          <tr>
            <td className={TD_CLS}>
              {t("reportPreview.academic.totalStudents")}
            </td>
            <td className={TD_RIGHT_CLS}>
              {fmtNumber(academic.totalStudents)}
            </td>
            <td className={TD_CLS}>—</td>
          </tr>
          <tr>
            <td className={TD_CLS}>
              {t("reportPreview.academic.activeStudents30")}
            </td>
            <td className={TD_RIGHT_CLS}>
              {fmtNumber(academic.activeStudents)}
            </td>
            <td className={TD_CLS}>
              {fmtPercent(
              ...
              {t("reportPreview.academic.adoption")}
            </td>
          </tr>
          <tr>
            <td className={TD_CLS}>
              {t("reportPreview.academic.atRiskStudents")}
            </td>
            <td className={TD_RIGHT_CLS}>
              {fmtNumber(academic.atRiskStudents)}
            </td>
            <td className={TD_CLS}>
              {academic.atRiskStudents === 0
                ? t("reportPreview.status.allActive")
                : t("reportPreview.status.inactive30")}
            </td>
          </tr>
          <tr>
            <td className={TD_CLS}>
              {t("reportPreview.academic.totalCourses")}
            </td>
...
function AdoptionSection({ data }: { data: ExecutiveReportData }) {
  ...
  return (
    <section>
      <h2 className={SECTION_TITLE}>{t("reportPreview.adoption.title")}</h2>
      <table className={TABLE_CLS}>
        <thead>
          <tr>
            <th className={TH_CLS}>{t("reportPreview.common.indicator")}</th>
            <th className={`${TH_CLS} text-right`}>
              {t("reportPreview.adoption.percentage")}
            </th>
            <th className={TH_CLS}>{t("reportPreview.summary.note")}</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td className={TD_CLS}>
              {t("reportPreview.adoption.studentAdoption")}
            </td>
            <td className={TD_RIGHT_CLS}>
              {fmtPercent(adoption.studentAdoptionPct)}
            </td>
            <td className={TD_CLS}>
              {t("reportPreview.adoption.studentNote")}
            </td>
          </tr>
          <tr>
            <td className={TD_CLS}>
              {t("reportPreview.adoption.teacherAdoption")}
            </td>
            <td className={TD_RIGHT_CLS}>
              {fmtPercent(adoption.teacherAdoptionPct)}
            </td>
            <td className={TD_CLS}>
              {t("reportPreview.adoption.teacherNote")}
            </td>
          </tr>
          <tr>
            <td className={TD_CLS}>
              {t("reportPreview.adoption.digitalScore")}
            </td>
            <td className={TD_RIGHT_CLS}>
            ...
            <td className={TD_CLS}>
              {adoption.adoptionScore >= 70
                ? t("reportPreview.status.digitalFirst")
                : adoption.adoptionScore >= 40
                  ? t("reportPreview.status.growing")
                  : t("reportPreview.status.needsPush")}
            </td>
          </tr>
        </tbody>
      </table>
...
function ROISection({ data }: { data: ExecutiveReportData }) {
  ...
  return (
    <section>
      <h2 className={SECTION_TITLE}>{t("reportPreview.roi.title")}</h2>
      <table className={TABLE_CLS}>
        <thead>
          <tr>
            <th className={TH_CLS}>{t("reportPreview.roi.category")}</th>
            <th className={`${TH_CLS} text-right`}>
              {t("reportPreview.roi.estimate")}
            </th>
            <th className={TH_CLS}>{t("reportPreview.roi.basis")}</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td className={TD_CLS}>{t("reportPreview.roi.paperSaved")}</td>
            <td className={TD_RIGHT_CLS}>
              ~{fmtNumber(roi.paperSavedSheets)} {t("reportPreview.roi.sheets")}
            </td>
            <td className={TD_CLS}>{t("reportPreview.roi.paperBasis")}</td>
          </tr>
          <tr>
            <td className={TD_CLS}>{t("reportPreview.roi.paperCost")}</td>
            <td className={TD_RIGHT_CLS}>{fmtCurrency(roi.paperSavedCost)}</td>
            <td className={TD_CLS}>{t("reportPreview.roi.paperCostBasis")}</td>
          </tr>
          <tr>
            <td className={TD_CLS}>{t("reportPreview.roi.teacherTime")}</td>
            <td className={TD_RIGHT_CLS}>
              ~{roi.teacherTimeSavedHours} {t("reportPreview.roi.hoursPerWeek")}
            </td>
            <td className={TD_CLS}>
              {t("reportPreview.roi.teacherTimeBasis")}
            </td>
          </tr>
        </tbody>
      </table>
...
function ReportFooter({ generatedAt }: { generatedAt: string }) {
  ...
  return (
    <footer className="mt-10 pt-4 border-t border-slate-300 text-xs text-slate-400 flex items-center justify-between">
      <span>{t("reportPreview.footer.generatedBy")}</span>
...
export function ReportPreview() {
  ...
  const reportType = (searchParams.get("type") ??
    "monthly") as ExecutiveReportData["reportType"];
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      ...
      if (detectStubResponse(data, t("reportPreview.detectStub"))) {
        throw new Error(t("reportPreview.errors.backendPlaceholder"));
      }
      if (data?.reportData) {
        setReportData(data.reportData as ExecutiveReportData);
      } else {
        throw new Error(t("reportPreview.errors.dataNotFound"));
      }
    } catch (err) {
      const msg =
        err instanceof Error ? err.message : t("reportPreview.errors.unknown");
      setError(msg);
    } finally {
      setIsLoading(false);
    }
  ...
  if (error || !reportData) {
    return (
      <ErrorState
        message={error ?? t("reportPreview.errors.unavailable")}
        onBack={handleBack}
      />
    );
  }
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 px-4 sm:px-6 py-3 flex items-center justify-between gap-3 shadow-sm">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="sm" onClick={handleBack}>
            {t("reportPreview.actions.back")}
          </Button>
          <div>
            <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">
              {t("reportPreview.actions.preview")}
            </p>
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            icon={<span>📊</span>}
          >
            {t("reportPreview.actions.downloadCsv")}
          </Button>
          <Button
            variant="primary"
            size="sm"
            ...
            icon={<span>🖨️</span>}
          >
            {t("reportPreview.actions.printPdf")}
          </Button>
        </div>
      </div>
