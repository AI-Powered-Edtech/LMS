...
export function ReportGenerator({ open, onClose }: ReportGeneratorProps) {
  ...
  function toggleFormat(fmt: ReportFormat) {
    setState((prev) => {
      ...
      return {
        ...prev,
        formats: has ? prev.formats.filter((f) => f !== fmt) : [...prev.formats, fmt],
      }
    })
  }
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Gagal membuat laporan. Coba lagi.'
      setError(msg)
    } finally {
      setIsLoading(false)
    }
  ...
  const selectedMonthLabel = ID_MONTHS.find((m) => m.value === String(state.month))?.label ?? ''
              ...
              {REPORT_TYPES.map((rt) => (
                ...
                >
                  <input
                    ...
                    onChange={() => setState((prev) => ({ ...prev, reportType: rt.value }))}
            ...
            <div className="flex gap-3">
              <div className="flex-1">
                <Select
                  ...
                  onChange={(e) =>
                    setState((prev) => ({ ...prev, month: parseInt(e.target.value, 10) }))
                  }
                />
              </div>
              <div className="flex-1">
                <Select
                  ...
                  onChange={(e) =>
                    setState((prev) => ({ ...prev, year: parseInt(e.target.value, 10) }))
                  }
                />
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 dark:text-slate-500">{fmt.desc}</p>
                    </div>
                  </label>
                )
              })}
