...
export function exportToCSV(reportData: ExecutiveReportData): void {
  ...
  sections.push(buildRow(['Bulan', 'Siswa Aktif', 'Penyelesaian Pelajaran', 'Percobaan Kuis']))
  for (const row of monthlyTrend) {
    sections.push(
      buildRow([row.month, row.active_students, row.lesson_completions, row.quiz_attempts])
    )
  }
  sections.push('')
  ...
  sections.push(buildRow(['Rata-rata Nilai Kuis', `${fmtNumber(academic.avgScore)}/100`]))
  sections.push(buildRow(['Tingkat Kelulusan Proyeksi', fmtPercent(academic.projectedPassRate)]))
  ...
  sections.push(buildRow(['Siswa Butuh Perhatian', fmtNumber(academic.atRiskStudents)]))
  sections.push(buildRow(['Total Kursus Aktif', fmtNumber(academic.totalCourses)]))
  sections.push('')

  // ── Section 5: Adopsi Platform ──
  sections.push(buildRow(['ADOPSI PLATFORM']))
  sections.push(buildRow(['Metrik', 'Persentase']))
  sections.push(buildRow(['Adopsi Siswa', fmtPercent(adoption.studentAdoptionPct)]))
  sections.push(buildRow(['Adopsi Guru', fmtPercent(adoption.teacherAdoptionPct)]))
  sections.push(buildRow(['Skor Adopsi Digital', `${fmtNumber(adoption.adoptionScore)}/100`]))
  sections.push('')

  // ── Section 6: ROI & Penghematan ──
  sections.push(buildRow(['ROI & ESTIMASI PENGHEMATAN']))
  sections.push(buildRow(['Metrik', 'Nilai']))
  sections.push(buildRow(['Lembar Kertas Dihemat', `~${fmtNumber(roi.paperSavedSheets)} lembar`]))
  sections.push(buildRow(['Estimasi Penghematan Kertas', IDR.format(roi.paperSavedCost)]))
  sections.push(buildRow(['Efisiensi Waktu Guru', `~${roi.teacherTimeSavedHours} jam/minggu`]))
  sections.push('')
