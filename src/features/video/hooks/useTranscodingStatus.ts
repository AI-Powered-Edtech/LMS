...
export function useTranscodingStatus(
  options: UseTranscodingStatusOptions = {},
) {
    ...
    async (id: string) => {
      ...
      try {
        ...
        const response = await fetch(
          `${API_BASE}/api/v1/storage/transcode-status/${id}`,
          {
            headers: {
        ...
        const transcodingStatus: TranscodingStatus = {
          ...
          progressPercent:
            rawStatus.progressPercent ?? rawStatus.progress_percent ?? 0,
          hlsManifestUrl:
            rawStatus.hlsManifestUrl ?? rawStatus.hls_manifest_url,
          thumbnailUrl: rawStatus.thumbnailUrl ?? rawStatus.thumbnail_url,
          durationSeconds:
            rawStatus.durationSeconds ?? rawStatus.duration_seconds,
