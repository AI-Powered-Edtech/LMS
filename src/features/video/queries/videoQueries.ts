 ...
 */
export function useVideoAssetByBlock(blockId: string | undefined, tenantId: string | null) {
  return useQuery({
    queryKey:
      blockId && tenantId ? videoQueryKeys.byBlock(tenantId, blockId) : ['video-assets', 'skip'],
 ...
 */
export function useVideoAssetsByLesson(lessonId: string | undefined, tenantId: string | null) {
  return useQuery({
    queryKey:
      lessonId && tenantId ? videoQueryKeys.byLesson(tenantId, lessonId) : ['video-assets', 'skip'],
...
export function useUploadVideo() {
  ...
  return useMutation({
    mutationFn: ({ file, lessonId, blockId, tenantId, onProgress }: UploadVideoArgs) =>
      videoUploadService.uploadVideo(file, lessonId, blockId, tenantId, onProgress),
