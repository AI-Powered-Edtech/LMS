...
export function useCourseActions(
    ...
    async (courseId: string) => {
      ...
      try {
        const { course, modules } =
          await builderCourseService.fetchCourseStructure(courseId, tenantId);
  ...
  const publishCourse = useCallback(async () => {
    ...
    if (state.modules.length === 0) {
      addToast({
        type: "error",
        message:
          "Gagal dipublish: Kursus harus memiliki setidaknya satu modul.",
      });
      return;
    }

    const hasLessons = state.modules.some(
    ...
    if (!hasLessons) {
      addToast({
        type: "error",
        message:
          "Gagal dipublish: Modul harus memiliki setidaknya satu pelajaran.",
      });
      return;
    }
    ...
    if (!hasPublishedLessons) {
      addToast({
        type: "error",
        message:
          "Gagal dipublish: Setidaknya satu pelajaran harus dipublikasikan terlebih dahulu.",
      });
      return;
    }
    ...
    try {
      ...
      addToast({
        type: "error",
        message:
          error instanceof Error ? error.message : "Gagal menerbitkan kursus",
      });
    }
  }, [
    state.courseId,
  ...
  const draftCourse = useCallback(async () => {
    ...
    try {
      ...
      addToast({
        type: "success",
        message: "Kursus berhasil dikembalikan ke draf",
      });
      // Audit log (fire-and-forget)
