...
export interface BuilderState {
  ...
  error: string | null;
  /**
   * Tracks block state for lessons that were edited offline and then closed.
   * Key = lessonId, Value = blocks at the time the lesson was closed.
   * Cleared when a lesson is freshly loaded from server (LOAD_BLOCKS_SUCCESS).
   */
  pendingBlocksByLesson: Record<string, DomainBlock[]>;
...
function safeDeepClone<T>(obj: T): T {
  try {
    ...
    if (import.meta.env.DEV)
      logger.warn(
        "[builderReducer] structuredClone failed, falling back to JSON:",
        err,
      );
  }
  ...
  switch (action.type) {
    ...
    case "DELETE_LESSON":
      return {
        ...
        activeLesson:
          state.activeLesson?.id === action.lessonId
            ? null
            : state.activeLesson,
      };
    case "LOAD_BLOCKS_START":
      return {
        ...state,
        loadingBlocks: true,
        pendingLessonId: action.lessonId,
      };
    ...
    case "LOAD_BLOCKS_SUCCESS": {
      ...
      // Fresh server data supersedes any pending offline edits for this lesson
      const { [action.lessonId]: _removed, ...remainingPending } =
        state.pendingBlocksByLesson;
      return {
        ...state,
        loadingBlocks: false,
        pendingLessonId: null,
        ...
        activeBlockId: null,
        pendingBlocksByLesson: remainingPending,
      };
    }
    case "CLOSE_LESSON":
      // If the closing lesson has blocks, preserve them for offline sync on reconnect
      if (state.activeLesson && state.activeLesson.blocks.length > 0) {
        return {
          ...
          pendingBlocksByLesson: {
            ...state.pendingBlocksByLesson,
            [state.activeLesson.id]: state.activeLesson.blocks,
          },
        };
      }
      return {
    ...
    case "DELETE_BLOCK":
      ...
      return {
        ...
        activeLesson: {
          ...state.activeLesson,
          blocks: state.activeLesson.blocks.filter(
            (b) => b.id !== action.blockId,
          ),
        },
        activeBlockId:
          state.activeBlockId === action.blockId ? null : state.activeBlockId,
      };
        ...
        modules: state.modules.map((m) => ({
          ...
          lessons: m.lessons.map((l) =>
            l.id === action.lessonId
              ? { ...l, is_remedial: action.isRemedial }
              : l,
          ),
        })),
      };
