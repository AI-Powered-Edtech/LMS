...
export function useClassManagementState() {
    ...
    const fetchCounts = async () => {
      ...
      await Promise.all(
        classIds.map(async (id) => {
          counts[id] = await classroomService.getActiveEnrollmentCount(
            id,
            tenantId!,
          );
        }),
  ...
  const fetchStudents = useCallback(
    async (classId: string) => {
      ...
      try {
        const enrolledStudents = await classroomService.getEnrolledStudents(
          classId,
          tenantId,
        );
  ...
  const undoableDeleteState = useRef<{
    classId: string;
    className: string;
  } | null>(null);
