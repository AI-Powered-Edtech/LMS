...
export const classroomService = {
  ...
  async fetchClassrooms(
    ...
    const classIds = ((enrollments ?? []) as Array<{ class_id: string }>)
  ...
  async createClassroom(
    ...
    const { error } = await db.from<Array<Classroom>>("classes").insert({
      name,
  ...
  async joinClassroom(joinCode: string): Promise<void> {
    ...
    if (error) {
      ...
      if (message.includes("matching the ON CONFLICT specification")) {
        throw new Error(
          "Sistem kelas sedang disinkronkan. Coba gabung lagi beberapa saat lagi.",
        );
      }
   ...
   */
  async getActiveEnrollmentCount(
    classId: string,
    tenantId: string,
  ): Promise<number> {
