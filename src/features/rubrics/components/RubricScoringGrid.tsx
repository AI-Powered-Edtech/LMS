...
export function RubricScoringGrid({
  ...
  const handleLevelClick = (
    ...
    onChange([
      ...scores.filter((s) => s.criterion_id !== criterionId),
      updated,
    ]);
  };
  ...
  const handleCommentChange = (criterionId: string, comment: string) => {
    ...
    if (existing) {
      onChange(
        scores.map((s) =>
          s.criterion_id === criterionId ? { ...s, comment } : s,
        ),
      );
    } else {
      ...
      onChange([
        ...scores,
        { criterion_id: criterionId, level_id: null, score: 0, comment },
      ]);
    }
  };
            ...
            <div className="grid grid-cols-1 gap-2">
              {sortedLevels.map((level) => (
                <RubricLevelCell
                  ...
                  onClick={() =>
                    handleLevelClick(criterion.id, level.id, level.points)
                  }
                />
              ))}
            </div>
