...
export function RubricLevelCell({ level, isSelected, onClick }: RubricLevelCellProps) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <div className="flex justify-between items-start mb-1">
        <span
          className={cn(
            'font-bold text-xs',
            isSelected ? 'text-green-700 dark:text-green-400' : 'text-slate-700 dark:text-slate-300'
          )}
        >
          {level.label}
        </span>
