...
type Action =
  ...
  | {
      ...
      payload: { id: string; updates: Partial<RubricCriterion> };
    }
...
export function useRubricBuilder(assignmentId?: string) {
  ...
  const reorderCriteria = useCallback(
    (criteria: RubricCriterion[]) =>
      dispatch({ type: "REORDER_CRITERIA", payload: criteria }),
    [],
  );

  const addLevel = useCallback(
    (criterionId: string) =>
      dispatch({ type: "ADD_LEVEL", payload: criterionId }),
    [],
  );
  ...
  const updateLevel = useCallback(
    (criterionId: string, levelId: string, updates: Partial<RubricLevel>) =>
      dispatch({
        type: "UPDATE_LEVEL",
        payload: { criterionId, levelId, updates },
      }),
    [],
  );
