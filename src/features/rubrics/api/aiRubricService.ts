...
Kembalikan HANYA JSON (tanpa teks lain):
{
  ...
  "criteria": [
    ...
    return {
      title:
        rubricData.title || `Rubrik Penilaian ${assignmentTitle || "Tugas"}`,
      description: "Dibuat oleh AI — silakan tinjau dan sesuaikan",
      is_template: false,
      assignment_id: null,
      created_by: "",
      criteria: (
        rubricData.criteria || buildFallbackRubric(assignmentTitle).criteria!
      ).map((c: AIRubricCriterion, i: number) => ({
  ...
  const DEFAULT_LEVELS = [
    {
      label: "Sangat Baik",
      points: 25,
      description: "Memenuhi semua kriteria dengan sangat baik",
    },
    {
      label: "Baik",
      points: 20,
      description: "Memenuhi sebagian besar kriteria dengan baik",
    },
    {
      label: "Cukup",
      points: 15,
      description: "Memenuhi beberapa kriteria dengan cukup",
    },
    {
