...
export const rubricService = {
  ...
  async getRubricByAssignment(
    ...
    if (findError) {
      logDevError(
        "rubricService",
        "Error finding rubric by assignment:",
        findError,
      );
      throw findError;
    }
    ...
    const rubricRowTyped = rubricRow as { id: string };
    const { data, error } = await db.rpc("get_rubric_with_criteria", {
      p_rubric_id: rubricRowTyped.id,
    });

    if (error) {
      logDevError(
        "rubricService",
        "Error fetching rubric with criteria:",
        error,
      );
      throw error;
    }
