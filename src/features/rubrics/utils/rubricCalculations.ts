...
export function validateRubric(rubric: {
  ...
  rubric.criteria.forEach((c, idx) => {
    ...
    if (c.levels.length === 0) {
      errors.push(
        `Kriteria #${num}: harus memiliki setidaknya satu tingkat penilaian.`,
      );
    }
    c.levels.forEach((l, li) => {
      if (!l.label?.trim()) {
        errors.push(
          `Kriteria #${num}, tingkat #${li + 1}: label tidak boleh kosong.`,
        );
      }
      if (l.points < 0) {
        errors.push(
          `Kriteria #${num}, tingkat #${li + 1}: poin tidak boleh negatif.`,
        );
      }
    });
  });
