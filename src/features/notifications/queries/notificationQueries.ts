...
const notificationKeys = {
    ...
    user: (tenantId: string, userId: string) =>
        [...base.all(tenantId), 'user', userId] as const,
...
export function useMarkAllAsRead() {
    ...
    return useMutation({
        mutationFn: () =>
            notificationService.markAllAsRead(user!.id, tenantId!),
        onSuccess: () => {
            if (tenantId && user) {
                queryClient.invalidateQueries({
                    queryKey: notificationKeys.user(tenantId, user.id),
...
export function useSendNotification() {
    ...
    return useMutation({
        mutationFn: ({ userId, title, message, type }: {
            userId: string;
            title: string;
            message: string;
            type?: string;
...
export function useUnreadCount() {
    ...
    const query = useQuery({
        queryKey: notificationKeys.unreadCount(tenantId!, user!.id),
        queryFn: () => notificationService.getUnreadCount(user!.id, tenantId!),
        enabled: !!tenantId && !!user,
    });
