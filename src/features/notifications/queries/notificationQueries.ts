 ...
 * Uses polling (60s) instead of WebSocket to reduce backend load.
 */
  ...
  return useMutation({
    ...
    onError: (err) => {
      ...
      const isRpcMissing =
        ...
        err.message.includes("create_notification belum tersedia");
      if (isRpcMissing) {
        addToast({
          type: "warning",
          message:
            "Gagal mengirim notifikasi — fungsi database belum tersedia.",
