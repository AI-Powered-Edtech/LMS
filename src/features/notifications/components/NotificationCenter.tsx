...
const NotificationItem = memo(function NotificationItem({
  ...
  const readStatus = notification.is_read
    ? t("notificationsCenter.status.read")
    : t("notificationsCenter.status.unread");
  const ariaLabel = `${notification.title}, ${readStatus}${url ? `, ${t("notificationsCenter.status.clickToOpen")}` : ""}`;
    ...
    >
      ...
      {!notification.is_read && (
        <button
          ...
          aria-label={t("notificationsCenter.actions.markItemReadAria").replace(
            "{title}",
            notification.title,
          )}
          className={cn(
...
const DATE_GROUP_LABEL_KEYS: Record<DateGroup, string> = {
  "Hari Ini": "notificationsCenter.dateGroups.today",
  Kemarin: "notificationsCenter.dateGroups.yesterday",
  "Minggu Lalu": "notificationsCenter.dateGroups.lastWeek",
  "Lebih Lama": "notificationsCenter.dateGroups.older",
};
...
export const NotificationCenter = memo(function NotificationCenter({
  ...
  return (
    <div
      aria-label={t("notificationsCenter.aria.panel")}
      aria-live="polite"
      aria-atomic="false"
    ...
    >
      ...
      <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between shrink-0 bg-white dark:bg-slate-900 z-20">
        <div>
          ...
          >
            {unreadCount > 0
              ? t("notificationsCenter.unreadCount").replace(
                  "__COUNT__",
                  String(unreadCount),
                )
        ...
        {activeTab === "notifications" && unreadCount > 0 && (
          <button
            ...
            onClick={markAllRead}
            aria-label={t(
              "notificationsCenter.actions.markAllReadAria",
      ...
      <div
        role="tablist"
        aria-label={t("notificationsCenter.aria.tabs")}
      ...
      >
        ...
        >
          ...
          {t("notificationsCenter.tabs.notifications")}
          {unreadCount > 0 && (
            <span
              aria-hidden="true"
        ...
        >
          ...
          {t("notificationsCenter.tabs.settings")}
        </button>
      </div>
        ...
        >
          {isLoading && recent.length === 0 ? (
            <div
              ...
              aria-label={t("notificationsCenter.loading")}
            >
            ...
            <EmptyState
              ...
              title={t("notificationsCenter.empty.title")}
              description={t("notificationsCenter.empty.description")}
            />
          ) : (
