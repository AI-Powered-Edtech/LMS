...
async function fetchAdminNotifications(
  ...
  if (error) {
    if (import.meta.env.DEV)
      logger.error("[useAdminNotifications] fetch error:", error);
    throw error;
  }
    ...
    const channel = getRealtimeProvider()
      ...
      .on(
        ...
        (payload) => {
          ...
          if (
            !ADMIN_NOTIFICATION_TYPES.includes(
              incoming.type as AdminNotificationType,
            )
          )
            return;
  ...
  const markReadMutation = useMutation({
    ...
    onSettled: () => {
      void queryClient.invalidateQueries({
        queryKey: adminNotificationKeys.all(tenantId!),
      });
    },
  });
