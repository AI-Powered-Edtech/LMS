...
export const timetableService = {
  async listForRombel(tenantId: string, rombelId: string, academicYearId?: string | null): Promise<TimetableSlot[]> {
    let q = db
  ...
  async upsert(input: Omit<TimetableSlot, 'id' | 'created_at' | 'updated_at'> & { id?: string }): Promise<TimetableSlot> {
    if (input.id) {
