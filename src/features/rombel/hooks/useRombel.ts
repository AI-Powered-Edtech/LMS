...
import {
  type Rombel,
  rombelService,
} from "@/features/rombel/api/rombelService";
...
export function useRombelMembers(rombelId: string | null) {
  return useQuery({
    ...
    queryFn: () =>
      rombelId ? rombelService.listMembers(rombelId) : Promise.resolve([]),
    enabled: !!rombelId,
  });
}
...
export function useCreateRombel() {
  ...
  return useMutation({
    ...
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: rombelKeys.all(tenantId!) }),
  });
}
...
export function useUpdateRombel() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      id: string;
      patch: Parameters<typeof rombelService.update>[2];
...
export function useEnrollRombelMember() {
  ...
  return useMutation({
    ...
    onSuccess: (_, vars) =>
      qc.invalidateQueries({ queryKey: ["rombel_members", vars.rombelId] }),
  });
}
