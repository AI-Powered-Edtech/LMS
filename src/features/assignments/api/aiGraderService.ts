    ...
    try {
      ...
      if (!response.ok) {
        if (import.meta.env.DEV)
          logger.error("[AI Grader] API error:", response.status);
      ...
      if (import.meta.env.DEV)
        logger.error("[AI Grader] Unexpected error:", err);
      ...
      throw new Error(
        "Terjadi kesalahan yang tidak terduga saat menghubungi layanan AI.",
      );
    }
  },
};
