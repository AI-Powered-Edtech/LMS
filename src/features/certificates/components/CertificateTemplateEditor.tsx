...
export function CertificateTemplateEditor({
  ...
  const [values, setValues] = useState<FormValues>(() => ({
    ...
    name: template?.name ?? '',
    background_color: template?.background_color ?? DEFAULT_TEMPLATE.background_color,
    ...
    show_teacher_sig: template?.show_teacher_sig ?? DEFAULT_TEMPLATE.show_teacher_sig,
      ...
      <div className="flex-1 space-y-5 min-w-0">
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            ...
            onChange={(e) =>
              set('font_family', e.target.value as CertificateTemplate['font_family'])
            }
            className={cn(
              'rounded-lg border px-3 py-2 text-sm',
