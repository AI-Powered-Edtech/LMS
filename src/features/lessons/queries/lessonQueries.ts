...
export function useStudentEnrollments() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<EnrolledCourse[]> => {
      ...
      const courseIds = ((data as Array<{ course_id: string }>) ?? []).map(
      ...
      return ((courses ?? []) as EnrolledCourse[]).filter(
...
export function useScormProgress(scormPackageId: string) {
  ...
  return useQuery({
    queryKey: lessonKeys.scormProgress(
      user?.id ?? "",
      scormPackageId,
      tenantId ?? "",
    ),
    queryFn: async () => {
      ...
      return lessonService.getScormRuntimeData(
        user.id,
        scormPackageId,
        tenantId,
      );
