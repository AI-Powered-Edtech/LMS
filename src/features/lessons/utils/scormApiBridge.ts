...
import type { ScormCommitPayload } from "./scormTypes";

// ── Types ──────────────────────────────────────────────────────

export type { ScormCommitPayload } from "./scormTypes";
...
export function createScormBridge(options: ScormBridgeOptions): ScormBridge {
  ...
  const scorm12Api = {
    ...
    LMSFinish(_param: string): string {
      ...
      onTerminate(payload).catch((err) =>
        logger.error("[ScormBridge] Terminate error:", err),
      );
    ...
    LMSCommit(_param: string): string {
      ...
      onCommit(payload).catch((err) =>
        logger.error("[ScormBridge] Commit error:", err),
      );
  ...
  return {
    attach(targetWindow: Window) {
      if (version === "1.2") {
        ...
        (targetWindow as unknown as Record<string, unknown>).API_1484_11 =
          scorm2004Api;
      }
