...
export function useOfflineSync({ lesson, moduleId, onSyncComplete }: UseOfflineSyncParams) {
  ...
  const queueUpdate = useCallback((updateFn: () => Promise<void>) => {
    const schedule = globalThis.requestIdleCallback ?? ((cb: () => void) => setTimeout(cb, 50))
    schedule(async () => {
      try {
        await updateFn()
      } catch (err) {
  ...
  const handleResumeAnchorUpdate = useCallback(
    (anchor: { lastBlockId: string; lastBlockIndex: number; lastBlockOffset: number }) => {
      if (!lesson || !user || !moduleId || !tenantId) return
      queueUpdate(async () => {
        await lessonService.queueProgressUpdate(lesson.id, tenantId, 'in_progress', 0, undefined, {
