...
export function useLessonViewerState() {
  ...
  const isLastLesson =
    currentLessonIndex >= 0 && currentLessonIndex === moduleLessons.length - 1;
  const completedLessonCount = moduleLessons.filter(
    (l) => moduleProgress[l.id]?.completed,
  ).length;
  ...
  const handleCompletionMet = useCallback(async () => {
    ...
    try {
      ...
      if (moduleId && moduleCompleteShownRef.current !== moduleId) {
        ...
        const allDone =
          ...
          moduleLessonsRef.current.every(
            (l) =>
              updatedProgress[l.id]?.completed ||
              updatedProgress[l.id]?.status === "completed",
          );
      ...
      addToast({
        message: "Gagal menandai selesai. Coba lagi.",
        type: "error",
      });
    }
  }, [
  ...
  const handleResumeAnchorUpdate = useCallback(
    async (anchor: {
      ...
      if (!lessonId || !tenantId || !user?.id || state.status === "completed")
        return;
  ...
  useEffect(() => {
    ...
    lessonService
      ...
      .catch((err) => {
        if (import.meta.env.DEV)
          logger.error("Failed to load module lessons:", err);
      })
      .finally(() => {
