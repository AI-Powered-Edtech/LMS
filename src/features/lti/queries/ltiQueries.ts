...
export function useCreateLtiPlatform() {
  ...
  return useMutation({
    mutationFn: (params: CreateLtiPlatformParams) =>
      ltiService.createPlatform(params),
    onSuccess: () => {
      if (tenantId) {
...
export function useUpdateLtiPlatform() {
  ...
  return useMutation({
    mutationFn: (params: UpdateLtiPlatformParams) =>
      ltiService.updatePlatform(params, tenantId!),
    onSuccess: () => {
      if (tenantId) {
        void queryClient.invalidateQueries({ queryKey: ltiKeys.all(tenantId) });
      }
