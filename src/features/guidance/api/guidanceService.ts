...
export const guidanceService = {
  async getApplicableGuides(
    targetType: string,
    targetId: string,
  ): Promise<ApplicableGuide[]> {
    ...
    if (error) {
      ...
      if (import.meta.env.DEV)
        logger.error("[guidanceService] getApplicableGuides:", error);
      return [];
    }
  ...
  async recordInteraction(guideId: string, action: string): Promise<void> {
    ...
    if (error) {
      if (import.meta.env.DEV)
        logger.error("[guidanceService] recordInteraction:", error);
  ...
  async listGuides(
    ...
    if (error) {
      if (import.meta.env.DEV)
        logger.error("[guidanceService] listGuides:", error);
      throw error;
    }
  ...
  async upsertGuide(
    ...
    if (error) {
      if (import.meta.env.DEV)
        logger.error("[guidanceService] upsertGuide:", error);
      throw error;
    }
  ...
  async deleteGuide(guideId: string): Promise<void> {
    ...
    if (error) {
      if (import.meta.env.DEV)
        logger.error("[guidanceService] deleteGuide:", error);
      throw error;
    }
  },
};
