...
export const guidanceService = {
  async getApplicableGuides(targetType: string, targetId: string): Promise<ApplicableGuide[]> {
    const { data, error } = await apiFetch('/rpc/get_applicable_guides', { method: 'POST', body: JSON.stringify({
  ...
  async recordInteraction(guideId: string, action: string): Promise<void> {
    const { error } = await apiFetch('/rpc/record_guide_interaction', { method: 'POST', body: JSON.stringify({
          p_guide_id: guideId,
          p_action: action,
        }) })
    if (error) {
  ...
  async listGuides(targetType?: string, targetId?: string): Promise<LearningGuide[]> {
    const { data, error } = await apiFetch('/rpc/list_learning_guides', { method: 'POST', body: JSON.stringify({
  ...
  async upsertGuide(
    ...
    const { data, error } = await apiFetch('/rpc/upsert_learning_guide', { method: 'POST', body: JSON.stringify({
  ...
  async deleteGuide(guideId: string): Promise<void> {
    const { error } = await apiFetch('/rpc/delete_learning_guide', { method: 'POST', body: JSON.stringify({
          p_guide_id: guideId,
        }) })
    if (error) {
