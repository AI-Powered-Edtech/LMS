...
export function useApplicableGuides(targetType?: string, targetId?: string) {
    ...
    return useQuery({
        ...
        queryFn: async (): Promise<ApplicableGuide[]> => {
            const dbGuides = await guidanceService.getApplicableGuides(targetType!, targetId!);
            if (dbGuides.length > 0) return dbGuides;
            // Fallback: filter DEFAULT_GUIDES by targetType and map to ApplicableGuide shape
            return DEFAULT_GUIDES
                .filter(g => !targetType || g.target_type === targetType)
                .map((g, idx) => ({
                    id: `default-${idx}`,
                    title: g.title,
                    content: g.content,
...
export function useUpsertGuide() {
    ...
    return useMutation({
        mutationFn: (params: Partial<LearningGuide> & { target_id: string; title: string; content: string }) =>
