...
export function useStudentProgressData() {
  ...
  const modules = useQuery({
    ...
    queryFn: () =>
      studentProgressService.fetchModules(tenantId!, user?.id ?? undefined),
    enabled: !!tenantId,
  });
  ...
  const lessonProgress = useQuery({
    ...
    queryFn: () =>
      studentProgressService.fetchLessonProgress(userId!, tenantId!),
  ...
  const achievements = useQuery({
    ...
    queryFn: async () => {
      const badges = await studentProgressService.fetchAchievements(
        userId!,
        tenantId!,
      );
  ...
  return {
    ...
    unlockModule: (_moduleId: string) => {
      ...
      if (import.meta.env.DEV)
        logger.warn("unlockModule not implemented in React Query hooks");
    },
  };
}
  ...
  return useMutation({
    ...
    onMutate: async (amount) => {
      ...
      if (userId && tenantId) {
        ...
        const previousXp = queryClient.getQueryData<number>(
