...
export function ProfileForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) return;
    if (!firstName.trim()) {
      setErrorMsg(t("profile.account.firstNameRequired"));
      return;
    }
    ...
    try {
      ...
      setErrorMsg(
        err instanceof Error
          ? err.message
          : t("profile.account.saveChangesError"),
      );
    }
  };
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-base font-bold text-slate-900 dark:text-white">
            {t("profile.account.profileInfoTitle")}
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            {t("profile.account.profileInfoDescription")}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 mb-6 p-4 bg-slate-50 dark:bg-slate-700/40 rounded-2xl">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-1 italic">
            {t("profile.account.avatarAutoGenerated")}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Input
            label={t("profile.account.firstNameLabel")}
            value={firstName}
          ...
          />
          <Input
            label={t("profile.account.lastNameLabel")}
            value={lastName}
        ...
        {status === "success" && (
          <div className="flex items-center gap-2 text-sm text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800/40 rounded-xl px-4 py-2.5">
            ...
            {t("profile.account.changesSavedSuccess")}
          </div>
        )}
        ...
        <div className="flex justify-end pt-2">
          ...
          >
            {t("profile.account.saveChangesButton")}
          </Button>
        </div>
      </form>
    </Card>
