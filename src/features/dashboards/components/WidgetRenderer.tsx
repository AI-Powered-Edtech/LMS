...
import { useEngagementSummary, useEngagementTrend, useAtRiskStudents, useRetentionMatrix, useFunnelList, useFunnelResults } from '@/src/features/analytics/queries/analyticsQueries';
...
function MetricCard({ metric, label, courseId }: { metric: string; label: string; courseId?: string }) {
  const { data: summary, isLoading } = useEngagementSummary(courseId ?? '');

  const getValue = () => {
    if (!summary) return '-';
    switch (metric) {
      case 'total_students': return summary.reduce((a: number, r: { student_count: number }) => a + r.student_count, 0);
      case 'at_risk_count': return summary.find((r: { segment: string }) => r.segment === 'at_risk')?.student_count ?? 0;
      case 'avg_engagement': {
        const total = summary.reduce((a: number, r: { student_count: number }) => a + r.student_count, 0);
        const high = summary.find((r: { segment: string }) => r.segment === 'high')?.student_count ?? 0;
        const med = summary.find((r: { segment: string }) => r.segment === 'medium')?.student_count ?? 0;
        if (total === 0) return '-';
        return `${Math.round(((high * 100 + med * 60) / (total * 100)) * 100)}%`;
      }
      case 'avg_completion': return `${Math.round(
        summary.reduce((a: number, r) => a + ((r as unknown as { avg_completion_pct?: number }).avg_completion_pct ?? 0), 0) / Math.max(summary.length, 1)
      )}%`;
      default: return '-';
    }
  };

  if (isLoading) return <Loader2 className="h-6 w-6 animate-spin text-slate-400 mx-auto" />;

  return (
    <div className="flex flex-col items-center justify-center h-full p-4 text-center">
      <div className="text-3xl font-black text-indigo-600 dark:text-indigo-400">{String(getValue())}</div>
...
function PieChartWidget({ courseId }: { courseId?: string }) {
  const { data: summary, isLoading } = useEngagementSummary(courseId ?? '');
  if (isLoading) return <div className="flex items-center justify-center h-full"><Loader2 className="h-6 w-6 animate-spin text-slate-400" /></div>;
...
function FunnelWidget({ courseId }: { courseId?: string }) {
  ...
  const { data: results, isLoading: resultsLoading } = useFunnelResults(firstFunnelId);

  if (funnelLoading || resultsLoading) return <div className="flex items-center justify-center h-full"><Loader2 className="h-6 w-6 animate-spin text-slate-400" /></div>;
  if (!results || results.length === 0) return <p className="text-center text-sm text-slate-400 py-8">Belum ada data funnel.</p>;
...
export function WidgetRenderer({ widget, courseId, className }: WidgetRendererProps) {
  ...
  return (
    <div className={`h-full w-full overflow-hidden ${className ?? ''}`}>
      ...
      {widget.type === 'pie_chart' && (
        <PieChartWidget courseId={courseId} />
      )}
      {widget.type === 'engagement_trend' && (
        <EngagementTrendWidget courseId={courseId} />
      )}
      {widget.type === 'risk_radar' && (
        <RiskRadarWidget courseId={courseId} />
      )}
      {widget.type === 'heatmap' && (
        <HeatmapWidget courseId={courseId} />
      )}
      {widget.type === 'funnel' && (
        <FunnelWidget courseId={courseId} />
      )}
      ...
      {!['metric_card', 'pie_chart', 'engagement_trend', 'risk_radar', 'heatmap', 'funnel', 'leaderboard'].includes(widget.type) && (
