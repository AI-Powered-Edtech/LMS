...
function MetricCard({
  ...
  const getValue = () => {
    ...
    switch (metric) {
      case 'total_students':
        return summary.reduce((a: number, r: { student_count: number }) => a + r.student_count, 0)
      case 'at_risk_count':
      ...
      case 'avg_completion':
        return `${Math.round(
          summary.reduce(
            (a: number, r) =>
              a + ((r as unknown as { avg_completion_pct?: number }).avg_completion_pct ?? 0),
            0
...
function FunnelWidget({ courseId }: { courseId?: string }) {
  ...
  if (!results || results.length === 0)
    return <p className="text-center text-sm text-slate-400 py-8">Belum ada data funnel.</p>
...
export function WidgetRenderer({ widget, courseId, className }: WidgetRendererProps) {
  ...
  return (
    <div className={`h-full w-full overflow-hidden ${className ?? ''}`}>
      ...
      {widget.type === 'engagement_trend' && <EngagementTrendWidget courseId={courseId} />}
