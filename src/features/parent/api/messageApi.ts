...
export async function getThreads(parentId: string): Promise<MessageThread[]> {
  const { data, error } = await db
    .from<any>("parent_teacher_threads")
    .select(
  ...
  if (error) {
    if (import.meta.env.DEV)
      logger.error("[MessageApi] getThreads error:", error);
    throw new Error("Gagal memuat daftar pesan. Silakan coba lagi.");
  }

  if (!data || (data as Array<unknown>).length === 0) return [];
...
export async function getMessages(threadId: string): Promise<ThreadMessage[]> {
  const { data, error } = await db
    .from<any>("parent_teacher_messages")
  ...
  if (error) {
    if (import.meta.env.DEV)
      logger.error("[MessageApi] getMessages error:", error);
...
export async function sendMessage(
  ...
  if (!rateLimitResult.allowed) {
    ...
    throw new Error(
      `Terlalu banyak pesan. Coba lagi dalam ${waitSeconds} detik.`,
    );
  }
  ...
  if (error) {
    if (import.meta.env.DEV)
      logger.error("[MessageApi] sendMessage error:", error);
...
export async function createThread(
  ...
  if (error) {
    if (import.meta.env.DEV)
      logger.error("[MessageApi] createThread error:", error);
...
export async function markThreadRead(
  ...
  const field =
    role === "parent" ? "parent_unread_count" : "teacher_unread_count";
  ...
  if (error) {
    if (import.meta.env.DEV)
      logger.error("[MessageApi] markThreadRead error:", error);
