...
const MODES: Array<{
...
}> = [
  {
    value: "quiz",
    label: "Kuis",
    desc: "Soal pilihan ganda siap pakai",
    icon: HelpCircle,
  },
  {
    value: "reading",
    label: "Bacaan",
    desc: "Blok teks bacaan untuk lesson",
    icon: FileText,
  },
  {
    value: "writing",
    label: "Tugas",
    desc: "Brief tugas menulis/esai",
    icon: PenTool,
  },
];
...
function resolveModeFromArtifact(
  ...
  if (
    explicitMode === "quiz" ||
    explicitMode === "reading" ||
    explicitMode === "writing"
  ) {
    return explicitMode;
  }

  if (
    artifact.output.quiz_payload &&
    typeof artifact.output.quiz_payload === "object"
  ) {
    return "quiz";
  }
...
export function AssessmentTab() {
  ...
  const [assignmentPayload, setAssignmentPayload] =
    useState<AssignmentDraftPayload | null>(null);
  ...
  useEffect(() => {
    if (!hydratedArtifact || hydratedArtifact.artifact_kind !== "assessment")
      return;
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      addToast({
        type: "error",
        message:
          err instanceof Error ? err.message : "Gagal menghasilkan asesmen.",
      });
    }
  };
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      addToast({
        type: "error",
        message:
          err instanceof Error ? err.message : "Gagal menerapkan asesmen.",
      });
    }
  };
  ...
  const isPreviewVisible =
    blocks.length > 0 || !!quizPayload || !!assignmentPayload;
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
          Hasilkan asesmen langsung dari konteks pelajaran aktif. Semua hasil
          tetap berupa draft sampai Anda menerapkannya ke builder.
        </p>
