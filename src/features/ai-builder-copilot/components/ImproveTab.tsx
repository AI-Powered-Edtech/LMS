...
export function ImproveTab() {
  ...
  const [selectedAction, setSelectedAction] = useState<TransformAction | null>(
    null,
  );
  ...
  const activeBlock = state.activeLesson?.blocks.find(
  ...
  if (!activeBlock || !blockContent) {
    return (
      <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Pilih blok teks untuk menggunakan fitur perbaikan. Klik pada blok
          konten di editor.
        </p>
      </div>
    );
  ...
  const handleTransform = async (action: TransformAction) => {
    ...
    try {
      ...
      addToast({
        type: "error",
        message:
          err instanceof Error ? err.message : "Gagal mentransformasi konten.",
      });
      ...
      <div className="flex flex-col h-full">
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">
            Hasil:{" "}
            {selectedAction
              ? TRANSFORM_ACTION_LABELS[selectedAction]
              : "Transformasi"}
          </h3>
          <DiffPreview
