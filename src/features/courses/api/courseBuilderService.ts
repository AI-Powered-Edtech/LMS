...
export const courseBuilderService = {
  ...
  async reorderModules(moduleIds: string[]): Promise<void> {
    ...
    const { error } = await supabase.from('course_modules').upsert(updates)
    if (error) throw new Error(error.message)
  },
  ...
  async updateLesson(
    lessonId: string,
    data: Partial<
      Pick<BuilderLesson, 'title' | 'type' | 'is_published' | 'duration_minutes' | 'passing_score'>
    >
  ): Promise<void> {
  ...
  async createBlock(lessonId: string, type: string, tenantId: string): Promise<LessonBlock> {
    const { count } = await supabase
      .from('lesson_resources')
      .select('id', { count: 'exact', head: true })
      .eq('lesson_id', lessonId)
  ...
  async updateBlock(
    ...
    const { error } = await supabase.from('lesson_resources').update(data).eq('id', blockId)
    if (error) throw new Error(error.message)
  },

  async deleteBlock(blockId: string): Promise<void> {
    const { error } = await supabase.from('lesson_resources').delete().eq('id', blockId)
    if (error) throw new Error(error.message)
  },
