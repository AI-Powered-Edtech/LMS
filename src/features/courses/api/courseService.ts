...
export const courseService = {
  ...
  async fetchCourses({
    ...
    if (error) {
      ...
      let fallbackQuery = db
        .from("courses")
        .select(
          "id, title, description, status, created_at, updated_at, created_by, tenant_id",
          {
            count: "exact",
          },
        )
      ...
      if (fallback.error) {
        logDevError(
          "courseService",
          "Error fetching courses (fallback):",
          fallback.error,
        );
  ...
  async getCourseById(
    ...
    if (getApiBackend() === "vil") {
      const requestId = createRequestId();
      const result = await requestVil<Course>(`/api/v1/courses/${courseId}`, {
        requestId,
      });
  ...
  async getTeacherName(
    ...
    if (!membership) {
      logDevWarn(
        "courseService",
        "Teacher does not belong to tenant, skipping name fetch.",
      );
      return null;
    }
