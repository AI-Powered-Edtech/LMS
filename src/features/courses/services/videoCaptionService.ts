...
export const videoCaptionService = {
  ...
  async deleteCaption(captionId: string): Promise<void> {
    ...
    if (selectError) throw new Error(`Failed to fetch caption: ${selectError.message}`)

    const url = caption.vtt_url
  ...
  async setDefaultCaption(captionId: string, lessonId: string): Promise<void> {
    ...
    if (error) throw new Error(`Failed to set default caption: ${error.message}`)
  },
}
