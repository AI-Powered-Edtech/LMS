...
export const documentApi = {
  ...
  async getCategoryCounts(
    ...
    let query = db.from<any>("school_documents").select("category");
    if (tenantId) {
    ...
    if (error)
      throw new Error(`Gagal memuat jumlah kategori: ${error.message}`);
  ...
  async updateDocument(
    ...
    const { data, error } = (await db
      ...
      .eq("id", id)
      .select("*")
      .single()) as { data: SchoolDocument; error: Error | null };
  ...
  async deleteDocument(id: string): Promise<void> {
    ...
    const { data: doc, error: selectError } = (await db
      ...
      .select("file_url")
      .eq("id", id)
      .single()) as { data: { file_url: string } | null; error: Error | null };

    if (selectError)
      throw new Error(`Dokumen tidak ditemukan: ${selectError.message}`);
