...
export async function fetchFinancePage(
  ...
  const count =
    rows.length > 0
      ? Number((data as Record<string, unknown>[])[0]?.total_count ?? 0)
      : 0;
...
export async function fetchFinanceMonthly(
  tenantId: string,
): Promise<MonthlyData[]> {
...
export async function reconcileInvoicePayment(
  ...
  if (error) {
    if (import.meta.env.DEV)
      logger.error("[Finance] reconcile_invoice_payment error:", error);
...
export async function sendInvoiceReminders(
  ...
  if (error) {
    if (import.meta.env.DEV)
      logger.error("[Finance] send_invoice_reminders error:", error);
