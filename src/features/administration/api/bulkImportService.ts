...
export async function createImportJob(tenantId: string): Promise<string> {
  ...
  return (data as { id: string }).id as string;
}
...
export async function getImportJobRows(
  ...
  const { data, error } = await db
    .from<any>("bulk_import_job_rows")
    .select(
      "row_number, email, full_name, role, nis, nomor_hp, status, error_reason",
    )
