...
export const administrationService = {
  ...
  async toggleTenantModule(
    ...
    try {
      ...
      if (error) {
        if (import.meta.env.DEV)
          logger.error("Failed to update tenant module:", error);
        throw error;
      }
    } catch (error) {
      if (import.meta.env.DEV)
  ...
  async getSyncHistory(): Promise<SyncHistoryItem[]> {
    try {
      ...
      if (error) {
        ...
        if (import.meta.env.DEV)
          logger.warn("No sync history available:", error.message);
        return [];
      }
      ...
      if (import.meta.env.DEV)
        logger.warn("Error fetching sync history, returning empty:", error);
      return [];
    }
  },
  ...
  getDefaultModules(): TenantModuleConfig[] {
    return [
      ...
      },
      {
        ...
        name: "Tugas Kelompok",
        description:
          "Kolaborasi tugas kelompok dengan sinkronisasi Google Classroom.",
        isEnabled: true,
        isCore: false,
        targetRoles: ["teacher", "student"],
      },
  ...
  async getAuditLogs(params: {
    ...
    if (error) {
      ...
      if (import.meta.env.DEV)
        logger.warn(
          "[AuditDashboard] get_audit_logs RPC unavailable:",
          error.message,
        );
      return [];
    }
  ...
  async getAppMetrics(): Promise<
    Array<{ metric_name: string; metric_value: number }>
  > {
    const { data, error } = await db
      .from<any>("app_metrics")
      .select("metric_name, metric_value")
      .order("recorded_at", { ascending: false })
      .limit(50);
    if (error) {
      if (import.meta.env.DEV)
        logger.warn("Could not fetch app_metrics:", error);
      return [];
    }
  ...
  async fetchStudentsForInvoice(
    ...
    if (error) {
      if (import.meta.env.DEV)
        logger.warn("[Finance] fetchStudentsForInvoice error:", error);
      return [];
    }
