import { AlertTriangle, BarChart2, MessageSquare, Star, Users2 } from 'lucide-react'
import { motion } from 'motion/react'

import { cn } from '@/utils/cn'

import { usePeerReviewConfig, useReviewsBySubmission } from '../queries/peerReviewQueries'
...
export function PeerReviewSummary({
  ...
  const { data: reviews, isLoading } = useReviewsBySubmission(submissionId, tenantId)
  ...
  const scores = submittedReviews.map((r) => r.overall_score).filter((s): s is number => s !== null)
  const avgScore =
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-center">
          ...
          <p className="text-lg font-bold text-slate-800 dark:text-white">{reviews.length}</p>
          <p className="text-xs text-slate-500 dark:text-slate-400">Total Review</p>
        </div>
        <div className="p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-center">
          <div className="flex items-center justify-center gap-1 mb-1">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">Rata-rata Nilai</p>
        </div>
        <div className="p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-center">
          <div className="flex items-center justify-center gap-1 mb-1">
      ...
      <div className="space-y-3">
        {reviews.map((review, i) => (
          <ReviewItem key={review.id} review={review} index={i} isAnonymous={isAnonymous} />
        ))}
      </div>
    </div>
  )
