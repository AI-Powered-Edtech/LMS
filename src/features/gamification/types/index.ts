/**
 * Gamification types for EduSync LMS
 *
 * These types are shared across the gamification feature module,
 * including streak tracking, badges, leaderboards, XP, and achievements.
 */

/**
 * Represents a user's learning streak data
 */
export interface UserStreak {
  user_id: string
  tenant_id: string
  current_streak: number
  longest_streak: number
  last_activity_date: string
  updated_at: string
}

/**
 * Represents a badge/achievement that can be earned (v1 compat)
 */
export interface Badge {
  id: string
  name: string
  description: string
  icon: string
  created_at: string
}

/**
 * Represents a badge that has been awarded to a user (v1 compat)
 */
export interface UserBadge {
  badge_id: string
  badge: Badge
  earned_at: string
}

/**
 * Represents a single entry on the leaderboard
 */
export interface LeaderboardEntry {
  rank: number
  score: number
  user_id: string
  profiles?: {
    full_name: string
    avatar_url: string | null
    level: number
  } | null
}

// ============================================================
// SP-20: Achievement System types
// ============================================================

export type BadgeRarity = 'common' | 'rare' | 'epic' | 'legendary'
export type BadgeType = 'completion' | 'streak' | 'mastery' | 'speed' | 'social'

/** Badge definition from get_student_badges RPC */
export interface BadgeDefinition {
  badge_id: string
  name: string
  description: string
  icon_emoji: string
  badge_type: BadgeType
  xp_reward: number
  rarity: BadgeRarity
  criteria: Record<string, unknown>
  is_earned: boolean
  earned_at: string | null
}

/** Certificate from get_student_certificates RPC */
export interface Certificate {
  id: string
  course_id: string
  course_title: string
  certificate_number: string
  issued_at: string
  template_config: Record<string, unknown>
}

// ============================================================
// SP-21: Streaks, XP & Leaderboard v2 types
// ============================================================

export type XPSourceType =
  | 'lesson_complete'
  | 'quiz_score'
  | 'streak_bonus'
  | 'badge_earned'
  | 'assignment_submit'

// ============================================================
// Sesi 1: XP Event Types — XP dari Semua Aktivitas
// ============================================================

/**
 * All activity types that can award XP.
 * Used by useAwardXP hook to trigger XP from various activities.
 */
export type XPEventType =
  | 'quiz_pass'          // Lulus kuis
  | 'lesson_complete'    // Selesai baca pelajaran
  | 'assignment_submit'  // Kirim tugas
  | 'assignment_ontime'  // Bonus kirim tepat waktu
  | 'discussion_post'    // Post diskusi (max 3/hari)
  | 'daily_login'        // Login harian
  | 'perfect_score'      // Nilai sempurna

/** XP amount mapping per event type */
export const XP_AMOUNTS: Record<XPEventType, number> = {
  quiz_pass: 30,
  lesson_complete: 10,
  assignment_submit: 15,
  assignment_ontime: 10,
  discussion_post: 5,
  daily_login: 5,
  perfect_score: 20,
}

/** Daily caps for repeatable events */
export const XP_DAILY_CAPS: Partial<Record<XPEventType, number>> = {
  discussion_post: 3, // max 3 posts per day
  daily_login: 1,     // max 1 per day
}

export interface XPTransaction {
  xp_amount: number
  source_type: XPSourceType
  source_id: string | null
  created_at: string
}

/** Student XP profile from get_student_xp_profile RPC */
export interface StudentXPProfile {
  total_xp: number
  level: number
  xp_current_level: number
  xp_next_level: number
  streak_current: number
  streak_longest: number
  last_active: string | null
  recent_xp: XPTransaction[]
}

/** Leaderboard v2 entry from get_leaderboard_v2 RPC */
export interface LeaderboardV2Entry {
  rank: number
  user_id: string
  student_name: string
  avatar_url: string | null
  value: number
  level: number
  streak: number
}

export type LeaderboardSortBy = 'xp' | 'streak'
export type LeaderboardPeriod = 'weekly' | 'monthly' | 'all_time'

// ============================================================
// Sesi 2: Streak Freeze types
// ============================================================

/** State for streak freeze feature */
export interface StreakFreezeState {
  freezesAvailable: number   // max 2 per week
  freezeUsedToday: boolean
  autoFreezeOnHoliday: boolean
  lastFreezeUsedAt: string | null
}

/** Default streak freeze state */
export const DEFAULT_STREAK_FREEZE: StreakFreezeState = {
  freezesAvailable: 2,
  freezeUsedToday: false,
  autoFreezeOnHoliday: false,
  lastFreezeUsedAt: null,
}

// ============================================================
// Sesi 6: Daily Mission types
// ============================================================

export type MissionStatus = 'locked' | 'active' | 'completed' | 'claimed'
export type MissionDifficulty = 'easy' | 'medium' | 'hard'

/** A single daily mission definition */
export interface DailyMission {
  id: string
  title: string
  description: string
  icon: string // emoji
  difficulty: MissionDifficulty
  xpReward: number
  progress: number
  target: number
  status: MissionStatus
  eventType: XPEventType // linked to XP system
}

/** Daily mission pool — rotates daily server-side */
export const MISSION_POOL: Omit<DailyMission, 'id' | 'progress' | 'status'>[] = [
  {
    title: 'Pelajar Rajin',
    description: 'Selesaikan 3 pelajaran',
    icon: '📚',
    difficulty: 'medium',
    xpReward: 50,
    target: 3,
    eventType: 'lesson_complete',
  },
  {
    title: 'Kuis Master',
    description: 'Lulus 2 kuis',
    icon: '🧠',
    difficulty: 'medium',
    xpReward: 60,
    target: 2,
    eventType: 'quiz_pass',
  },
  {
    title: 'Diskusi Aktif',
    description: 'Buat 3 post diskusi',
    icon: '💬',
    difficulty: 'easy',
    xpReward: 30,
    target: 3,
    eventType: 'discussion_post',
  },
  {
    title: 'Nilai Sempurna',
    description: 'Raih 1 nilai sempurna di kuis',
    icon: '💯',
    difficulty: 'hard',
    xpReward: 80,
    target: 1,
    eventType: 'perfect_score',
  },
  {
    title: 'Tepat Waktu',
    description: 'Kirim 2 tugas tepat waktu',
    icon: '⏰',
    difficulty: 'medium',
    xpReward: 40,
    target: 2,
    eventType: 'assignment_ontime',
  },
]

/** Difficulty config for UI rendering */
export const DIFFICULTY_CONFIG: Record<MissionDifficulty, { label: string; color: string; bg: string }> = {
  easy: {
    label: 'Mudah',
    color: 'text-green-600 dark:text-green-400',
    bg: 'bg-green-100 dark:bg-green-900/30',
  },
  medium: {
    label: 'Sedang',
    color: 'text-yellow-600 dark:text-yellow-400',
    bg: 'bg-yellow-100 dark:bg-yellow-900/30',
  },
  hard: {
    label: 'Sulit',
    color: 'text-red-600 dark:text-red-400',
    bg: 'bg-red-100 dark:bg-red-900/30',
  },
}

/** Rarity display config */
export const RARITY_CONFIG: Record<BadgeRarity, { border: string; bg: string; label: string }> = {
  common: {
    border: 'border-slate-300 dark:border-slate-600',
    bg: 'bg-slate-100 dark:bg-slate-800',
    label: 'Umum',
  },
  rare: {
    border: 'border-blue-400 dark:border-blue-600',
    bg: 'bg-blue-50 dark:bg-blue-900/30',
    label: 'Langka',
  },
  epic: {
    border: 'border-purple-400 dark:border-purple-600',
    bg: 'bg-purple-50 dark:bg-purple-900/30',
    label: 'Epik',
  },
  legendary: {
    border: 'border-yellow-400 dark:border-yellow-600',
    bg: 'bg-yellow-50 dark:bg-yellow-900/30',
    label: 'Legendaris',
  },
}
