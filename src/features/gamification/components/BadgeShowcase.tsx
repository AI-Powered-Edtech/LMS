    ...
    return (
        ...
        >
            ...
            <div className={cn(
                ...
                <span className={cn(!isEarned && 'opacity-60')}>{badge.icon_emoji}</span>
                {!isEarned && (
                ...
                <>
                    ...
                    {isEarned && badge.earned_at && (
                        <span className="text-[9px] font-medium text-emerald-600 dark:text-emerald-400">
                            Diraih {new Date(badge.earned_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                        </span>
                    )}
                    ...
                    {badge.xp_reward > 0 && (
                        <span className={cn(
                            'text-[9px] font-bold',
                            isEarned ? 'text-yellow-600 dark:text-yellow-500' : 'text-slate-400 dark:text-slate-500',
                        )}>
                            +{badge.xp_reward} XP
                        </span>
                    )}
    ...
    if (compact) {
        return (
            <div className="grid gap-3 grid-cols-4 sm:grid-cols-6">
                {earned.map(b => <BadgeCard key={b.badge_id} badge={b} compact />)}
                {locked.map(b => <BadgeCard key={b.badge_id} badge={b} compact />)}
            </div>
        );
    }
        ...
        <div className="space-y-5">
            {earned.length > 0 && (
                <div>
                    ...
                    <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
                        {earned.map(b => <BadgeCard key={b.badge_id} badge={b} />)}
                    </div>
                </div>
            )}
            ...
            {locked.length > 0 && (
                <div>
                    ...
                    <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
                        {locked.map(b => <BadgeCard key={b.badge_id} badge={b} />)}
                    </div>
                </div>
            )}
        </div>
