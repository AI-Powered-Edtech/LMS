        ...
        <div className="space-y-6">
                    ...
                    >
                        <div className="rounded-xl border border-indigo-200 dark:border-indigo-800 bg-indigo-50/50 dark:bg-indigo-950/20 p-4 space-y-4">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    <label className="text-xs font-bold text-slate-600 dark:text-slate-400">Nama</label>
                                    <input
                                        value={editing.name}
                                ...
                                <div>
                                    <label className="text-xs font-bold text-slate-600 dark:text-slate-400">Emoji</label>
                                    <input
                            ...
                            <div>
                                <label className="text-xs font-bold text-slate-600 dark:text-slate-400">Deskripsi</label>
                                <input
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                <div>
                                    <label className="text-xs font-bold text-slate-600 dark:text-slate-400">Tipe</label>
                                    <select
                                        value={editing.badge_type}
                                        onChange={e => setEditing({ ...editing, badge_type: e.target.value as BadgeType })}
                                        className="mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm"
                                    >
                                ...
                                <div>
                                    <label className="text-xs font-bold text-slate-600 dark:text-slate-400">Raritas</label>
                                    <select
                                        value={editing.rarity}
                                        onChange={e => setEditing({ ...editing, rarity: e.target.value as BadgeRarity })}
                                        className="mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm"
                                    >
                                ...
                                <div>
                                    <label className="text-xs font-bold text-slate-600 dark:text-slate-400">XP Reward</label>
                                    <input
                                        type="number"
                                        min={0}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    <label className="text-xs font-bold text-slate-600 dark:text-slate-400">Kriteria</label>
                                    <select
                                ...
                                {CRITERIA_TYPES.find(c => c.value === editing.criteria_type)?.hasThreshold && (
                                    <div>
                                        <label className="text-xs font-bold text-slate-600 dark:text-slate-400">Threshold</label>
                                        <input
                                            type="number"
                                            min={1}
                                            value={editing.criteria_threshold}
                                            onChange={e => setEditing({ ...editing, criteria_threshold: Number(e.target.value) })}
                                            className="mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm"
                                        />
                                    </div>
                                )}
            ...
            {tenantBadges.length > 0 && (
                <div className="space-y-2">
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Badge Kustom</h4>
                    <div className="space-y-2">
            ...
            {systemBadges.length > 0 && (
                <div className="space-y-2">
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Badge Sistem</h4>
                    <div className="space-y-2">
...
function BadgeRow({ badge, onEdit, isSystem }: { badge: any; onEdit?: () => void; isSystem?: boolean }) {
        ...
        <div className={cn(
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                    <span className="font-bold text-sm text-slate-800 dark:text-slate-200 truncate">{badge.name}</span>
                    <span className={cn(
                        'rounded-full px-1.5 py-0.5 text-[9px] font-bold',
                        rarity.bg, 'text-slate-600 dark:text-slate-300',
                    )}>
                        {rarity.label}
                    </span>
                    {!badge.is_active && (
                        <span className="rounded-full bg-red-100 px-1.5 py-0.5 text-[9px] font-bold text-red-600">Nonaktif</span>
                    )}
                </div>
            ...
            {!isSystem && onEdit && (
                <button onClick={onEdit} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
