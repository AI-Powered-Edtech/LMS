...
export function LeaderboardV2() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useLeaderboardV2({ sortBy, period });
  ...
  const isCurrentUser = useCallback(
    (e: LeaderboardV2Entry) => e.user_id === user?.id,
    [user?.id],
  );
  const currentUserEntry = useMemo(
    () => list.find(isCurrentUser),
    [list, isCurrentUser],
  );
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Terjadi kesalahan saat memuat papan peringkat. Silakan coba lagi
          nanti.
        </p>
      </div>
    );
...
function PodiumCard({
  ...
  const avatarSize =
    rank === 1 ? "w-16 h-16 md:w-20 md:h-20" : "w-12 h-12 md:w-16 md:h-16";
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "w-full rounded-t-2xl border-t-4 mt-2",
          heights[rank],
          podiumGradient,
        )}
      />
