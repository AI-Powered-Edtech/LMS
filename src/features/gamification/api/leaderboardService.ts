...
async function fetchProfiles(userIds: string[]): Promise<Map<string, ProfileLite>> {
...
): Array<T & { profiles: Pick<ProfileLite, 'full_name' | 'avatar_url'> | null }> {
	return rows.map((r) => {
...
export const leaderboardService = {
	...
	async getLeaderboard(_classId: string, tenantId: string): Promise<LeaderboardEntry[]> {
		...
		return hydrated.map((e) => ({
			...e,
			score: (e as { points?: number; score?: number }).points ??
