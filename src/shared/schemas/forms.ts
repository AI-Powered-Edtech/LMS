...
export type ForgotPasswordFormData = v.InferOutput<
...
export type ResetPasswordFormData = v.InferOutput<
...
export const AssignmentFormSchema = v.pipe(
  ...
  v.forward(
    v.partialCheck(
      ...
      (input) =>
        input.allow_text_submission ||
        input.allow_file_submission ||
        input.allow_link_submission,
...
export const QuizFormSchema = v.object({
  ...
  time_limit_minutes: v.nullable(
    v.pipe(v.number(), v.minValue(0, "Waktu tidak boleh negatif")),
  ),
