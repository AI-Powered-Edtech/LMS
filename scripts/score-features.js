    ...
    for (const entry of fs.readdirSync(d, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) walk(fullPath)
        ...
        try {
          ...
          console.error(`[score-features] Tidak dapat membaca ${fullPath}: ${err.message}`)
        }
      }
    }
  }
...
function scoreFeature(featureName) {
  ...
  const uiux = Math.min(100, 20 + Math.min(50, darkModeFiles * 5) + Math.min(30, skeletonFiles * 8))
  ...
  const catatan = gaps.length === 0 ? 'Semua dimensi terpenuhi ✓' : `Kurang: ${gaps.join(', ')}`
...
async function main() {
  ...
  const scores = features.map((f) => {
    ...
    console.log(
      `  ${icon} ${score.name.padEnd(22)} C:${String(score.completeness).padStart(3)} D:${String(score.dokumentasi).padStart(3)} U:${String(score.uiux).padStart(3)} → ${score.total}/100`
    )
    return score
  })
