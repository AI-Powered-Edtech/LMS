...
**Impact:** In PostgreSQL, RLS policies on a partitioned parent table do *not* cascade to child partitions automatically. Any direct query bypassing the parent (or if accessed improperly) could expose all tenant attempts.
...
**Impact:** PostgreSQL evaluates these function calls *per row*. Scanning a table with 100,000 rows will execute the auth function 100,000 times, causing massive CPU load and slow query times.
**Recommendation:** Wrap volatile function calls in a `(SELECT ...)` block so PostgreSQL caches the result per query.
*Example:* `(tenant_id = (SELECT get_my_tenant_id()))` instead of `(tenant_id = get_my_tenant_id())`.
